...
function printBill(doc_no,doc_tp,co_promo_code,status_no){
	...
	$(function(){
		...
		$("#transfer_commit").click(function(){
			...
			// jAlert('กรุณาระบุเลขที่เอกสาร'+$dest_branch+ $dest_ip,'ข้อความแจ้งเตือน',function(){
			// 	$doc_no_start.focus();
			// 		return false;
			// });		
			...
			var check_docno={
			     		        ...
			     		        success:function(data){
			     			        if(data=='Y'){
										jAlert('สำเร็จ','ข้อความแจ้งเตือน',function(){
											 return false;});
			     				     }else{
			     				    	 jAlert('เกิดความผิดพลาด','ข้อความแจ้งเตือน',function(){
			     		    				 return false;
			     			    		 });			    		
			     					 }
			     			    }
		...
		//ดึงข้อมูลจากDB กลางมาอัพเดทDBสาขา

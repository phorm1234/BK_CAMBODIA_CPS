	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.status == "y"){
				...
				jAlert('Please close the daily bill....before saving cash again','Msg Alert!',function(){
					//$("#eid").focus().select();
					//return false;
					setTimeout(
			                  function() 
				...
				cleardata();
				jAlert('You have completed the transaction','Msg Alert!',function(){
					//$("#eid").focus().select();
					//return false;
					setTimeout(
			                  function() 
...
$("#eid").keypress(function(event){
	...
	if(keycode == '13'){  //enter
		if(eid == ""){
			jAlert('Please enter your Employee ID','Msg Alert!',function(){
				$("#eid").focus();
				return false;
	     	});
		}else{
			$.ajax({
				...
				success:function(data){	
					if(data.status == "y"){
						...
						jAlert('Employee ID not found','Msg Alert!',function(){
							$("#eid").focus().select();
							return false;
				     	});
					}
	...
	if(keycode == '13'){  //enter
		...
		if(IsNumeric($("#"+elm).val())){
			...
			case 'mcash025': 
			  ...
			  jAlert('Please enter numerical data.','Msg Alert!',function(){
					$("#"+elm).focus().select();
					return false;
		     	});
		  }
...
function save_data(){	
		...
		if($("#chk").val() == 'y'){
			$( "#dialog" ).dialog( "open" );	
			jConfirm('Confirm saving cash check', 'แจ้งเตือน', function(r) {
					if(r==false){
						return false;
					}else{
					...
					$.ajax({
						...
						function(data){
							if(data == 'y'){
								cleardata();
								jAlert('Save success','Msg Alert!',function(){
									setTimeout(
							                  function() 
							                  {
							                     location.reload();
			...
			jAlert("Can not process, Please enter the information.");
		}

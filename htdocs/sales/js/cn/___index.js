...
//		 });
	...
	function saveTransCN(){
		...
		if(cn_sum_quantity=='0'){
			jAlert('Not found the product transaction, can not save','Alert',function(){
				$("#cn_product_id").focus();
				 	return false;
    		 });
		}else if(cn_doc_no_ref.length==0){
			jAlert('Please Enter Doc No.','Alert',function(){
				$("#cn_doc_no").focus();
				 	return false;
    		 });
		}else if(cn_status_no.length==0){
			jAlert('Please Enter type of Doc.','Alert',function(){
				$("#cn_status_no").focus();
				 	return false;
    		 });
		}
		var opts={
				...
				success:function(data){
					...
					if(arr_data[0]=='1'){
						...
						jAlert('Save Success '+arr_data[1],'Save Result',function(){
							//init form cn
							initCnForm();
							initCnTemp();										
		    				 return false;
						...
						jAlert('Error\n'+arr_data[1]+'\n can not save','Save Result',function(){
							 $("#cn_product_id").focus();
							 return false;
			    		 });
					}
			...
			jConfirm('Do you want to save?', 'Confirm Save', function(r) {
			    if(r){	
			    	$.ajax(opts);
				}else{
					$("#cn_product_id").focus();
	...
	function flgCommand(com,grid){
		...
		if (com == 'Delete') {
			...
			jConfirm('Do you want to delete '+del_list_show+' Yes or No?','Confirm Delete', function(r){
		        if(r){
					var opts={
							   type:"POST",
							   cache:false,
							   ...
							   success: function(data){
										if(data=='0'){
											 jAlert('Can not delete this list','Alert',function(){
												 return false;
											  });
										}else{
											getCN();
	        ...
	        if(del_list==''){
	        	 jAlert('You did not select any list','Alert',function(){
					 return false;
				  });
		    }
		}//if delete
		...
		var opts={
		        	...
		        	success:function(data){
			        	if(data=='1'){
			        		...
			        		jAlert('Can not save, please check again','Alert',function(){
								 $("#cn_product_id").focus();
			    				 return false;
				    		 });
				        }
	...
	function msgSelProductError(){
		jAlert('Not enough quantity of onhand, please key the quantity not over onhand of select list','Alert',function(r){
			if(r){
				$("#cn_quantity").focus();
			}
    		return false;
	...
	function bwsCnProduct(product_id){
		...
		if(doc_no_ref.length==0){
			...
			jAlert('Please Enter Members No.','Alert',function(){
				$("#cn_member_no").focus();
				 	return false;
    		 });
		}else{
			...
			var dialogOpts_cnproduct = {
					...
					title:"Items list by Doc. No. "+doc_no_ref,
					closeOnEscape:true,
					open: function(){ 
																					...
																					buttons: {
																						"Submit":function(){ 	
																								...
																								if(lot_expire_alternate.length==0){
																									jAlert('Please specify the date of manufacture','Alert',function(){
																								...
																								if(lot_no.length==0){
																									jAlert('Please insert lot no.','Alert',function(){
																										$("#lot_no").focus();
																					  				 	return false;
																						    		 });
																									return false;
																								...
																								if(cn_remark2.length==0){
																									jAlert('Please insert a reason','Alert',function(){
																										$("#cn_remark2").focus();
																					  				 	return false;
																						    		 });
																									return false;
		...
		var opts={
	        	...
	        	success:function(data){
		        	if(data=='1'){
		        		...
		        		jAlert('Number of items exceeded in the list, please check again','Alert',function(){
							 $("#cn_product_id").focus();
		    				 return false;
			    		 });
			        }else if(data=='3'){
		        		jAlert('Product ID not found on this bill., please check again','Alert',function(){
							 $("#cn_product_id").focus();
		    				 return false;
			    		 });
			        }
	...
	$(function(){
		...
		$("#tbl_cn").flexigrid(
				{
					...
					colModel : [
						...
						{display: 'Product ID', name : 'product_id', width : 70, sortable : true, align: 'center'},
						{display: 'Promo Code', name : 'promo_code', width : 70, sortable : true, align: 'center'},
						{display: 'Product Name', name : 'product_name',width :gWidth, sortable : false, align:'left'},
						{display: 'Members Discount', name : 'discount', width : 90, sortable : true, align: 'center'},
						{display: 'Quantity', name : 'quantity', width : 50, sortable : false, align: 'center'},
						{display: 'Price', name : 'price', width :70, sortable : false, align: 'center'},
						{display: 'Amount', name : 'amount', width :70, sortable : false, align: 'center'},
						{display: 'Discount', name : 'discount', width :70, sortable : false, align: 'center'},
						{display: 'Total Amount', name : 'net_amt', width :90, sortable : false, align: 'center'}
						],
					sortname: "id",
					sortorder: "asc",
					action:'gettmp',
		...
		$("#cn_doc_no").keypress( function(evt) {			
	        ...
	        if(key == 13){
	            ...
	            if(cn_doc_no==''){
	            	jAlert('Please insert Doc. No.','Alert',function(){
						 $("#cn_doc_no").focus();
	    				 return false;
		    		 });
		        }else{	   
			        $.ajax({
			        		...
			        		success:function(data){
				        		var arr_data=data.split('#');
				        		if(arr_data[0]=='1'){
				        			jAlert('Can not found Doc. No. , Please check again','Alert',function(b){
										 $("#cn_doc_no").val('').focus();
					    				 return false;
						    		 });
					        	}else if(arr_data[0]=='2'){
					        		jAlert('This Doc. No. has been Cancle, can not process, please check again','Alert',function(b){
										 $("#cn_doc_no").val('').focus();
					    				 return false;
						    		 });
						        }else if(arr_data[0]=='3'){
						        	jAlert('The document number is not in the backward condition of'+arr_data[1]+' days, please check again','Alert',function(b){
										 $("#cn_doc_no").val('').focus();
					    				 return false;
						    		 });
		...
		$("#cn_status_no").keypress( function(evt) {									
	        ...
	        if(key == 13){
	            ...
	            if(cn_doc_no.length==0){
	            	...
	            	jAlert('Please Enter Condition','Alert',function(){
						 $("#cn_status_no").focus();
	    				 return false;
		    		 });
		        }else{
	            	...
	            	$.getJSON(
	        				...
	        				function(data){
	        					if(data!='0'){
	        						...
	        						jAlert('Can not found this Member No., Please check again','Alert',function(){
	        							 $("#cn_member_no").focus();
	        		    				 return false;
	        			    		});	        						 
		        				}
	        ...
	        if(key == 13){
		        ...
		        var cn_fullname= $("#cn_fullname");
		        	...
		        	if(cn_fullname.length<1){
		        		jAlert('Please Enter Members Name - Surname','Alert',function(){			        		
							 $("#cn_fullname").focus();
		    				 return false;
			    		});
			    		return false;
	            	...
	            	jAlert('Please Enter Doc. No.','Alert',function(){
		    		 ...
		    		 return false;
	           		...
	           		if(cn_member_no.length==0){
		            	...
		            	jAlert('Please Enter Product ID.','Alert',function(){
							 $("#cn_product_id").focus();
		    				 return false;
			    		 });
		            	 return false;
...
//	            $.ajax(opts_chk_seq);
		...
		$("#btn_cn_cancel").click(function(){
			jConfirm('Do you want to Cancle?', 'Confirm Cancle', function(r) {
			    if(r){	
			    	initCnForm();
					initCnTemp();
				}
		...
		$("#btn_cn_submit").click(function(e){
				...
				if(cn_sum_net_amt=='x0.00'){
					jAlert('Can not found list of reduce debt, Please check again','Alert',function(){
						 $("#cn_product_id").focus();
	    				 return false;
		    		 });
					return false;
							...
							buttons: {
								"Submit":function(){ 
										...
										if(acc_name.length==0){
											jAlert('Please insert Acc. Name','Alert',function(){
												 $("#acc_name").focus();
							    				 return false;
								    		 });
											return false;
										...
										if(bank_acc.length==0){
											jAlert('Please insert Bank Acc.','Alert',function(){
												 $("#bank_acc").focus();
							    				 return false;
								    		 });
											return false;
										...
										if(bank_name.length==0){
											jAlert('Please insert bank name/branch','Alert',function(){
												 $("#bank_name").focus();
							    				 return false;
								    		 });
											return false;
										...
										if(tel1.length==0){
											jAlert('Please insert phone no.1','Alert',function(){
												 $("#tel1").focus();
							    				 return false;
								    		 });
											return false;

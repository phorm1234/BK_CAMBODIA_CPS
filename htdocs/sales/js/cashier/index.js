...
function capturecamera(doc_no) {
    $.ajax({
        ...
        url: "/sales/cashier/capturecamera",
        data: {
            doc_no: doc_no,
            rnd: Math.random(),
        },
        success: function (data) {
            if (data == "y") {  // y : camera offline or cant get_content
                return false;
                var text = "<b style='color: rgb(227, 22, 22);font-size: 30px;'>กล้องบันทึกภาพตำแหน่งกลางร้านไม่ทำงาน โปรดตรวจสอบกล้อง...</b><br>";
            } else if (data != "404") {
                var text = '<img src="' + data + '" style="width: 279px;padding-left: 12%;"></img>'
            }

            if (data != "404") {  // check cam_ip_1 from database
                $("<div id='dlgCloseCaptureCamera'></div>").dialog({
                    autoOpen: true,
                    modal: true,
                    width: 430,
                    height: 320,
                    title: "<span class='ui-icon ui-icon-cart'></span>บันทึกรูปภาพสำเร็จ",
                    position: "middle",
                    resizable: false,
                    closeOnEscape: true,
                    open: function () {
                        ...
                        $(this).dialog("widget").find(".ui-dialog-buttonpane").css({
                            padding: ".1em .1em .1em 0",
                            margin: "0 0 0 0",
                            "background-color": "#a2a3bc",
                        });
                        $(this).dialog("widget").find("button").css({
                            padding: "0 .1em 0 .1em",
                            margin: "0 .1em 0 0",
                        });
                        $(this).html(text);
            ...
            if (obj_json.status_ck == 'Y') {
                ...
                if (obj_json.month_enable == 'Y' && obj_json.outofday == 'Y' && obj_json.outtime == 'Y') {
                    jAlert('สามารถใช้สิทธิได้ระหว่างวันที่ ' + obj_json.start_day + ' ถึงวันที่ ' + obj_json.end_day + '\n' +
                        ' เวลา ' + obj_json.start_time + ' ถึง ' + obj_json.end_time + ' น.', 'WARNING!',
                        function (b) {
                            initTblTemp();
                            initFormCashier();
                            initFieldOpenBill();
...
function registermempay() {
    if ($('#dlgKeyMobile').dialog('isOpen') === true) {
        if ($('#ajaxBusy').length) {
            $('#ajaxBusy').hide();
        }
        return false;
    ...
    $("<div id='dlgKeyMobile'></div>").dialog({
        autoOpen: true,
        width: '25%',
        height: 'auto',
        modal: true,
        ...
        closeOnEscape: true,
        title: "<span class='ui-icon ui-icon-person'></span>Member register",
        open: function () {
            if ($('#ajaxBusy').length) {
                $('#ajaxBusy').hide();
            }
            ...
            $.ajax({
                type: 'post',
                url: '/sales/member/formregistermobile',
                data: {
                    rnd: Math.random()
                },
                success: function (data) {
                    $("#dlgKeyMobile").html(data);
                    $("#key_mobile_no").val('');
                    $("#key_mobile_no").focus();
                    $('#btn_otp_code_req').keypress(function (evt) {
                        var key = evt.keyCode ? evt.keyCode : evt.which ? evt.which : evt.charCode;
                        if (key == 13) {
                            evt.preventDefault();
                            $("#btn_otp_code_req").trigger("click");
                            return false;
                        }
                    });

                    $("#btn_otp_code_req").click(function (evt) {
                        evt.preventDefault();
                        var mobile_no = $("#key_mobile_no").val();
                        mobile_no = $.trim(mobile_no);
                        if (mobile_no.length < 1) {
                            jAlert('Please enter Mobile', 'Attention', function () {
                                $("#key_mobile_no").focus();
                                return false;
                            });
                            return false;
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                var objrq = $.parseJSON(data);
                                if (objrq.status == "OK") {

                                    jAlert(" OTP Code Sent to " + mobile_no + "\n Enter OTP Code ", 'Comfirm', function () {
                                        $("#key_mobile_otp").focus();
                                        $("#tmp_member_id").val(objrq.member);
                                        $('#csh_tmp_dummy').val('');
                                        $('#csh_tmp_dummy_mobile').val('');
                                        return false;
                                    });

                                } else {
                                    jAlert(objrq.msg, 'Attention', function () {
                                        $("#key_mobile_no").focus();
                                        return false;
                                    });
                                }
        ...
        buttons: [{
            ...
            click: function (evt) {
                ...
                evt.stopPropagation();
                /*
                $("#csh_member_no").val("ID16000000037");
                $("#csh_member_no").trigger($.Event("keypress", { keyCode: 13 }));
                $("#dlgKeyMobile").dialog("close");
                return false;
                */
                var mobile_no = $("#key_mobile_no").val();
                mobile_no = $.trim(mobile_no);
                var otp_code = $("#key_mobile_otp").val();
                otp_code = $.trim(otp_code);
                if (mobile_no.length < 1) {
                    jAlert('Please enter mobile number', 'Attention', function () {
                        $("#key_mobile_no").focus();
                        return false;
                    });
                    return false;
                ...
                if (otp_code.length < 4) {
                    jAlert('Please enter OTP Code', 'Attention', function () {
                        $("#key_mobile_otp").focus();
                        return false;
                    });
                    return false;
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            var objsnd = $.parseJSON(data);
                            if (objsnd.status == 'OK') {
                                $('#csh_member_chanel').val("T");
                                $('#csh_member_no').val('');
                                $('#csh_member_no').val($("#member_code").val());
                                $('#csh_tmp_dummy').val($("#member_code").val());
                                $('#csh_tmp_dummy_mobile').val($("#key_mobile_no").val());
                                ...
                                jAlert(objsnd.msg, 'Attention', function () {
                                    $("#key_mobile_no").focus();
                                    return false;
                                });
        ...
        close: function (evt) {
            var member_no = $('#csh_member_no').val();
            var new_member = $('#csh_member_type').html();
            $("#dlgKeyMobile").remove();
            $("#dlgKeyMobile").dialog("destroy");
            if ($('#csh_tmp_dummy_mobile').val() == '') {
                if (new_member == "NEW MEMBER" && member_no == "") {
                    jAlert("Please register", 'Attention', function () {
                        registermempay();
                        return false;
                    });
                } else {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if ($("#dlgSetBagBarcode").dialog("isOpen") !== true) {
                console.log('triger bug' + $("#csh_promo_code").val() )

                if( $("#csh_promo_code").val()=='' &&  $("#manual_promo_code_hide").val() == '2206W301'){
                    initTblTemp();
                    initFormCashier();
                    initFieldOpenBill();
                }else{
            ...
            if ($("#csh_member_no").val() == "" && new_member == "NEW MEMBER") {
                flg_chk_point = 'yes';
            }
            $.getJSON(
                //"/sales/cashier/ajax",
                ...
                function (data) {
                    $.each(data.sumcsh, function (i, m) {
                        ...
                        if (m.exist == 'yes' && parseFloat(m.sum_net_amt) > 0) {   // if (m.exist == 'yes' && parseInt(m.sum_net_amt) != '0') {
...
function saveTransaction() {
    ...
    if ($('#payment_channel').val() == "Alipay") {
        ...
        var credit_tp = $("#sel_credit").val(); //ประเภทบัตรเครดิต
    ...
    var member_ch = $('#csh_member_chanel').val();
    var csh_ticket_no = $('#csh_ticket_no').val();
    //register SSCPS Module

    if ($('#csh_tmp_dummy').val() != '') {
        status_no = '01';
        application_id = 'OPID300';
        csh_mobile_no = $('#csh_tmp_dummy_mobile').val();
        $.ajax({
            type: 'post',
            url: '/sales/member/incdummy',
            cache: false,
            data: {
                rnd: Math.random()
            },
            ...
            if (arr_data[0] == '1') {
                ...
                $("<div id='dlgCloseBill'></div>").dialog({
                    ...
                    open: function () {
                        ...
                        $(this).html('<p align="center" style="margin-top:35px;">Save ' + arr_data[1] + ' Success', 'ผลการบันทึก</p>');
                    },
                    close: function () {
                    ...
                    success: function () {
                        ...
                        if (chk_walkin !== "WALK IN" && (status_no == '00' || status_no == '02' || status_no == '04')) {
                            if (prefix_member_no_idcard == 'ID' && $("#csh_lock_status").val() == 'Y') {
                                ...
                                //alertReturnIdcard();
                            } else if ($("#csh_member_chanel").val() == 'T') {
                                checkSaleMan();
                            } else {
                                conFirmMemberID();
                            }
...
var ajaxShipping = false;

function paymentBillOrg(status_no) {
    /**
     *@desc last modify : 20082018
     *@param String status_no :
     *@return void
     */
    ...
    if (!ajaxShipping) {
        ...
        $.ajax({
            type: 'post',
            url: '/sales/cashier/addpdtautoshipping',
            async: false,
            data: {
                rmd: Math.random()
            },
            success: function (data) {
                ...
                setTimeout(function () { paymentBillOrgNextStep(status_no); }, 400);
...
function paymentBillOrgNextStep(status_no) {
    /**
    ...
    if ($("#dlgPayment").dialog("isOpen") === true) {
        return false;
    } //if
    ...
    var dialogOpts_payment = {
                    ...
                    $("#btn_payment_cancel").click(function (e) {
                        ...
                        jConfirm('Do you want to cancel this payment?', 'CONFIRM MESSAGE', function (r) {
                            if (r) {
                                ...
                                if( $("#csh_promo_code").val() == '2206W301'){
                                    console.log('fn : clear temp from 2206W301')
                                    lastdelpro();
                                    cancelBill();
        ...
        close: function (evt, ui) {
            ...
            // isPayment
            if ( $("#isPayment").val() == '0' && $("#csh_promo_code").val() == '2206W301') {
                    console.log('inif')
                    lastdelpro();
                    cancelBill();
                    initTblTemp();
            ...
            if (data == 1) {
                ...
                if (member_no != '' && play_main_pro == 'N' && play_last_pro == 'N' && csh_gap_promotion != 'Y' && $('#csh_trn_diary2_sl').val() == 'Y') {
                    if (start_baht != '' && end_baht != '') {
                        //*WR17122014
                        var promo_tp = $('#csh_promo_tp').val();
                        if (promo_tp == 'NEWBIRTH' && promo_code == 'BD001') { //เล่นโปรท้ายบิลได้
                            setCshTemp_BD();
                        } else if (promo_tp == 'NEWBIRTH' && promo_code == 'SS12020918') { //ไม่คิดราคา
                            setCshTemp();
                        } else {
                    ...
                    if ($('#csh_member_vip').val() == '1' || $("#csh_status_no").val() == '03' || $("#csh_status_no").val() == '05') {
                        ...
                        if ($("#status_pro").val() == '0') {
                            ...
                            if (promo_code == 'OX02460217' || promo_code == 'OX02460217_2') {
                                ...
                                setCshValTemp();
                                hotchkkey($('#csh_member_type').html(), $("#csh_product_id").val(), $("#csh_quantity").val());
                                return false;
                            }
...
function setCshValTemp() {
    ...
    if (product_id.length == 0) {
        jAlert('กรุณาระบุ รหัสสินค้า1', 'WARNING!', function () {
            $("#csh_product_id").focus();
            return false;
        });
        return false;
    ...
    //*WR25042013
    var new_member = $('#csh_member_type').html();
    if (member_percent1 != '' && member_no == '' && new_member == "") {
        jAlert('Walkin discount not allow Please reopen bill', 'Attention', function () {
            $("#csh_product_id").focus();
            return false;
        });
        return false;
...
function setCshTemp() {
    ...
    if (product_id.length == 0) {
        jAlert('กรุณาระบุ รหัสสินค้า2', 'WARNING!', function () {
            $("#csh_product_id").focus();
            return false;
        });
    }
                ...
                success: function (data) {
                    ...
                    $(".tableNavPromoPointDetail ul li").not('.nokeyboard').navigate({
                        ...
                        if (selprodetail.promo_id != 0) {
                            ...
                            // setTimeout(function() {  // comment like OP-Cambodia
...
function redeemPoint() {
     ...
     *@return
     */
    initFieldPromt(); // hard code for REDEEM POINT cant key in csh_product_id || TODO::find where disable input id=csh_product_id
    var $csh_status_no = $("#csh_status_no");
    var status_no = jQuery.trim($csh_status_no.val());
    var $csh_member_no = $("#csh_member_no");
    var member_no = jQuery.trim($csh_member_no.val());
...
function setNewCardMember() {
    ...
    switch (ops_day_new) {
        case 'TH1':
            msg_show_sel = "บัตรพฤหัสบดีที่1";
            break;
        case 'TH2':
            msg_show_sel = "บัตรพฤหัสบดีที่2";
            break;
        case 'TH3':
            msg_show_sel = "บัตรพฤหัสบดีที่3";
            break;
        case 'TH4':
            msg_show_sel = "บัตรพฤหัสบดีที่4";
            break;
        case 'TU1':
            msg_show_sel = "บัตรอังคารที่1";
            break;
        case 'TU2':
            msg_show_sel = "บัตรอังคารที่2";
            break;
        case 'TU3':
            msg_show_sel = "บัตรอังคารที่3";
            break;
        case 'TU4':
            msg_show_sel = "บัตรอังคารที่4";
            break;
    }
            ...
            $("#csh_new_card").keypress(function (evt) {
                ...
                if (key == 13) {
                    ...
                    if ($csh_newcard_val.length == 0) {
                        ...
                        switch (ops_day_old) {
                            case 'TH1':
                                msg_show = "บัตรพฤหัสบดีที่1";
                                break;
                            case 'TH2':
                                msg_show = "บัตรพฤหัสบดีที่2";
                                break;
                            case 'TH3':
                                msg_show = "บัตรพฤหัสบดีที่3";
                                break;
                            case 'TH4':
                                msg_show = "บัตรพฤหัสบดีที่4";
                                break;
                            case 'TU1':
                                msg_show = "บัตรอังคารที่1";
                                break;
                            case 'TU2':
                                msg_show = "บัตรอังคารที่2";
                                break;
                            case 'TU3':
                                msg_show = "บัตรอังคารที่3";
                                break;
                            case 'TU4':
                                msg_show = "บัตรอังคารที่4";
                                break;
                        }
                        //*WR27032014 append check opsday old card and new card
                        var opts_member = {
                    ...
                    $('.tableNavCatList ul li').not('.nokeyboard').navigate({
                        ...
                        if (csh_application_type != 'NEW') {
                            ...
                            switch (chk_ops_day_old) {
                                case 'TH1':
                                    str_opsday = "บัตรวันพฤหัสบดีที่1";
                                    break;
                                case 'TH2':
                                    str_opsday = "บัตรวันพฤหัสบดีที่2";
                                    break;
                                case 'TH3':
                                    str_opsday = "บัตรวันพฤหัสบดีที่3";
                                    break;
                                case 'TH4':
                                    str_opsday = "บัตรวันพฤหัสบดีที่4";
                                    break;
                                case 'TU1':
                                    str_opsday = "บัตรวันอังคารที่1";
                                    break;
                                case 'TU2':
                                    str_opsday = "บัตรวันอังคารที่2";
                                    break;
                                case 'TU3':
                                    str_opsday = "บัตรวันอังคารที่3";
                                    break;
                                case 'TU4':
                                    str_opsday = "บัตรวันอังคารที่4";
                                    break;
                                default:
                                    chk_ops_day_old = '';
                            }
                                ...
                                if (m.link_status == 'ONLINE') {
                                    ...
                                    switch (m.cust_day) {
                                        case "TH0":
                                            ...
                                            break;
                                        case "TH1":
                                            cust_day = "พฤหัสบดีที่1";
                                            break;
                                        case "TH2":
                                            cust_day = "พฤหัสบดีที่2";
                                            break;
                                        case "TH3":
                                            cust_day = "พฤหัสบดีที่3";
                                            break;
                                        case "TH4":
                                            cust_day = "พฤหัสบดีที่4";
                                            break;
                                        case "TU1":
                                            cust_day = "อังคารที่1";
                                            break;
...
function callMemberInfo(flg) {
    ...
    if (jQuery.trim(member_no) == '' && flg == 'N') {
        jConfirm('DO YOU WANT MEMBER REGISTER?', 'MEMBER REGISTER', function (r) {
            if (r) {
                $('#csh_member_fullname').html("&nbsp;");
                $('#csh_member_type').html("NEW MEMBER");
                $('#csh_percent_discount').html(5);
                $("#csh_get_point").val("Y");
                $("#csh_member_tp").val("Y");
                initFieldPromt();
                return false;
                        ...
                        var opt_frmcardlost = {
                            ...
                            open: function () {
                                ...
                                $(this).dialog('widget')
                                    ...
                                    .addClass('ui-corner-top');
                                $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({
                                    "background-color": "#CFE2E5", //#CFE2E5
                                    "padding": "0 0 0 0",
                                    "margin": "0 0 0 0",
                                    "font-size": "27px",
                                    "color": "#000"
...
function initFormCashier() {
    ...
    $('#csh_tmp_dummy_mobile').val('');
    $('#csh_tmp_dummy').val('');
    //for avoid bug close btn on paymentDialog
    $("#isPayment").val('0');
} //func
...
function showDlgKeyMobile(flg = '') {
    ...
    if (flg == "ORD") {
        $('#csh_application_type').val("MOBILEORDER");
    }
    $("<div id='dlgKeyMobile'></div>").dialog({
        autoOpen: true,
        width: '25%',
        ...
        position: { my: "center TOP", at: "center center", of: window },
        closeOnEscape: true,
        title: "<span class='ui-icon ui-icon-person'></span>Mobile form",
        open: function () {
            $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({
                "background-color": "#BCDCD7",
                "font-size": "24px",
            ...
            $.ajax({
                type: 'post',
                url: '/sales/member/formkeymobile',
                data: {
                    rnd: Math.random()
                },
                success: function (data) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (objsnd.status == 'OK') {
                                ...
                                $('#csh_member_chanel').val("T");
                                $('#csh_member_no').val('');
                                $('#csh_member_no').val($("#tmp_member_id").val());
                                $('#csh_member_no').focus();
                                $("#csh_member_no").trigger($.Event("keypress", { keyCode: 13 }));
                                $("#dlgKeyMobile").dialog("close");
                            } else {
...
function showDlgRegister(flg = '') {
    ...
    $("<div id='dlgKeyMobile'></div>").dialog({
        ...
        title: "<span class='ui-icon ui-icon-person'></span>Member register",
        open: function () {
            $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({ "background-color": "#BCDCD7", "font-size": "24px", "color": "#000000", "padding": "5 0.1em 0 0.1em" });
            $("#dlgKeyMobile").html('');
            $(this).dialog('widget')
                .find('.ui-dialog-titlebar')
                .removeClass('ui-corner-all')
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (parseInt(arr_data[0]) > 0) {
                ...
                if ($("#csh_promo_code").val() != '2206W301') {
                    jAlert('Not found Sell ​​Items. Please check and try again2.', 'WARNING!', function () {
                        $("#btn_cal_promotion").removeAttr("disabled");
                        $("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
                        return false;
                    });
...
$(function () {
    ...
    $("#btn_cal_promotion").click(function (e) {
        ...
        var new_member = $('#csh_member_type').html();
        if (chk_percent_discount.length > 0 && chk_member_no.length == 0 && new_member == "") {
                ...
                success: function (data) {
                    ...
                    if (data == "Y") {
                        ...
                        if (promo_type == 'MCOUPON') {
                            ...
                            return false;
                        } else if (promo_type == 'NEWBIRTH' && promo_code == 'BD001') {
                            lastdelpro();
                            setTimeout(function () {
                                var promo_code = $("#csh_promo_code").val();
                                var net_amt = $('#csh_net').val().replace(/[^\d\.\-\ ]/g, '');
                        ...
                        } else if (promo_type == 'NEWBIRTH' && promo_code == 'SS12020918') {
                            lastdelpro();
                            setTimeout(function () {
                                var promo_code = $("#csh_promo_code").val();
                                var net_amt = $('#csh_net').val().replace(/[^\d\.\-\ ]/g, '');
    ...
    var gHeight = 320,
        wcol = 250,
        gWidth = parseInt((screen.width * 50) / 100) - 530;
                ...
                var opts_ontrans = {
                    ...
                    success: function (data) {
                        ...
                        if (mdata[0] == 'Y') {
                            ...
                            if ($("#csh_member_chanel").val() == '') {
                                $("#csh_member_chanel").val(member_ch);
                            }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $('.tableNavOtherPro ul li').not('.nokeyboard').navigate({
                            ...
                            if (sel_promo.promo_code == 'FREEMARKOF') {
                                $("#dlgOtherPromotion").dialog("close");
                                if (sel_promo.promo_code == 'FREEMARKOF') {
                                    var onsales = onSales();
                                    if (onsales > 0) {
                            ...
                            // check onSales for dont play another promotion
                            if (sel_promo.promo_code == "2206W301") {
                                var onsales = onSales();
                                if (onsales > 0) {
                                    jAlert('You pending another transaction.  unable to continue', 'Warning message', function () {
                                        $("#csh_product_id").focus();
                                        return false;
                                    });
                                    $("#dlgOtherPromotion").dialog("close");

	$(function(){	
				...
				$('#btnBwsIdCard').click(function(e){
					...
					if(chk_member_no.length>0){
						jAlert('Found a pending transaction. Please cancel the document and make a new transaction.', 'Warning message',function(){
							$('#btnBwsIdCard').focus();
							return false;
						});	
						return false;
		...
		$("<div id='dlgFormTour'></div>").dialog({
    							...
    							$('#tour_barcode').keypress(function(evt){
    						        ...
    						        if(key == 13){
    						            ...
    						            if(tour_barcode.length<1){
    						            	jAlert('Please specify tour barcode.','Warning message',function(){
    				    						$('#popup_ok').focus();
    				    						setTimeout(function(){				    							
    						            ...
    						            //alert("chk_tour_digit=>" + chk_tour_digit);    						            
    						           	if(chk_tour_digit!=='PRCTG'){
    						           		...
    						           		jAlert('Barcode coupon code is invalid.','Warning message',function(){
    				    						$('#popup_ok').focus();
    				    						setTimeout(function(){				    							
    				    							if($('#popup_ok').not(":focus")){
    						    						$('#popup_ok').focus();
    							...
    							$('#tour_passport_no').keypress(function(evt){
    						        ...
    						        if(key == 13){
    						            ...
    						            if(tour_passport_no.length<1){
    						            	jAlert('Please specify Passport number.','Warning message',function(){
    				    						$('#popup_ok').focus();
    				    						setTimeout(function(){				    							
    				    							if($('#popup_ok').not(":focus")){
    						    						$('#popup_ok').focus();
		    	                ...
		    	                click: function(evt){ 
						        	...
						        	//*WR modify 22052017
						           	...
						           	if(tour_country.length<1){							 			
							 			jAlert('Please specify country.','Warning message',function(){		
							 				$('#popup_ok').focus();
				    						setTimeout(function(){				    							
				    							if($('#popup_ok').not(":focus")){
						    						$('#popup_ok').focus();
				...
				,beforeClose:function(evt){
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){	
						//evt.stopPropagation();
						evt.preventDefault();
						jConfirm('Do you want to cancel promotions?','Warning message', function(r){
					        if(r){
					        	initFormCashier();
								initTblTemp();
								getCshTemp('Y');
	...
	function callBackCardLost(data){
		...
		if(arr_search_result[0]==arr_callbackdata[0]){
			...
			jAlert("CRM DATA :: " + str_search_result + "\nIDCARD READER :: " + data+"\nข้อมูลการแจ้งบัตรเสีย/บัตรหาย ไม่ตรงกับขอมูลจากบัตรประชาชน\nกรุณาตรวจสอบอีกครั้ง","Warning message",function(){				
				$('#csh_id_card').val('');
				initTblTemp();
				initFormCashier();
	...
	function mstMcpPromotion(promo_code,chk_status,line_msg_error,idcard,coupon_code){
	   ...
	   //alert(promo_code +","+chk_status+","+line_msg_error +","+ idcard +","+ coupon_code);
		if(chk_status=='Y'){
			...
			//*WR08092015
    		var chk_tbl_temp="";
			$('<div id="dlgPdtOtherPro"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span><input type="text" size="15" id="pdt_otherpro"/></div>').dialog({
		           autoOpen:true,
				   width:'400',
				   height:'200',	
				   modal:true,
		           ...
		           open:function(){
				    	            ...
				    	            { 
				    	                ...
				    	                click: function(evt){ 
								    		...
								    		if(product_id.length<1){
							    				jAlert('กรุณาสแกนรหัสสินค้า', 'Warning message',function(){
				  							 ...
				  							 $.ajax({
				  								 ...
				  								 data:{
				  									...
				  									if(arr_data[0]=='1'){	
								    					...
								    					//*WR04092045 Mcoupon
								    					jAlert(arr_data[1],'Warning message',function(){
								    						$('#popup_ok').focus();
								    						setTimeout(function(){
						...
						if(buy_type=='N' ){
							...
							var start_baht=start_baht.replace(/[^\d\.\-\ ]/g,'');
							 ...
							 if(net_amt<start_baht){
								 jAlert("ยอดเงินสุทธิอย่างน้อยต้อง " + start_baht + " บาท", 'Warning message',function(){																						
									 $('#pdt_otherpro').val('').focus();
										return false;
								});
								 return false;																		    										 
					...
					,close:function(evt){
						...
						if(onSales()==0){
							jAlert('No sale transaction can not be found.', 'Warning message',function(){
								initFormCashier();
								initFieldOpenBill();
								$("#csh_member_no").focus();
		    					return false;
			...
			jAlert(line_msg_error, 'Warning message',function(){
				initFormCashier();
				initFieldOpenBill();
				$("#csh_member_no").focus();
				return false;
	...
	function lineApp3(application_id,a,b,c,d,e,idcard,coupon_code,friend_id_card,friend_mobile_no,promo_code,reg_mobile_no){
	 ...
	 function lineApp(promo_code,chk_status,line_msg_error,idcard,coupon_code){
		   ...
		   //alert(promo_code +","+chk_status+","+line_msg_error +","+ idcard +","+ coupon_code);
			if(chk_status=='Y'){
				...
				//*WR04072016 OI02330516 04/07/2016 - 30/09/2016 ลด 35% 2000
				//*WR11032016 OI02220216 14/03/2016 - 31/05/2016 ลด 35% 2000
				...
				$('<div id="dlgPdtLineApp"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span>' + 
	 ...
	 function callBackToDo2(promo_code,chk_status,line_msg_error,idcard,coupon_code,s_data){
		 ...
		 */		
		if(chk_status=='Y'){
			//*WR22052017 for passport,country code
			var s_country_code='';			
			if(s_data!=''){
			...
			//*WR11102016 ON03250916 50% 1 set
			//*WR20092016 OI06230816 30% เพื่อเซื้อสินค้าได้ไม่เกิน 1 ชิ้นเท่านั้น 1-15/10/2016
			//*WR20092016 OI06240816 30% เพื่อเซื้อสินค้าได้ไม่เกิน 1 ชิ้นเท่านั้น
			//*WR20092016 OK03120816 ลด 200 ได้ 1 ชุด  16/10-15/112016
			//*WR20092016 OK03110816 ราคา 355 บาท 1-31/10/2016
			//*WR20092016 OK03100816 ลด 200 ได้ 1 ชิ้น  1-15/102016
			//*WR20092016 OK03160816 ลด 100 ได้ 1 ชิ้น  1-31/102016
			//*WR20092016 OC04250816 ซื้อ 1 แถม 1 1/10-30/112016						
			//*WR 06092016 OI06220816 50% เพื่อเซื้อสินค้าได้ไม่เกิน 1 ชิ้นเท่านั้น
			//*WR08082016 OI03170716 ลด 100 ได้ 1 ชิ้น  1-15/092016
			//*WR08082016 OK03160716 ราคา 455 บาท 1-30/09/2016
			//*WR08082016 OK03150716 ลด 200 ได้ 1 ชิ้น  1-30/092016
			//*WR08082016 OK04140716 ซื้อ 1 แถม 1 1-30/092016
			//*WR02082016 OI03100716 Line Coupon Journey for the senses 70ml  ราคา 415 บาท 16-31/08/2016
			//*WR 23052016 OM13160416 mini buatibook 2016 
			//OS06150616 Gift Voicher 20% 
			//OK04390416
			//OI07200216 Line 2016 ซื้อสินค้า 2 ชิ้นแถมฟรี 2 ชิ้น ยอดซื้อสุทธิ 300 บาทขึ้นไป
			
			if(promo_code=='OX02460217_2'){
				initFieldPromt();
			}else{
	 ...
	 function setPdt2Temp(promo_code){
			 ...
			 * @modify 20102014
			 */
			$('<div id="dlgPdtMaster"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span><input type="text" size="15" id="pdt_master_id"/></div>').dialog({
			           autoOpen:true,
			           width:'40%',
					   height:'200',	
					   modal:true,
					    	            ...
					    	            { 
					    	                ...
					    	                click: function(evt){ 
					    	                	evt.preventDefault();
										 		...
										 		product_id=$.trim(product_id);																		 			
									    		if(product_id.length<1){
								    				jAlert('Scan the main item at the promotion.', 'Warning message',function(){
					  							 ...
					  							 $.ajax({
					  								 ...
					  								 data:{
					  									...
					  									if(arr_data[0]=='1'){	
					  										...
					  										getCshTemp('P');
					  										////////////////////// show dialog product detail สินค้าตัวแถม /////////////////////
					  										var dlg_titledetail_show=$('#other_promotion_title').html();
					  										if($.trim(dlg_titledetail_show)==''){
					  											dlg_titledetail_show="สแกนรายการสินค้าฟรีที่ร่วมโปรโมชั่น";
					  										}
					  										$('<div id="dlgPdtDetail"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span><input type="text" size="15" id="pdt_detail_id"/></div>').dialog({
							  								           autoOpen:true,
							  								           width:'50%',
							  										   height:'200',	
							  										   modal:true,
							  										    	            ...
							  										    	            { 
							  										    	                ...
							  										    	                click: function(evt){ 
							  										    	                	evt.preventDefault();
							  															 		...
							  															 		product_id=$.trim(product_id);																		 			
							  														    		if(product_id.length<1){
							  													    				jAlert('กรุณาสแกนรหัสสินค้าฟรีที่ร่วมรายการโปรโมชั่น', 'Warning message',function(){
							  											...
							  											if(promo_code=='OK01160115'){
							  												//if(buy_type=='N' ){
							  													...
							  													var start_baht=start_baht.replace(/[^\d\.\-\ ]/g,'');
							  													 ...
							  													 if(net_amt>end_baht){
							  														 jAlert("ยอดเงินสุทธิต้องไม่เกิน " + end_baht + " บาท กรุณาทำรายการใหม่", 'Warning message',function(){																						
							  															$('#pdt_detail_id').val('').focus();
							  															return false;
							  														});
							  														 return false;																		    										 
...
//							  												},400);
	...
	function changeCardTuesDay(member_id_old){
		...
		var dlgSetBag = $('<div id="dlgChangeCardTuesDay">\
	            ...
	            dlgSetBag.dialog({
				   ...
				   modal:true,
				   resizable:false,
		           title: "กรุณาระบุรหัสบัตรใหม่(อังคาร) ",		
		           position:"center",
		            open:function(){		
		            	$(this).dialog('widget')
			            .find('.ui-dialog-titlebar')
					        ...
					        if(key == 13){
					            ...
					            $.ajax({
					            	...
					            	data:{
					            		...
					            		if(data!='Y'){
					            			jAlert(data, 'Warning message',function(){
					            				$('#new_card_tues').focus();
	           									return false;
	           								});
					            		}else{					            		
	...
	function chkOldCardToChange(){
		...
		var dlgSetBag = $('<div id="dlgChkOldCardToChange">\
	            ...
	            dlgSetBag.dialog({
				   ...
				   modal:true,
				   resizable:false,
		           title: "กรุณาระบุรหัสบัตรเก่า(พฤหัสบดี) ",	
		           position:"center",
		            open:function(){		
		            	$(this).dialog('widget')
			            .find('.ui-dialog-titlebar')
					        ...
					        if(key == 13){
					            ...
					            //*WR 12052015
					           	...
					           	if(chk_member_th_old!='1120'){
					           		jAlert("กรุณาระบุรหัสบัตรเก่าเป็นบัตรวันพฤหัสบดีเท่านั้น", "Warning message",function(){       									
       									$('#old_card_thu').focus();
       									return false;
       								});
					           		return false;
					           	...
					           	var objReqChkCardToChange=$.getJSON(
					           						...
					           						function(data){							
					           							...
					           							if(data=='2'){
					           								jAlert('ไม่พบรหัสสมาชิกนี้กรุณาตรวจสอบอีกครั้ง', 'Warning message',function(){
					           									initFieldOpenBill();
					           									return false;
					           								});
					           							}else{
					           								$.each(data.member, function(i,m){
					           									if(m.exist=='yes'){					           										
					           										if(m.age_card!='4'){
					           											jAlert('ต้องเป็นสมาชิก 4 ปีเท่านั้น', 'Warning message',function(){
								           									$('#old_card_thu').focus();
								           									return false;
								           								});
					           											return false;
					           								...
					           								//------------------ zone alert --------------------
						           								...
						           								if(m.forgot_card=='Y'){
					           										jAlert('รหัสนี้ ถูกแจ้งลืม ในระบบแล้ว<br>สอบถามรหัสรับบัตรคืน(SMS) จากลูกค้า<br>หรือติดต่อ Admin', 'Warning message',function(){
					           											initFieldOpenBill();
					           											return false;
					           										});	
					           										return false;	
					           									...
					           									if(m.expire_status=='Y'){
					           										jAlert('รหัสสมาชิกนี้บัตรหมดอายุกรุณาตรวจสอบอีกครั้ง', 'Warning message',function(){
					           											initFieldOpenBill();
					           											return false;
					           										});	
					           										return false;				
	...
	function callMemberOffline(){
		...
		if(jQuery.trim(member_no)==''){	
			jAlert('Please specify member id.','Warning message',function(){
				initFieldOpenBill();
				return false;
			});	
		}else{	
...
//									}
									...
									$('#csh_vip_sum_amt').val(m.sum_amt);//ยอดเงินที่ใช้ได้ balance
									if(m.vip=='1'){
										...
										//check today balance
										var str_type="";
										(m.limited_type=='N')?str_type="ราคาสุทธิ":str_type="ราคาขาย";									
										//ยอดเงินคงเหลือ
										var vip_balance=parseFloat(m.sum_amt)-parseFloat(m.diary1_sum_amt);										
										//ยอดเงินใช้ไป
										var vip_used=parseFloat(m.limited)-vip_balance;
										//ยอดเงินที่ใช้ได้ balance
										$('#csh_vip_sum_amt').val(vip_balance);										
										var str_vip_info="วงเงิน "+addCommas(m.limited)+" บาท  "+str_type+"<br>ยอดเงินที่ใช้แล้ว "+addCommas(vip_used)+" บาท<br>คงเหลือ "+addCommas(vip_balance)+" บาท";
	...
	function chkCnMember(){
		...
		if(member_no!=cn_member_no){
			jAlert('รหัสสมาชิกไม่ตรงกับรหัสสมาชิกในเอกสารลดหนี้ (CN) <br>Please check and try again.','Warning message',function(b){
					 return false;
	   		 });
		}else{
			$('#other_promotion_title').append("  อ้างถึง "+cn_doc_no+"  ยอดเงิน "+cn_amount+" บาท");
			initFieldPromt();
		}
		...
		*/		
		...
		var opts_checkpro={
				...
				//////////////////////////// show
					...
					$("#other_promotion_cmd").html("ส่วนลด = "+discount+" บาทในชิ้นต่อไป");
...
//		return false;
			...
			success:function(data){
				...
				if(obj_json.status_ck=='Y'){
					...
					if(obj_json.month_enable=='Y' && obj_json.outofday=='Y' && obj_json.outtime=='Y'){
						jAlert('สามารถใช้สิทธิได้ระหว่างวันที่ '+obj_json.start_day+' ถึงวันที่ '+obj_json.end_day+'\n'
								+' เวลา '+obj_json.start_time+' ถึง '+obj_json.end_time+' น.'
								,'Warning message',function(b){
							initTblTemp();
					        initFormCashier();
							initFieldOpenBill();
		    				 return false;
						...
						jAlert('สามารถใช้สิทธิได้ระหว่างเวลา '+obj_json.start_time+' ถึง '+obj_json.end_time+' น.\n','Warning message',function(b){
							initTblTemp();
					        initFormCashier();
							initFieldOpenBill();
		    				 return false;
						...
						jAlert('เดือนนี้ไม่สามารถใช้สิทธิได้','Warning message',function(b){
							initTblTemp();
					        initFormCashier();
							initFieldOpenBill();
		    				 return false;
	...
	function chkEmpPrivilage(){
		...
		*/
		...
		if(employee_id.length==0){
			jAlert('กรุณาระบุรหัสพนักงาน','Warning message',function(b){
				$("#csh_member_no").focus();
				 return false;
    		 });		
	   		 return false;	
				...
				success:function(data){
					if(data!=''){
						...
						if(ecp.error_opsday=='Y'){
							jAlert('พนักงานไม่สามารถใช้สิทธิในวัน OPS Day ได้','Warning message',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.error_cam=='Y'){
							jAlert('กรุณา Scan บัตรพนักงาน หรือถ่ายรูปขอสิทธิก่อน','Warning message',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.error_empty=='Y'){
							jAlert('พนักงานท่านนี้ใช้สิทธิไม่ได้ กรุณาพยามอีกครั้งหรือติดต่อ HR','Warning message',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.is_used=='Y'){							
							jAlert('พนักงานท่านนี้ใช้สิทธิไปแล้ว','Warning message',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
							...
							var amount_balance=parseFloat(ecp.amount_balance);
							 ...
							 $('#other_promotion_title').html(" "+ecp.name+" "+ecp.surname+
										"  วงเงิน "+addCommas(amount_op)+
										"  ใช้ไป "+addCommas(amount_used)+
										" บาท คงเหลือ "+addCommas(amount_balance)+" บาท");
								...
								var msg_emp_tp="";
								if(ecp.emp_tp!=='O'){
									msg_emp_tp="----------------------------------------------------------------------\n<span style='color:#FF0000'>สิทธิพนักงานใช้ได้ทุกสาขา...ใช้ได้เพียงครั้งเดียวเท่านั้น</span>";
								}
								jAlert('ผู้มีส่วนร่วมในการใช้สิทธิ์พนักงานของบุคคลอื่น \nจะถูกตัดสิทธิ์สวัสดิการทุกประเภท<br>' +
										msg_emp_tp,'Warning message',function(b){
									initFieldPromt();
				    				 return false;
					    		 });	
						}
					}else{
						jAlert('ไม่สามารถใช้สิทธิได้ Please check and try again.','Warning message',function(b){
							$("#csh_member_no").focus();
		    				 return false;
			    		 });
					}
	...
	function selCN(){
		...
		*/
		var dialogOpts_dlgSelCN = {
				...
				showOpt: {direction: 'up'},		
				closeOnEscape: true,	
				title:"<span class='ui-icon ui-icon-cart'></span>บิลลดหนี้",
				open: function(){  
		 ...
		 jConfirm('DO YOU WANT TO DELETE ITEM NO :\r\n '+del_list_show,'Confirm message', function(r){
		        if(r){
					var opts={
							   ...
							   success: function(data){
								    if(data=='LOCKDEL'){
								    	 jAlert('ไม่สามารถลบรายการขณะเล่นโปรนี้ได้ ท่านสามารถเลือกยกเลิกรายการขายแทน','Warning message',function(b){
								    		 initFieldPromt();
						    				 return false;
							    		 });
		...
		if(flgstop=='Y'){
			...
			if($('#status_couponpromo').val()=='Y'){
				...
				if(csh_net > 0 && buy_status=='G' && csh_net < start_baht ){
					jAlert('ยอดซื้อน้อยกว่า ' + start_baht + ' ไม่สามารถชำระเงินได้ Please check and try again.','Warning message',function(){
	...
	function calPaid(){
		...
		*@last modify 31052017
		*@return void
		*/
		
		//$ ตัวเลขแสดงยอดเงินที่ต้องจ่ายสุทธิด้านบนฟอร์ม
	...
	function rePaymentBill(){
		...
		if($("#csh_status_no").val()=='01'){
			...
			//บัตรเสียบัตรหาย,สมัครสมาชิกใหม่
			initFormCashier();
			initTblTemp();
			getCshTemp('Y');
	...
	function clearFormPayment(){
		 ...
		 * @param
		 */		
		$('#txt_cash').val('');//จำนวนเงินที่ลูกค้าชำระเงินสด
		$('#txt_cash_2').val('');
		
		$('#txt_credit').val('0.00');//จำนวนเงินที่ลูกค้าชำระบัตรเครดิต
		$("#txt_redeem_point_cash").val('0.00');//แลกคะแนน ตอนนี้ยังอิง csh_point_net
		$('#txt_voucher').val('0.00');//รหัส coupon or voucher
		$('#txt_voucher_cash').val('0.00');//จำนวนเงิน		
		$('#txt_cash_back').val('0.00');//จำนวนเงินทอน	
	}//func
	    				           ...
	    				           position:"center",
	    			    			    	...
	    			    			    	$(this).keypress(function(evt){
	    								        ...
	    								        if(key == 13){
	    								            ...
	    								            if(this_bag_barcode.substr(0,2)!='14'){
						            					jAlert('Invalid Barcode','Warning message',function(b){	
	    				            ...
	    				            buttons: {
									 	"OK":function(evt){
										 	...
										 	if(chk_keyin == 0){
										 			jAlert('กรุณาคีย์บาร์โคดให้ครบ','Warning message',function(b){			
	...
	function saveTransaction(){
		...
		var member_no=$('#csh_member_no').val();//รหัสสมาชิก
		     member_no=$.trim(member_no);								
		var arr_percent_discount=$('#csh_percent_discount').html().split('+');
		var member_percent=arr_percent_discount[0];//ส่วนลดปกติสมาชิก%	
		var special_percent=arr_percent_discount[1];//ส่วนลดพิเศษสมาชิก%									
		var refer_member_id=$('#csh_refer_member_id').val();//อ้างถึงรหัสสมาชิก?																
		if($.trim(status_no)=='01'){
			//สมัครสมาชิก ต่อบัตร บัตรเสีย/บัตรหาย
			...
			if(member_no.length==0 && $('#csh_application_type').val()=="NEW" && $('#csh_card_type').val()=="MBC"){
				jAlert('ไม่พบรหัสบัตรใหม่ กรุณายกเลิกเอกสารแล้วทำรายการใหม่','Warning message',function(){			
					return false;
				});		
				return false;
			}else if(refer_member_id.length==0 && $('#csh_application_type').val()=="ALL"){
				jAlert('ไม่พบรหัสบัตรเก่าที่ใช้อ้างอิง กรุณายกเลิกเอกสารแล้วทำรายการใหม่','Warning message',function(){			
					return false;
				});	
				return false;
			}//end if			
		...
		/*var paid=$('#sel_credit:selected').val();//ประเภทการชำระเงินใช้กรณีชำระผ่านบัตรเครดิตอย่างเดียว*/
		
		var pay_cash=$('#txt_cash').val();//จำนวนเงินที่ลูกค้าชำระเงินสด $
		var pay_cash2=$('#txt_cash_2').val();//จำนวนเงินที่ลูกค้าชำระเงินสด R
		
		
		var pay_credit=$('#txt_credit').val();//จำนวนเงินที่ลูกค้าชำระบัตรเครดิต
		
		//alert( "pay_credit=>" + pay_credit);
		
		var credit_no=$('#txt_credit_no').val();//หมายเลขบัตรเครดิต																
		...
		var coupon_code=$("#csh_coupon_code").val();//*WR05082014 รหัส coupon promotion only							
		if(coupon_code!=''){
			if($.trim(status_no)=='01'){
...
//		}
		...
		if($('#payment_channel').val()=="Alipay"){
			var credit_no=$('#payment_transid').val();//หมายเลขบัตรเครดิต		
			var credit_tp=$('#payment_channel').val();//ประเภทบัตรเครดิต
			var bank_tp=$('#payment_channel').val();//บัตรธนาคาร
		}else{
			var credit_no=$('#txt_credit_no').val();//หมายเลขบัตรเครดิต		
			var credit_tp=jQuery.trim($("#sel_credit option:selected").text());//ประเภทบัตรเครดิต
			var bank_tp=jQuery.trim($("#sel_credit option:selected").text());//บัตรธนาคาร
		...
		var change=$('#txt_cash_back').val().replace(/[^\d\.\-\ ]/g,'');//จำนวนเงินทอน  $
		var change2=$('#txt_cash_back_2').val().replace(/[^\d\.\-\ ]/g,'');//จำนวนเงินทอน R
		...
		var csh_get_point=$("#csh_get_point").val();//สถานะได้รับคะแนนหรือไม่ N,Y
		var csh_point_receive=$("#csh_point_receive").val();   //คะแนนที่ได้รับ
		var csh_point_used=$("#csh_point_used").val();	     //คะแนนที่ใช้ 
		var csh_point_net=$("#csh_point_net").val();      //คะแนนสุทธิ
					...
					if(arr_data[0]=='1'){
						...
						if(getOnlineStatus()===1){
							...
							if(application_id=='OPMGMC300' || application_id=='OPMGMI300'){
								...
								$('#status_couponpromo').val('Y');
								promo_code='OX02041215';//amp ใช้รหัสโปรเดียว
							}
			    		...
			    		$("<div id='dlgCloseBill'></div>").dialog({
			    		        ...
			    		        width: 430,
			    		        height:255,
			    		        title:"<span class='ui-icon ui-icon-cart'></span>ผลการบันทึกรายการ",
			    		        position: 'middle',
			    		        resizable:false,
			    		        closeOnEscape:true,	
			    		        open:function(){
			    		        	...
			    		        	$(this).html('<p align="center" style="margin-top:35px;">บันทึกข้อมูล '+arr_data[1]+' สมบูรณ์','ผลการบันทึก</p>');
			    		        },close:function(){
						...
						jAlert('ระบบเกิดปัญหาไม่สามารถบันทึกได้ \n'+arr_data[1] + '\n<u>กรุณา Click ปุ่ม Refresh แล้วทำรายการขายใหม่</u>','ผลการบันทึก',function(){
							$("#dlgSaleMan").dialog("close");
	    					return false;
			    		 });
					}
						        ...
						        if(key == 13) {
						            ...
						            var opts={
							            	...
							            	success:function(data){
								            	var arr_data=data.split('#');
												if($.trim(arr_data[0])==""){
													jAlert('ไม่พบรหัสผู้ขาย Please check and try again.','Warning message',function(){
														$("#chk_saleman_id").val('').focus();
														return false;
									    			});
												}else if($.trim(arr_data[3])=='P'){
													jAlert('ขณะนี้พนักงานขายไม่อยู่ในระบบ Please check and try again.','Warning message',function(){
														$("#chk_saleman_id").val('').focus();
														return false;
									    			});
												}else if($.trim(arr_data[3])=='N'){
													jAlert('ขณะนี้พนักงานขายไม่ได้ลงเวลาเข้าระบบ Please check and try again.','Warning message',function(){
														$("#chk_saleman_id").val('').focus();
														return false;
									    			});
												}else{
	...
	function conFirmMemberID(){
		...
		$("<div id='dlgConFirmMemberID'></div>").dialog({
	       	   autoOpen:true,
	  				...
	  				showOpt: {direction: 'up'},		
	  				closeOnEscape: false,
	  				title:"<span class='ui-icon ui-icon-person'></span>กรุณายืนยันรหัสสมาชิก",
	  				open: function(){ 
						$(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"background-color":"#BCDCD7",
																										"font-size":"24px","color":"#000000",
																										"padding":"5 0.1em 0 0.1em"});   //ui-corner-all
    						                ...
    						                success:function(data){
    						                        ...
    						                        if(arr_data[0]=='Y'){
    						                        	...
    						                        	if(cmp_member_no==confirm_member_no){
        		    						            	...
        		    						            	jAlert("รหัสสมาชิกที่ยืนยันไม่ตรงกัน Please check and try again.", 'Warning message',function(){
        							            				$('#confirm_member_no').focus();
        			           									return false;
        			           								});
        		    						            	return false;
    						                        	...
    						                        	jAlert("Code หมดเวลากรุณาขอ Code อีกครั้ง","Warning message",function(){	                                       
    				                                        $("#confirm_member_no").val('');
    				                                        $("#confirm_member_no").focus();
    				                                        return false;
    				                                    });
...
//    						            }
	...
	function callEDC(credit_net_amt){
		...
		if(parseFloat(credit_net_amt)<1){
			jAlert('กรุณาระบุจำนวนเงินชำระบัตรเครดิต','Warning message',function(){
				$("#txt_credit_value").focus();
				return false;
			});
		}else if(parseFloat(credit_net_amt)>curr_total_payment){
			jAlert('จำนวนเงินชำระบัตรเครดิตมากกว่ายอดสุทธิที่ต้องชำระ','Warning message',function(){
				$("#txt_credit_value").focus();
				return false;
			});
		}else{
			var opts_edc={
					...
					data:{
						...
						//////////////////////\\\\\alert\\\\\/////////////////////////
						jConfirm('ตรวจสอบยอดชำระเงินบัตรเครดิต','Warning message', function(r){
					        if(r){
			...
			jAlert('กรุณารูดหรือเสียบบัตรเครดิต','Warning message',function(r){
				$('#popup_ok').focus();
				if(r){
					$.ajax(opts_edc);
				}			
											  ...
											  return true;
											    ...
											    if (key == 13) {
		        									...
		        									if(credit_no.length<4 || credit_no.length>16){
		        										jAlert('กรุณาระบุเลขที่บัตรเครดิตให้ถูกต้อง','Warning message',function(r){
		        											if(r){
		        												$("#txt_credit_no_value").focus();
		        												return false;
		        											}	
...
//        							});//keypress
				 	...
				 	"ตกลง":function(evt){
				 		...
				 		if(status_no=='01' && application_id=='OPKTC300'){				 			
				 			...
				 			if(jQuery.inArray(chk_ktc300, arr_ktc300) != -1) {
				 				...
				 				jAlert('กรุณาระบุเลขที่บัตรเครดิต KTC ด้านหน้า 6 หลัก\nเฉพาะชุดสมัคร OPKTC300 เท่านั้น','Warning message',function(r){
									if(r){
										$("#txt_credit_no_value").focus();
										return false;
									}	
				 			...
				 			if(credit_no_value.length<6 ){
					 			jAlert('กรุณาระบุเลขที่บัตรเครดิตด้านหน้า 6 หลักเฉพาะชุดสมัคร OPKTC300','Warning message',function(r){
									if(r){
										$("#txt_credit_no_value").focus();
										return false;
									}	
		...
		var dlgAlertReturnIdCard = $('<div id="dlgAlertReturnIdCard"></div>');
	            dlgAlertReturnIdCard.dialog({
			           ...
			           open:function(){				
				            ...
				            .addClass('ui-corner-top');
		    			$(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"padding":"10px 5px","background-color":"#ebebeb",
		    			    																				"margin":"0 0 0 0","font-size":"28px","color":"#0000A0"});/*#606060#A52A2A#C7D9DC*/
		    			$(this).dialog("widget").find(".ui-button-text")
		                    .css({"padding":".1em .2em .1em .2em","font-size":"25px"});
		    			$(this).parent().children().children('.ui-dialog-titlebar-close').hide();
		    			    
		            	$('#dlgAlertReturnIdCard').html('<p align="center" style="margin-top:35px;"><span id="msgReturnIdCard" style="font-size:35px;color:#FF0000;"><u>อย่าลืม!!!</u></span>   คืนบัตรประชาชนให้ลูกค้าด้วยค่ะ</p>');	
			           ...
			           open:function(){				
		            	...
		            	return false;
					 	...
					 	"ตกลง":function(evt){
	            			...
	            			if( $chk_none_item=='Y'){
	            				jAlert('กรุณาระบุจำนวนถุงอย่างน้อย 1 รายการ (ถ้าไม่สะดวกให้คีย์ 0 ถุง) ','Warning message',function(){
									return false;
				    			});
	            				return false;
	            			}
	            			...
	            			if($chk_over_bag_qty=='Y'){
	            				jAlert('กรุณาระบุจำนวนไม่เกิน 99 ','Warning message',function(){
									return false;
				    			});
	            				return false;
	            			}
	...
	function chkBeforeConfirmPayment(){
		...
		if(chk_txt_cash>0 && chk_txt_credit>0 ){
        	jAlert('Please select only one payment method.Please check and try again.','Warning message',function(){
				$("#txt_cash").focus();
				return false;
			});
        ...
        if(chk_net_curr!=0){
        	$("#btn_payment_confirm").removeClass("ui-state-focus ui-state-hover ui-state-active");
        	jAlert('Incomplete net purchase amount. Please check and try again.','Warning message',function(){
				$("#txt_cash").focus();
			});
	    	return false;
	    }else if(chk_txt_cash>999999){
	    	$("#btn_payment_confirm").removeClass("ui-state-focus ui-state-hover ui-state-active");
        	jAlert('Overpayment! Please check and try again.','Warning message',function(){
				$("#txt_cash").focus();
			});
	    	return false;
	    }else{
	    	var csh_bill_manual_no=$('#csh_bill_manual_no').val();
	 		...
	 		if(csh_bill_manual_no.length>0){
	 			jConfirm('คุณต้องการบันทึกการเปิดบิลมือ\nเลขที่บิล : ' + csh_bill_manual_no +
	 					'\nเลขที่ TICKET : ' + csh_ticket_no +
	 					'\nใช่หรือไม่','Confirm message', function(r){
	 		        if(r){
	 		        	setBag2Temp();
	 		        }else{
	 		        	return false;
						...
						function(){
						    ...
						    $("#btn_payment_confirm").click( function(e){
								...
								$.ajax({
									...
									data:{
										...
										if(data=='Y'){												
											if($('#payment_channel').val()!="Alipay"){
												jAlert('โปรโมชั่นนี้(OX07140317)ต้องชำระเงินผ่าน Alipay เท่านั้น','Warning message',function(){														
													return false;
								    			});
												return false;
											}else{
							...
							$("#btn_payment_alipay").click( function(evt){
								...
								if(member_type!=='WALK IN'){
									jAlert('การชำระเงินผ่าน Alipay ใช้สำหรับลูกค้า WALK IN เท่านั้น', 'Warning message',function(){									    	
								    	return false;
								    });	
								    return false;
								}else{
							...
							$("#btn_key_manual").click(function(evt){
								...
								$("<div id='dlgFrmKeyManual'></div>").dialog({
							       	   autoOpen:true,
							  				...
							  				showOpt: {direction: 'up'},		
							  				closeOnEscape: false,
							  				title:"<span class='ui-icon ui-icon-person'></span>กรุณาระบุข้อมูลเอกสารบิลมือ",
							  				open: function(){ 
												$(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"background-color":"#BCDCD7",
																																"font-size":"24px","color":"#000000",
							  				...
							  				buttons: [ 			
									    	                	...
									    	                	evt.preventDefault();
														 		...
														 		$.ajax({
														 			...
														 			data:{
														 				...
														 				if(data=='Y'){
														 					jAlert('เลขที่การเปิดบิลมือซ้ำไม่สามารถบันทึกได้ Please check and try again.', 'Warning message',function(){
								    	                ...
								    	                { 
									    	                ...
									    	                click: function(evt){ 
									    	                	evt.preventDefault();
														 		evt.stopPropagation();	
														 		jConfirm('คุณต้องการยกเลิกรายการบิลมือ ใช่หรือไม่?','Confirm message', function(r){
															        if(r){
															        	$('.frmKeymanualFixedTextbox').val('');
																 		$('.frmKeymanualFixedTextboxTicket').val('');
																 		$('#csh_bill_manual_no').val('');
	...
	function cancelBill(){
		...
		*/
		var opts={
					...
					success:function(data){
						if(data=='0'){
							jAlert('ไม่สามารถยกเลิกรายการได้ Please check and try again.', 'Warning message',function(){
								return false;
							});	
						}else{						
	...
	function getProduct(){
		...
		*/	
		...
		if(product_id.length==0){
			jAlert('กรุณาระบุ รหัสสินค้า', 'Warning message',function(){
				return false;
			});	
		...
		var opts={
				...
				success:function(data){		
					if(data==1){
						jAlert('This product code is not available. Please check and try again.', 'Warning message',function(){
						...
						if(pdt_lost_id=='23983'){
							//*WR15092015 for support card lost stock out
							$("#csh_product_id").val('');
							cardLostStock();
							//*WR15092015 for support card lost sock out
						}else{
							
							jAlert('รหัสสินค้านี้สต๊อกไม่พอ Please check and try again.', 'Warning message',function(){			
								if($('#csh_gap_promotion').val()=='Y'){								
									//*WR22072014
									if($('#csh_promo_code').val()=='50BTO1P'){
										endOfProOther();//uncomment on 05012013
						...
						jAlert('รหัสสินค้านี้ถูกล๊อก Please check and try again.', 'Warning message',function(){	
							if($('#csh_gap_promotion').val()=='Y'){
								//endOfProOther();								
							}
							initFieldPromt();		
						...
						jAlert('สินค้า Tester ห้ามขาย Please check and try again.', 'Warning message',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
					}else if(data==5){
						jAlert('สินค้า ไม่ร่วมโปรโมชั่น Please check and try again.', 'Warning message',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
					}else if(data==6){
						jAlert('ไม่สามารถซื้อสินค้าเกินกว่าจำนวนที่กำหนดได้  Please check and try again.', 'Warning message',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
						...
						if(member_no!='' && play_main_pro=='N' && play_last_pro=='N' && csh_gap_promotion!='Y' &&  $('#csh_trn_diary2_sl').val()=='Y'){							
									...
									var opts_amt={
													...
													data:{
														...
														if(arr_chk_data[0]=='N'){
															...
															jAlert(msg_error_amount+' Please check and try again.', 'Warning message',function(){						
																initFieldPromt();		
																return false;							
															});			
															return false;
							...
							if($('#csh_member_vip').val()=='1' || $("#csh_status_no").val()=='03' || $("#csh_status_no").val()=='05'){								
								...
								// other promotion เล่นโปรระหว่างโปร
								setGapValTemp();
							}else{								
								if($("#status_pro").val()=='0'){										
...
//								       return false;
									...
									if($('#csh_status_no').val()=='02' && parseFloat(csh_first_limited)>0){			
										...
										var opts_chk_first_lmt={
													...
													data:{
														...
														if(data=='Y'){
															...
															jAlert('ยอดซื้อต้องไม่เกิน ' + parseFloat(csh_first_limited).toFixed(2) + ' Please check and try again.', 'Warning message',function(){						
																initFieldPromt();		
																return false;							
															});	
														}
										...
										if(status_no==''){
											jAlert('Document status is invalid. Please cancel the sale and re-open the bill.', 'Warning message',function(){
												$("#csh_product_id").focus();
												return false;
											});	
											return false;
	...
	function initFieldOpenBill(){
		...
		*/
		...
		$('#csh_member_no').focus();
		$('#csh_application_type').val('');//NEW=สมัครใหม่, ALL=ต่ออายุ
		$('#csh_card_type').val('');//MBC=บัตรสมาชิก, IDC=บัตรประจำตัวประชาชน
		return false;
	}//func
		...
		*/
		...
		var opts={
				...
				success:function(data){
					var arr_data=data.split('#');	
					if(arr_data[0]=='0'){
						jAlert('ไม่สามารถทำรายการสินค้านี้ได้ Please check and try again.', 'Warning message',function(){
							initFieldPromt();
							return false;
						});	
					}else{		
	...
	function setSmsProduct(sms_promo_code){
		...
		var opts_smsproduct={
				...
				data:{
					...
					var arr_data=data.split('#');
					if(arr_data[0]=='0'){
						jAlert('ไม่สามารถบันทึกรายการได้ Please check and try again.', 'Warning message',function(){							
							return false;
						});	
					}else{
	...
	function callSmsPromo(sms_promo_code,sms_mobile,redeem_code,member_tp,idcard){
		...
		var opts_smspromo={
				...
				data:{
					...
					if(data=='null'){
						jAlert("ไม่พบข้อมูล Please check and try again.", 'Warning message',function(){
					...
					if(objJson.status=='OK'){
						...
						$('#id_redeem_code').val(redeem_code);
						 //*WR23012013						
						  if(prefix_member_no_idcard!='ID' && member_tp=='Y'  && objJson.member!=$('#csh_member_no').val()){
							  jAlert("รหัสบัตรสมาชิกไม่ตรง Please check and try again.", 'Warning message',function(){
			    				   $("#sms_mobile").focus();
									return false;
								});	
							  return false;
						...
						//*WR20092016 fix code for mcs promotion						  
						 ...
						 jAlert(objJson.status_msg, 'Warning message',function(){
							  //---------------- 25072013 --------------------------------
							 $('#status_smspromo').val('');
							 $('#id_smspromo').val('');
		...
		var opts_dlgFormMobile={
	    	  ...
	    	  open:function(){
  			    ...
  			    $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"padding":"10px","margin-top":"0",
  			    					"background-color":"#BCDCD7","font-size":"22px","color":"#000"});
  			    $(this).dialog("widget").find(".ui-dialog-buttonpane")
  			                      .css({"padding":".1em .1em .1em 0","margin":"0 0 0 0","background-color":"#C7D9DC"});
  			    //button style	
  			    $(this).dialog("widget").find(".ui-btndlgpos")
...
//	    		    			  alert("index of a is " + a + "\n and length of a is " + b);
	    		    			  if((arr_tp.indexOf("T") != -1) && (sms_mobile.length!=10)){
	    		    				  jAlert('กรุณาระบุเบอร์โทรศัพท์จำนวน 10 หลัก', 'Warning message',function(){
					    				   $("#sms_mobile").focus();
											return false;
										});	    				  
	    		    				  return false;
	...
	function playCoPromo(promo_code,product_id,member_no){
		...
		var opts_chkcopromo={
				...
				data:{
					...
					//return รหัสโปรโมชั่นที่ร่วมเล่น
					if($.trim(data)!=''){
						var promo_play=$.trim(data);//โปรโมชั่นที่ร่วมเล่น
						var member_no=$.trim($('#csh_member_no').val());
						/// play co promotion
						var dlgOpts_dlgCopromotion = {
								...
								width:'25%',
								height:250,
								title:"<span class='ui-icon ui-icon-cart'></span>โปรโมชั่น ชิ้นที่ 1 ชิ้นที่ 2 ลด 50%",
	        					closeOnEscape:true,
	        					open: function(){ 
	        						$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
	        						$(this).dialog('widget')
									    	   ...
									    	   data:{
									    		  ...
									    		  var arr_data=data.split('#');
									    		   if(arr_data[0]=='1'){
									    			   ...
									    			   jAlert('สินค้าไม่ร่วมโปรโมชั่น Please check and try again.', 'Warning message',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='1'){
									    			   jAlert('ไม่พบสินค้าในทะเบียน Please check and try again.', 'Warning message',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='2'){
									    			   jAlert('ไม่พบสินค้าในสต๊อก Please check and try again.', 'Warning message',function(){
									    				   $("#co_product_id").select().focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='3'){
									    			   jAlert('สินค้าอยู่ในสถานะ Lock Please check and try again.', 'Warning message',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='4'){
									    			   jAlert('ราคาสินค้าร่วมโปรโมชั่นมากว่าสินค้าปกติ Please check and try again.', 'Warning message',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }
		...
		*/
		...
		var opts={
				...
				success:function(data){					
					...
					var play_last_pro=$.trim($("#csh_play_last_promotion").val());
					var arr_data=data.split('#');	
					$("#csh_bal_discount").val(arr_data[3]);//*WR17102012 ส่วนลดคงค้างใช้กับโปร 50BTO1P
					if(arr_data[0]=='0'){
						jAlert('ไม่สามารถทำรายการสินค้านี้ได้ Please check and try again.', 'Warning message',function(){
							initFieldPromt();
							return false;
						...
						jAlert('ไม่สามารถบันทึกสินค้าซ้ำได้ หรือ กด ESC Key เพื่อออกจากโปรโมชั่นก่อนคีย์สินค้าทั่วไป', 'Warning message',function(){
							initFieldPromt();
							return false;
						});	
					}else{						
						if($('#csh_promo_tp').val()=='F' && $('#csh_limite_qty').val()=='1'){
							//ยกเลิกโปรทันทีกรณี 1 ชิ้นต่อ 1 บิล
							endOfProOther();
						}
						if(arr_data[2]=='trn_tdiary2_sl'){
							/*
							if($('#csh_promo_tp').val()=='F' && $('#csh_limite_qty').val()=='1'){
								$('#csh_promo_tp').val('N');//clear สถานะสินค้าเป็นตัวซื้อ
							}
							*/
							getCshTemp('N');
						}else{
							...
							///* case promotion 50BTO1P ตรวจสอบว่ามีสินค้าที่ร่วมโปรหรือไม่
	...
	function setCshValTemp(){
		...
		*/
		...
		if(quantity.length==0){
			jAlert('กรุณาระบุ จำนวนสินค้า', 'Warning message',function(){
				$("#csh_quantity").focus();
				return false;
			});	
			return false;
		...
		if(member_percent1 != '' && member_no==''){
			jAlert('ไม่ใช่สมาชิกไม่ได้รับส่วนลด Please cancel the sale and re-open the bill.', 'Warning message',function(){
				$("#csh_product_id").focus();
				return false;
			});	
			return false;
	...
	function setCshTemp(){
		...
		*/
		...
		var opts={
				...
				success:function(data){
					...
					if(arr_data[0]=='0'){
						...
						if($('#csh_promo_tp').val()=='F' && $('#csh_limite_qty').val()=='1'){//สินค้าฟรี 1ชิ้นต่อบิล
							$('#csh_promo_tp').val('N');//clear สถานะสินค้าเป็นตัวซื้อ
							$('#csh_promo_code').val('');					
						}
		...
		if($('#csh_member_vip').val()=='1'){
			...
			var sum_amt=$('#csh_vip_sum_amt').val();//ยอดเงินคงเหลือที่ใช้ได้
			...
			var opts_vip={
						...
						success:function(data){
							if(data=='Y'){
								...
								jAlert('ยอดซื้อมากกว่าวงเงินคงเหลือ '+addCommas(vip_balance)+' บาท','Warning message',function(){
									initFieldPromt();
									return false;
								});	
								return false;
	...
	function getSubTotal(flgSale){
		...
		*@problem : OPPD300 ไม่ได้รับคะแนน x3 เพราะยังไม่มีรหัสสมาชกใหม่นะ
		*/
		var cn_amount=$("#cn_amount").val();//สำหรับตัดคะแนนหลังหลักยอดเงิน CN
		var flg_chk_point='no';
	...
	function displayDigit(ndigit){
		...
		*@return void
		*/		
		if(typeof(ndigit)=='undefined' || ndigit=='' || ndigit=='0') ndigit="0.00";
		var arr_num=ndigit.split('');
		var str_num="";
		var str_imgname="";
		var w = screen.width;
	...
	function rePromoPointDiscount(){
		/**
		 *@desc สมาชิกแลกคะแนน กลับไปคำนวณ net_amt ให้กลับมาค่าเดิมก่อนหักส่วนลดหลังแลกคะแนน
		 *@return null
		 */
		//cal % discount									
		$.ajax({
	...
	function redeemPointOffline(){
		/**
		*@desc อนุญาติให้แลกคะแนนกรณี offline ได้ โดยใช้คะแนนที่เครื่องเท่านั้น
		*@return
		*/		
									    ...
									    var opts_dlgRedeemPointDetail={
												...
												open:function(){
													...
													$(this).dialog('widget')
											            ...
											            .addClass('ui-corner-top');
									    			$(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"padding":"0 0.05em 0 0.05em",
									    				"background-color":"#BCDCD7","font-size":"26px","color":"#000"});
													$(this).parent().find('select, input, textarea').blur();
													...
													//** bug กลับมาดูใหม่ปัญหาคือ ESCAP ไปแล้วพอ key new member กลับมาเล่นอีก? 
			...
			data:{
				...
				if($.trim(data)!=''){
					$('<div id="dlgSelCoPromotion"></div>').dialog({
						   ...
						   resizable:true,
						   closeOnEscape:false,		
				           title: "เลือกรายการโปรโมชั่นท้ายบิล",		
				           position:"center",
				           open:function(){
				        	   $(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
								$(this).dialog('widget')
							    ...
							    $('#dlgSelCoPromotion').empty().html(data);	
	    			    		 $('.tableNavSelCopromotion ul li').not('.nokeyboard').navigate({
								    ...
								    //*WR19122014 แก้ปัญหาเฉพาะของแถม OPPN300
								    if(promo_play=='OX07320814' || promo_play=='OX07561014'){
								    	var chk_oppn=$('#info_apply_promo').html();
								    	chk_oppn=$.trim(chk_oppn);
								    	if(chk_oppn != 'OPPN300'){
								    		jAlert('เฉพาะชุดสมัครใหม่ OPPN300 เท่านั้น','Warning message',function(){
												return false;
											});	
								    		return false;
								    	}
								    ...
								    /////////////////// KEY PRODUCT FREE ///////////////////////////
								    $('<div id="dlgCoProduct">' + 'รหัสสินค้า : <input type="text" size="15" id="co_product_id2"/></div>').dialog({
									           autoOpen:true,
											   width:'35%',
											   height:'auto',	
											   modal:true,
															    		...
															    		$.ajax({
															    			...
															    			data:{
															    				...
															    				if(arr_data[0]=='1'){															    					
															    					...
															    					msg_result="ไม่พบรายการสินค้าในสต๊อก";						
															    					jAlert(msg_result,'Warning message',function(){
																						$("#btn_cal_promotion").removeAttr("disabled");
																						$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
															    					...
															    					msg_result="รายการสินค้า สต๊อกไม่พอ";						
															    					jAlert(msg_result,'Warning message',function(){
																						$("#btn_cal_promotion").removeAttr("disabled");
																						$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
																						$('#co_product_id2').focus();
															    					...
															    					msg_result="รายการสินค้าห้ามสแกนซ้ำ";
															    					jAlert(msg_result,'Warning message',function(){
																						$("#btn_cal_promotion").removeAttr("disabled");
																						$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
																						$('#co_product_id2').focus();
															    					...
															    					msg_result="รายการสินค้านี้ไม่ร่วมโปรโมชั่น";
															    					jAlert(msg_result,'Warning message',function(){
																						$("#btn_cal_promotion").removeAttr("disabled");
																						$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
																						$('#co_product_id2').focus();
															    					...
															    					msg_result="รายการสินค้านี้ต้องไม่เกิน " + unitn +" ชิ้น";
															    					jAlert(msg_result,'Warning message',function(){
																						$("#btn_cal_promotion").removeAttr("disabled");
																						$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
																						$('#co_product_id2').focus();
	...
	function redeemPointSpecial(promo_code){
		 ...
		 * * 22072014 ใช้คะแนนแทนส่วนลดแล้วได้คะแนน
		 * @return
		 */
		$('<div id="dlgProRedeemSpecial"></div>').dialog({
	           ...
	           open:function(){
					...
					//button style	
	  			    $(this).dialog("widget").find(".ui-btndlgpos")
	                  .css({"padding":"2","background-color":"#C7D9DC","font-size":"16px"});
	  				 ...
	  				 data:{
	  					 ...
	  					 $('.pro_rp_sp').each(function(){
	  						 $(this).click(function(e){
	  							 ...
	  							 /////////////////////////////////////// FORM KEYPRODUCT \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
	  							 //สแกนรหัสสินค้าที่ร่วมรายการโปรโมชั่น 
			  							$('<div id="dlgPdtRedeemPoint"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span><input type="text" size="15" id="pdt_redeempoint"/></div>').dialog({
									           autoOpen:true,
											   width:'400',
											   height:'200',	
											   modal:true,
											    	            ...
											    	            { 
											    	                ...
											    	                click: function(evt){ 
											    	                	evt.preventDefault();
																 		...
																 		product_id=$.trim(product_id);																		 			
															    		if(product_id.length<1){
														    				jAlert('กรุณาสแกนรหัสสินค้าที่ร่วมรายการโปรโมชั่น', 'Warning message',function(){
		    	                ...
		    	                { 
			    	                ...
			    	                click: function(evt){ 
			    	                	evt.preventDefault();
								 		evt.stopPropagation();					 		
								 		$('#dlgProRedeemSpecial').dialog('close');
								 		//check ท้ายบิล next to tomorrow 500 net free coupon 900062								 		
								 		var net_amt=$('#csh_net').val().replace(/[^\d\.\-\ ]/g,'');
								 		net_amt=parseFloat(net_amt);
								 		var amount=$('#csh_sum_amount').val().replace(/[^\d\.\-\ ]/g,'');
								 		amount=parseFloat(amount);
								 		if(net_amt>=500){
								 			//last pro check free product method กลางๆ
...
//								    }else 
		...
		if(member_no.length==0){
							...
							$.each(data.member, function(i,m){
								if(m.exist=='yes'){
									...
									if(status_no=='04' && m.link_status=='OFFLINE'){										
										jAlert("ไม่พบการตอบสนองจาก Server ไม่สามารถแลกคะแนน(OFFLINE)ได้","Warning message",function(){
											initFieldOpenBill();
    	     				            	return false;
    	     					        });
    	     				    		return false;										
			...
			setTimeout(function(){
				if(opts_redeempoint){
					...
					jAlert('ไม่พบการตอบสนองจาก Server กรุณาลองใหม่อีกครั้ง','Warning message',function(){
						initFieldOpenBill();
						return false;
					});	
				}
		...
		var dialogOpts_NewCard={
				...
				open:function(){
	    			...
	    			$(this).parents(".ui-dialog:first")
	    				...
	    				.css({"padding":"20px","background-color":"#BCDCD7","font-size":"26px","color":"#000"});
						    ...
						    evt.preventDefault();		
						       ...
						       var $csh_newcard=$("#csh_new_card");
								...
								if($csh_newcard_val.length==0){
									jAlert('กรุณาระบุรหัสบัตรสมาชิก', 'Warning message',function(){
										$csh_newcard.focus();
										return false;
									});	
								}else{
									...
									var opts_member={
											...
											success:function(data){
												if(data=='1'){
													jAlert('รหัสบัตรนี้ถูกใช้สมัครสมาชิกแล้ว Please check and try again.', 'Warning message',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='2'){
													jAlert('ไม่พบรหัสบัตรนี้ หรือรหัสนี้ถูกใช้สมัครแล้ว Please check and try again.', 'Warning message',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='3'){
													jAlert('กรุณาเลือก ' + msg_show, 'Warning message',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='4'){
													jAlert('กรุณาเลือกบัตรวันพฤหัสบดี', 'Warning message',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='5'){
													jAlert('กรุณาสแกนบัตร ' + msg_show_sel, 'Warning message',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='6'){
													jAlert('กรุณาเปลี่ยนคีย์บอดเป็นภาษาอังกฤษ', 'Warning message',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else{
	...
	function giftsetProduct(application_id,employee_id,member_no,status_no,doc_tp){
		...
		*/		
		...
		var dialogOpts_giftsetproduct = {
				...
				showOpt: {direction:'up'},		
				closeOnEscape:true,	
				title:"<span class='ui-icon ui-icon-cart'></span>เลือกสินค้ามูลค่า "+parseFloat($gift_set_amount.val()).toFixed(2)+" บาท",
				open: function(){  
						$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
						$(this).dialog('widget')
				            .find('.ui-dialog-titlebar')
				            .removeClass('ui-corner-all')
				            .addClass('ui-corner-top');
					    $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"background-color":"#BCDCD7","color":"#000000"});	
										...
										if($input_giftset_quantity_val.length==0){
											...
											var opts={
													...
													success:function(data){	
														var arr_data=data.split('#');
														if(arr_data[0]=='3'){
															jAlert('สินค้านี้ถูก lock ไม่สารมารถทำรายการได้ Please check and try again.', 'Warning message',function(){
																$input_giftset_product.val('').focus();
																return false;
															});	
														}else if(arr_data[0]=='2'){
															jAlert('จำนวนสินค้าไม่พอที่จะทำรายการ Please check and try again.', 'Warning message',function(){
																$input_giftset_product.val('').focus();
																return false;
															});	
														}else if(arr_data[0]=='0'){
				...
				close: function(evt,ui) {
					...
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){
						if($csh_net_val<$gift_set_amount_val){
							jAlert("ยอดซื้อน้อยกว่า " + parseFloat($("#csh_gift_set_amount").val()).toFixed(2) + " Please check and try again.","Warning message",function(){
							    ...
							    $('#dlgSelLStPromotion').empty().html(data);	
	    			    		 $('.tableNavSelCopromotion ul li').not('.nokeyboard').navigate({
					    			...
					    			$('<div id="dlgLstProduct">' + 
					    					'<center><span style="color:#336666;font-size:20px">จำนวน&nbsp;:&nbsp;</span>' + 
					    					'<input type="text" size="1" value="1" id="lst_quantity_id" readonly style="width: 30px;align:center"/>' +
					    					'<span style="color:#336666;font-size:20px;">&nbsp;&nbsp;รหัสสินค้า : </span>' + 
					    					'<input type="text" size="10" id="lst_product_id" style="width: 180px;"/></center></div>').dialog({
									           ...
									           open:function(){
												...
												//button style	
								  			    $(this).dialog("widget").find(".ui-btndlgpos")
									                  .css({"padding":"2","background-color":"#C7D9DC","font-size":"18px"});
	...
	function mstPromotion(promo_code,seq_pro,s_play,title){
		...
		var btn_titles="ถัดไป >>";
		if(promo_code=='OX02460217'){
			btn_titles=">> เล่นโปร Value Set for Tourists/ชำระเงิน";
			is_readonly="";
		}
		...
		$('<div id="dlgMstPdtPromotion">' + 
	           ...
	           open:function(){
					    	...
					    	var key = evt.keyCode ? evt.keyCode : evt.which ? evt.which : evt.charCode; 
		   			        if(key == 13){	
					    		...
					    		if(keyin_mstquantity_id.length<1){
				    				jAlert('กรุณาระบุจำนวนสินค้า', 'Warning message',function(){
				...
				,close:function(evt){
					...
					//alert("s_play: " + s_play);//จะไม่เข้า case n ตรงนี้
					if(s_play=='N'){
						paymentBill('01');
					}else{
						seq_pro++;						
	...
	function freeProduct(application_id,employee_id,member_no,status_no,doc_tp){
		...
		*/
		...
		var dialogOpts_freeproduct = {
				...
				showOpt: {direction:'up'},		
				closeOnEscape:false,	
				title:"<span class='ui-icon ui-icon-cart'></span>สินค้าแถมฟรี",
				open: function(){  
						$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
						$(this).dialog('widget')
				            .find('.ui-dialog-titlebar')
														...
														if(application_id=='OPPC300'){				
																	...
																	data:{
																		...
																		rnd:Math.random()
																			...
																			if(price_check > parseFloat($free_product_amount.val())){
																				jAlert("ยอดซื้อมากกว่า " + parseFloat($free_product_amount.val()).toFixed(2) + " Please check and try again.","Warning message",function(){
																					window.parent.$(".ui-dialog").remove();
																...
																$.ajax({
																	...
																	data:{
																		...
																		if(data=='N'){
																			jAlert("กรุณาเลือกสินค้าในกลุ่ม Body/Hair ","Warning message",function(){
																				$("#input_free_product").val('').focus();
																				return false;
																			});	
																			return false;
																	...
																	$.ajax({
																			...
																			success:function(data){	
																				...
																				if(arr_data[0]=="E_QTYLIMIT"){
																		 		...
																		 		var amount=$('#csh_sum_amount').val().replace(/[^\d\.\-\ ]/g,'');
																		 		amount=parseFloat(amount);
																		 		var qty_chk=parseInt(arr_data[3]);//เล่นได้ไม่เกิน
				...
				close: function(evt,ui) {
					...
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){
						if($csh_net_val<parseFloat($free_product_amount.val())){
							jAlert("ยอดซื้อน้อยกว่า " + parseFloat($free_product_amount.val()).toFixed(2) + " Please check and try again.","Warning message",function(){
								window.parent.$(".ui-dialog").remove();
								$('#dlgFreeProduct').dialog('open');
								$("#input_free_product").focus();
								return false;
	...
	function freePremium(application_id,employee_id,member_no,status_no,doc_tp){
		/**
		*@desc : รอป้อนรหัสสินค้า สำหรับสินค้า premium
		*@return
		*/
		var $free_product=$("#csh_free_product");
		var $free_product_amount=$("#csh_free_product_amount");
		...
		var dialogOpts_freepremium = {
				...
				showOpt: {direction:'up'},		
				closeOnEscape:true,	
				title:"<span class='ui-icon ui-icon-cart'></span>สินค้าพิเศษ "+parseFloat($free_premium_amount.val()).toFixed(2)+" บาท",
				open: function(){  
					$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
					$(this).dialog('widget')
			            .find('.ui-dialog-titlebar')
								...
								var opts_freepremium={
											...
											success:function(data){
												...
												if(c_free_premium_amount>parseFloat($free_premium_amount.val())){
													c_free_premium_amount=parseFloat(c_free_premium_amount)-parseFloat(arr_fp[1]);
													jAlert('มูลค่าสินค้ามากกว่าจำนวนเงินสินค้าพิเศษ Please check and try again.', 'Warning message',function(){
												...
												if(arr_fp[0]=='4'){
													jAlert('สินค้ารหัสนี้ไม่ร่วมโปรโมชั่น Please check and try again.', 'Warning message',function(){
														$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
												if(arr_fp[0]=='1'){
													jAlert('ไม่พบข้อมูลรหัสสินค้านี้ในทะเบียน Please check and try again.', 'Warning message',function(){
								           				$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
												if(arr_fp[0]=='2'){
													jAlert('สินค้าสต๊อกไม่พอ Please check and try again.', 'Warning message',function(){
								           				$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
													if(arr_fp[0]=='3'){
														jAlert('สินค้าถูกล๊อก Please check and try again.', 'Warning message',function(){
									           				$input_free_premium.focus().select();
									        				return false;
									        			});	
												}else if(arr_fp[0]=='5'){
	...
	function getCatProduct(application_id,product_no,product_seq,product_sub_seq,pn,ps){
		 ...
		 *@param Integer maxno : max value of product_no ชุดของสินค้าสูงสุดกี่ชุด
		 *@param Integer i : number of round maxno
		 *@param Integer product_seq : ลำดับการเล่น product_no
		...
		var dialogOpts_MemberCatalogList={
				...
				showOpt: {direction: 'up'},		
				closeOnEscape:true,	
				title:"<span class='ui-icon ui-icon-cart'></span>เลือกรายการ",
				open:function(){
					$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
					$(this).dialog('widget')
			            .find('.ui-dialog-titlebar')
			            .removeClass('ui-corner-all')
			            .addClass('ui-corner-top');
	    			$(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"padding":"0 0.05em 0 0.05em","background-color":"#BCDCD7","font-size":"27px","color":"#000"});
						...
						success:function(data){
							...
							$('.tableNavCatList ul li').not('.nokeyboard').navigate({
							  ...
							  //*WR05022014 OPPN300 Revised
								...
								if(chk_ops=='Y'){
									jAlert('ลูกค้าถือบัตร ' + arr_data[9] + ' อยู่ไม่สามาถถือบัตรซ้ำได้', 'Warning message',function(){				           				
				        				return false;
				        			});	
									return false;
								}
								...
								//*WR25052015 for support ต่อบัตรจาก OPS เป็น OPT อะไรก็ได้ แต่ถ้าเป็น OPS เป็น OPS ต้องวันตรงกันเท่านั้น
								if(csh_application_type!='NEW'){								
									//case เลือกบัตรใหม่เป็นบัตรวันพฤหัสบดี สำหรับชุดต่ออายุ
									var chk_ops_day_old=$('#info_apply_promo_detail').html();		
									chk_ops_day_old=$.trim(chk_ops_day_old);
									var str_opsday="";	
									switch(chk_ops_day_old){
										case 'TH1':str_opsday="บัตรวันพฤหัสบดีที่1";break;
										case 'TH2':str_opsday="บัตรวันพฤหัสบดีที่2";break;
										case 'TH3':str_opsday="บัตรวันพฤหัสบดีที่3";break;
										case 'TH4':str_opsday="บัตรวันพฤหัสบดีที่4";break;
										case 'TU1':str_opsday="บัตรวันอังคารที่1";break;
										case 'TU2':str_opsday="บัตรวันอังคารที่2";break;
										case 'TU3':str_opsday="บัตรวันอังคารที่3";break;
										case 'TU4':str_opsday="บัตรวันอังคารที่4";break;
										default:chk_ops_day_old='';
									}									
									$('#csh_ops_day').val(chk_ops_day_new); //*WR เฉพาะต่ออายุเท่านั้น ระวังกระทบ OPID300
								}//if		
								//*WR25052015
								//----------------- set csh temp ----------------------
								//alert("arr_data[1]=>"+arr_data[1]);//สินค้าใน application_product.product_no ไม่มีตามลำดับ appcliation_list.product_no
								if(arr_data[1]!=''){
										var opts={
												type:'post',
												url:'/sales/member/setcshtemp',	
												...
												success:function(data){	
													...
													if(arr_res[0]=='2'){
														jAlert('จำนวนสินค้าไม่พอ Please check and try again.', 'Warning message',function(){															
															//TEST ON 13052012
															$('div.tableNavCatList ul>li').navigate('destroy');
															$("#dlgMemberCatalogList").dialog("close");
														...
														jAlert('ไม่สามารถทำรายการสินค้านี้ได้ Please check and try again.', 'Warning message',function(){
															...
															if(i<=parseInt(max_product_no)){
																...
																if($("#csh_gift_set").val()=='N'){
																	...
																	//จบโปร เพราะรับสินค้าเป็นเซตแล้ว
																	$("#csh_quantity").disable();
																	$("#csh_product_id").disable();
																	//ปรับปรุง flow ใหม่ key new member card หลังเล่นโปรเสร็จ
																	$("#dlgMemberCatalogList").dialog("close");
...
//																	}
										...
										//*WR 26062013 ทดสอบแก้ปัญหาบิล 01 บันทึกเป็นบิล 00
										if(status_no!='01'){
											$("#dlgMemberCatalogList").dialog("close");
											jAlert('เกิดปัญหาการตรวจสอบสถานะบิล \nกรุณายกเลิกการขาย แล้วเลือกบิล 01 อีกครั้ง', 'Warning message',function(){
												initTblTemp();
												initFormCashier();
												getCshTemp('N');
												browsDocStatus();
									...
									//case สินค้าในลำดับนี้ไม่พอ หรือ หมด stock
									//alert(i+"***<="+parseInt(max_product_no));
									if(i<=parseInt(max_product_no)){
										getCatProduct(application_id,i,j,k,pn,ps);
									}else{
										...
										if(status_no=='01' && $("#csh_gift_set").val()!='Y'){
											//ตรวจสอบ free product ได้สินค้าฟรีหรือไม่ free premium ให้สินค้าพิเศษหรือไม่										
											if($("#csh_gift_set_amount").val()!='0'){												
												giftsetProduct(application_id,employee_id,member_no,status_no,doc_tp);		
											}else if($("#csh_free_product").val()=='Y'){
												freeProduct(application_id,employee_id,member_no,status_no,doc_tp);
											}else if($("#csh_free_product").val()=='O'){												
												//*WR07032016  gifset ไม่พอ
												playMstPromotion(application_id,1);												
											}else if($("#csh_free_premium").val()=='Y'){
												freePremium(application_id,employee_id,member_no,status_no,doc_tp);
											}else{
											...
											//*WR26022014 case สินค้าหมด 2014
											$.ajax({
												type:'post',
...
//											},400);
	...
	function chkOpsExist(application_id){
		...
		var NewDialog = $('<div id="dlgIDCardOPS">\
	            <p><input type="text" size="13" id="chk_idcard_exist"><a href="#"  id="btn_idcard" class="btnGrayCleanIco">ตกลง</a></p>\<p><span id="ops_id_exist"></span></p>\</div>');
	        NewDialog.dialog({
	            modal: true,
	            title: "กรุณาระบุบัตรประชาชน",
	            //show: 'clip',
	            //hide: 'clip',
	            width:'27%',
	            height:'auto',
	            ...
	            open:function(){
	            	...
	            	,
	            buttons: [
	                {text: "ต่อไป >>", click: function(e) {
	                	 e.stopPropagation();
	                	 e.preventDefault();
	                	 var chk_idcard_exist=$('#chk_idcard_exist').val();
			             if(chk_idcard_exist.length==0){
			            	 jAlert('กรุณาระบุรหัสบัตรประชาชน', 'Warning message',function(){
	                ...
	                //,{text: "ยกเลิก", click: function() {$(this).dialog("close")}}
	            ]
	        });
	        return false;
	}//func
	...
	function checkMemberByCompany(promo_code,promo_des){
		...
		$("<div id='dlgCheckMemberByCompany'></div>").dialog({
	       	   autoOpen:true,
	  				...
	  				//showOpt: {direction: 'up'},		
	  				closeOnEscape: false,
	  				title:"<span class='ui-icon ui-icon-person'></span>กรุณาระบุรหัสสมาชิกปัจจุบันของบริษัท",
	  				open: function(){ 
						$(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"background-color":"#BCDCD7",
																										"font-size":"24px","color":"#000000",
																										"padding":"5 0.1em 0 0.1em"});   //ui-corner-all
			    	                ...
			    	                click: function(evt){ 
			    	                	 ...
			    	                	 var member_no_by_comp=$('#check_member_no_by_comp').val();
			 				             if(member_no_by_comp.length==0){
			 				            	jAlert('กรุณาระบุรหัสสมาชิกเพื่อตรวจสอบ', 'Warning message',function(){
			 				            		$('#check_member_no_by_comp').focus();	
												return false;
											});	
			 				            	return false;
			 					         ...
			 					         var opts_chkops={
			 					            		...
			 					            		data:{
			 					            			...
			 					            			if(objMb.status=='Y'){	
			 					            				...
			 					            				str_show=str_show + "<hr> ไม่สามารถใช้โปรโมชั่นนี้ได้ Please check and try again. \n";			 					            				
			 					            				jAlert(str_show, 'Warning message',function(){
		 									    				$("#dlgCheckMemberByCompany").dialog('close');
		 														return false;
		 													});	
	...
	function getRegMemberPromotion($member_no_ref){
		/**
		*@pram String $member_no_ref : แสดงโปรโมชั่นยกเว้นบิล OPPN300 
		*@return
		*/
									...
									$('div.tableNavMemberPromotion2 ul>li').not('.nokeyboard').navigate({
									  ...
									  //*WR06122013 New OPPC300 2014
									    ...
									    if(selpromo.application_id=='OPPC300' && mp_point_sum < 30){
									    	 jAlert('คะแนนไม่พอ Please check and try again.', 'Warning message',function(){
									    		 $("#dlgRegPromo").dialog("close");
									    		 initTblTemp();
											     initFormCashier();
											     initFieldOpenBill();
									    	...
									    	$("#csh_point_receive").val(0);   //คะแนนที่ได้รับ
											$("#csh_point_used").val('30');	     //คะแนนที่ใช้ 
											$("#csh_point_net").val('-30');      //คะแนนสุทธิ
											$("#csh_xpoint").val('1');
									    }
										...
										$("#csh_gift_set").val(selpromo.gift_set);//สมัครสมาชิกเลือกรับ Gift Set หรือไม่		
										$("#csh_gift_set_amount").val(selpromo.amount);//มูลค่าสินค้าแทนชุด Gift Set	
										$("#csh_register_free").val(selpromo.register_free);//check for free is 'Y' or not free is 'N' effect for bill DN or SL
										$("#csh_free_product").val(selpromo.free_product);//ได้สินค้าฟรีหรือไม่
										$("#csh_free_product_amount").val(selpromo.free_product_amount);
										$("#csh_product_amount_type").val(selpromo.product_amount_type);
										$('#csh_free_premium').val(selpromo.free_premium);//ให้สินค้าพิเศษหรือไม่		
										...
										if(selpromo.redeem_point!=0){
											...
											opts_aa={
							           			...
							           			data:{
							           				...
							           				$("#csh_point_total").html(data);//คะแนนสะสม
							           				var point_receive=$("#csh_point_receive").val();
							           				var point_used=selpromo.redeem_point;//คะแนนที่ใช้
							           				var point_net=parseInt(point_receive)-parseInt(point_used);
							           				$("#csh_point_used").val(point_used);
							           				$("#csh_point_net").val(point_net);//คะแนนสุทธิ									           		
							           			}
							           		};
							           		$.ajax(opts_aa);
							           	}//if	
	...
	function selMemberPromotion(){
		...
		var dialogOpts_MemberRef={
		        ...
		        modal: true,
		        closeOnEscape:true,
		        title:"<span class='ui-icon ui-icon-person'></span>รหัสบัตรสมาชิกเก่า",
		        width:'auto',
		        height:'auto',
		        ...
		        open:function(event,ui){
					...
					$(this).dialog('widget')
					 ...
					 $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"padding":"10px","margin-top":"0",
	    					"background-color":"#BCDCD7","font-size":"22px","color":"#000"});
	    			$(this).dialog("widget").find(".ui-dialog-buttonpane")
                    .css({"padding":"0","margin":"0 0 0 0","background-color":"#C7D9DC"});
			   ...
			   buttons:{
				    	'ตกลง':function(evt){
				            ...
				            if($refer_member_st_val.length==0){
				            	...
				            	//กรณีไม่ระบุรหัสสมาชิกเก่าให้กำหนดเป็นเปิดบิลสมาชิกใหม่ OPPN300
				            	getRegMemberPromotion('');
					            return false;
					        }else{
					            ...
					            if(getOnlineStatus()===1){
					           	...
					           	var opts_memexpire={
						           		...
						           		success:function(data){						           			
							           		if(data==''){
							           			jAlert('ไม่พบรหัสบัตรสมาชิกเก่า Please check and try again.', 'Warning message',function(){
							           				initMemberCatalog();//*WR25092012
							           				$refer_member_st.val('').focus();
							        				return false;
							        			});	
									           	...
									           	/*######################################*/
													...
													if(m.link_status=='ONLINE'){
														...
														switch(m.cust_day){
															...
															case "TH1":cust_day="พฤหัสบดีที่1";break;
															case "TH2":cust_day="พฤหัสบดีที่2";break;
															case "TH3":cust_day="พฤหัสบดีที่3";break;
															case "TH4":cust_day="พฤหัสบดีที่4";break;
															case "TU1":cust_day="อังคารที่1";break;
															case "TU2":cust_day="อังคารที่2";break;
															case "TU3":cust_day="อังคารที่3";break;
															case "TU4":cust_day="อังคารที่4";break;
															default:cust_day="";
														}											
														$('#csh_member_fullname').html(member_fullname);
														$('#csh_birthday').html(birthday);
														...
														if(m.exist_status=='NO'){
															jAlert('ไม่พบข้อมูลรหัสสมาชิก Please check and try again.', 'Warning message',function(){
																initFormCashier();
																initFieldOpenBill();
																return false;
															});	
	...
	function getMemberCatalog(){
		/**
		*@desc old process last update 02022012 ตัวนี้เป็นการเลือกโปรโมชั่นก่อนการป้อนรหัสสมาชิก เป็น flow แบบเก่า
		*@return
		*/
		var dialogOpts_MemberCatalog = {
				...
				showOpt: {direction: 'up'},		
				closeOnEscape:true,	
				title:"<span class='ui-icon ui-icon-person'></span>เลือกชุดสมัคร",
				open: function(){    
						//init value 			
						...
						function(){	
							...
							$('.tableNavCat ul li').not('.nokeyboard').navigate({
								...
								$("#csh_gift_set").val(arr_cat[5]);//สมัครสมาชิกเลือกรับ Gift Set หรือไม่		
								$("#csh_gift_set_amount").val(arr_cat[6]);//มูลค่าสินค้าแทนชุด Gift Set	
								...
								if(arr_cat[2]=='Y'){
									//refer_member_st='Y' แสดงว่าป้อนรหัสสมาชิกเก่า
									$("#dlgMemberRef").dialog({
								        autoOpen:true,
								        resizable:false,
								        modal: true,
											           	...
											           	var opts={
												           		...
												           		success:function(data){												           			
													           		...
													           		if(arr_cat[8]!=0){
														           		...
														           		$.ajax({
															           			...
															           			data:{
															           				...
															           				$("#csh_point_total").html(data);//คะแนนสะสม
															           				var point_receive=$("#csh_point_receive").val();
															           				var point_used=arr_cat[8];//คะแนนที่ใช้
															           				var point_net=parseInt(point_receive)-parseInt(point_used);
															           				$("#csh_point_used").val(point_used);
															           				$("#csh_point_net").val(point_net);//คะแนนสุทธิ
																           			//----------------------point-------------------------------
													           		...
													           		if(arr_app[0]=='2'){														           			
													           			...
													           			jAlert('ไม่สามารถต่อชุดสมัครหลังเดือนหมดอายุได้ Please check and try again.', 'Warning message',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='4'){
													           			jAlert('ไม่สามารถต่อชุดสมัครได้ Please check and try again.', 'Warning message',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='5'){
													           			jAlert('ยอดเงินน้อยกว่า '+addCommas(arr_app[2])+' ไม่สามารถต่อชุดสมัครได้ Please check and try again.', 'Warning message',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='6'){														           			
													           			jAlert('ไม่พบรายการชุดสมัคร Please check and try again.', 'Warning message',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='7'){														           			
													           			jAlert('คะแนนสะสมน้อยกว่า '+arr_cat[8]+' Please check and try again.', 'Warning message',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='8'){														           			
													           			jAlert('AFTER CASE COMMING SOON.', 'Warning message',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='9'){
													           			jAlert('ยอดเงินเท่ากับหรือมากกว่า '+addCommas(arr_app[2])+' ไม่สามารถต่อชุดสมัครได้ Please check and try again.', 'Warning message',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='10'){
													           			jAlert('สามารถต่อก่อนเดือนหมดอายุ 1 เดือนหรือภายในเดือนหมดอายุเท่านั้น Please check and try again.', 'Warning message',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
									...
									//สมัครสมาชิกใหม่ไม่ต้องอ้างบัตรเดิม OPPN300,OPPG300
									initFieldOpenBill();
									//$("#csh_member_no").focus();
								}//end if
	...
	function checkMemberExist(){
		/**
		*@desc ตรวจสอบมีสมาชิกตามรหัสที่ key หรือไม่
		*@return
		*/
			...
			var opts_member={
					...
					success:function(data){
						if(data=='1'){
							...
							$("#csh_expire_date").val(data);
							$("#info_member_expiredate").html(expire_date);
							//แบบใหม่ต้อง show promotion ให้เลือกก่อนแม้เป็นมีโปรโมชั่นเดียว
							//---------------------- start ---------------------------------------------
							$("#dlgRegPromo").dialog({
										autoOpen:true,
										width:'60%',
		...
		*/
		...
		if(member_id.length==0){
			...
			$.getJSON(
					...
					function(data){
						if(data==1){
							jAlert('ไม่พบรหัสสมาชิกในทะเบียน หรือวันที่สมัครไม่ตรงกับวันที่เปิดบิล','Warning message',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
							...
							jAlert('รหัสสมาชิกเคยใช้สิทธิแล้ว ไม่สามารถใช้สิทธิซ้ำได้','Warning message',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
							...
							jAlert('เกิดผิดพลาด ไม่สามารถใช้สิทธิซ้ำได้','Warning message',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
	...
	function cardLostStock(){
		...
		*/
		...
		var ops_day=$("#info_apply_promo_detail").html();
		switch(ops_day){
			case "OPS1":msg_show="พฤหัสบดีที่1";break;
			case "OPS2":msg_show="พฤหัสบดีที่2";break;
			case "OPS3":msg_show="พฤหัสบดีที่3";break;
			case "OPS4":msg_show="พฤหัสบดีที่4";break;
			case "OPS0":msg_show="พฤหัสบดีที่0";break;
			case "OPT1":msg_show="อังคารที่1";break;
			case "OPT2":msg_show="อังคารที่2";break;
			case "OPT3":msg_show="อังคารที่3";break;
			case "OPT4":msg_show="อังคารที่4";break;
			default:msg_show="";
		}
		var opts={
					type:'post',
					...
					success:function(data){
						if(data=='0'){
							jAlert('รหัสบัตรนี้ไม่พบในทะเบียน หรือต้องเป็นบัตรเสีย-บัตรหายเท่านั้น','Warning message',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='2'){
							jAlert('รหัสสมาชิกนี้ถูกใช้สมัครแล้ว','Warning message',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='3'){
							jAlert('กรุณาเลือกบัตร ' + msg_show,'Warning message',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='4'){
							jAlert('รหัสบัตรนี้ไม่พบในทะเบียน หรือท่านยังไม่ได้ลงทะเบียนบัตร','Warning message',function(){
								initFieldOpenBill();
								return false;
							});	
						}else{
							if(ops_day=='OPT1'){
								$("#csh_product_id").val('25654');
							}else if(ops_day=='OPT2'){
								$("#csh_product_id").val('25655');
							}else if(ops_day=='OPT3'){
								$("#csh_product_id").val('25656');
							}else if(ops_day=='OPT4'){
								$("#csh_product_id").val('25657');
							}else if(ops_day=='OPS1'){
								$("#csh_product_id").val('23099');
							}else if(ops_day=='OPS2'){
								$("#csh_product_id").val('22249');
							}else if(ops_day=='OPS3'){
								$("#csh_product_id").val('21887');
							}else if(ops_day=='OPS4'){
								$("#csh_product_id").val('23852');
							}								
		...
		var opts={
					...
					success:function(data){
						if(data=='0'){
							...
							jAlert('รหัสบัตรนี้สินค้าคงเหลือไม่พอ Please check and try again.','Warning message',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='6'){
							jAlert('กรุณาเลือกบัตร OPS White Card','Warning message',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='7'){
							jAlert('กรุณาเลือกบัตร OPS Gold Card','Warning message',function(){
								initFieldOpenBill();
								return false;
							});	
						}else{
		...
		var opts={
					...
					success:function(data){
						if(data=='0'){
							...
							//$("#csh_product_id").val('23983');//fixed product id
							if(cmdEnterKey("csh_product_id")){
								setTimeout("delayPayment()",1000);
							}
						}
	...
	function getMemberProfile(col,txtsearch){
		...
		*/
		 $.getJSON(
					...
					function(data){
						if(data=='0'){
							jAlert('ไม่พบข้อมูลสมาชิก', 'Warning message',function(){
	...
	function getProMemPriv(member_no,birthday,apply_date,expire_date,tabs){
				...
				data:{
					...
					$('.tableNavMemberPriv ul li').not('.nokeyboard').navigate({
					    ...
					    if(selpromo.promo_status=='N'){
						    //ใช้สิทธิแล้ว
					    	jAlert(selpromo.promo_detail,'Warning message',function(){
								return false;
							});	
						    return false;
						}
					    ...
					    if(selpromo.promo_code=='NEWBIRTH'){
							    ...
							    var opts_pro={
							    			...
							    			success:function(data){
								    			if(data!=''){
								    				 ...
								    				 $('#csh_web_promo').val('Y');//*WR14082012 สถานะใช้สิทธิสมาชิกบนเวบ
						    ...
						    $.ajax({
							    ...
							    data:{
								    ...
								    if(data=='0'){
								    	jAlert('ไม่พบข้อมูลโปรโมชั่น Please check and try again.', 'Warning message',function(){
								    		$("#other_promotion_cmd").html('');
									    	$('.tableNavMemberPriv ul').navigate('destroy');
		...
		setTimeout(function(){
			if(objReqMemPriv){
				...
				objReqMemPriv.abort();
				objReqMemPriv=null;
				jConfirm('ไม่พบข้อมูลที่ร้องขอจาก Server ลูกค้ายืนยันใช้สิทธิประโยชน์สมาชิก ใช่หรือไม่?','Confirm message', function(r){
			        if(r){
...
//									}
									...
									if(m.application_id=='' || m.application_id==undefined){
										var spd_age_card=m.special_day + " (" + m.age_card + " ปี) ";
			...
			setTimeout(function(){
			  ...
			  if (objReqMemberInfo){
				  ...
				  objReqMemberInfo=null;
				  jConfirm('ไม่มีการตอบสนองจากการร้องขอข้อมูล ONLINE\n คุณต้องการเปลี่ยนเป็นโหมด	OFFLINE หรือไม่?','Confirm message', function(r){
				        if(r){
				        	callMemberOffline();
				        	return false;
				        }else{
	...
	function stockLost(){
		...
		*/
		...
		if(employee_id.length==0){
			jAlert('กรุณาป้อนรหัสพนักงาน', 'Warning message',function(){
				initFieldOpenBill();
				return false;
			});	
			return false;
			...
			//check employee info
			 var opts={
		            	...
		            	success:function(data){
			            	var arr_data=data.split('#');
							if($.trim(arr_data[0])==""){
								jAlert('ไม่พบรหัสพนักงาน Please check and try again.','Warning message',function(){
									initFieldOpenBill();
									return false;
				    			});
							}else if(data!=""){
				               ...
				               $("<div id='dlgAuditChkStock'></div>").dialog({
				            	   autoOpen:true,
					   				...
					   				showOpt: {direction: 'up'},		
					   				closeOnEscape: true,
					   				title:"<span class='ui-icon ui-icon-person'></span>รหัสผ่าน Audit",
					   				open: function(){    
					   					$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
										$(this).dialog('widget')
								            .find('.ui-dialog-titlebar')
						   			        ...
						   			        if(key == 13){
							   			        $.ajax({
										            	...
										            	success:function(data){
										            		var arr_data=data.split('#');
															if($.trim(arr_data[0])==""){
																jAlert('ไม่พบข้อมูล Audit Please check and try again.','Warning message',function(){
																	$("#audit_password").focus();
																	return false;
												    			});
															}else if(data!=""){
	...
	function dlgCardLost(){
		...
		*/
		var dialogOpts_dlgCardLost = {
				...
				showOpt: {direction: 'up'},		
				closeOnEscape: true,
				title:"<span class='ui-icon ui-icon-person'></span>ประเภทบัตร",
				open: function(){     
					$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
					$(this).dialog('widget')
			            .find('.ui-dialog-titlebar')
			            .removeClass('ui-corner-all')
			            .addClass('ui-corner-top');
	    			$(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"background-color":"#BCDCD7","padding":"0","color":"#000"});
						$("input:text").blur();
						$("#dlgCardLost").html("");
						...
						function(){	
							$('.tableNavCardLost ul li').not('.nokeyboard').navigate({
							    ...
							    e.stopPropagation();
							    var chk_ble=0;
							    var card_status=$(this).attr("id");//C1 บัตรหาย,C2 บัตรเสีย
								...
								var opt_frmcardlost={
										...
										showOpt: {direction: 'up'},		
										closeOnEscape: true,
										title:"<span class='ui-icon ui-icon-person'></span>กรุณาระบุเงื่อนไขการค้นหา",
										open:function(){
											$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
											$(this).dialog('widget')
									            .find('.ui-dialog-titlebar')
																...
																*/
																...
																if(obj.link_status=='ONLINE'){
																	...
																	if(obj.cust_day!=null && obj.cust_day!=''){
																				switch(obj.cust_day){
																				case "OPS1":cust_day="พฤหัสบดีที่1";break;
																				case "OPS2":cust_day="พฤหัสบดีที่2";break;
																				case "OPS3":cust_day="พฤหัสบดีที่3";break;
																				case "OPS4":cust_day="พฤหัสบดีที่4";break;
																				case "OPS0":cust_day="พฤหัสบดีที่0";break;
																				case "OPT1":cust_day="อังคารที่1";break;
																				case "OPT2":cust_day="อังคารที่2";break;
																				case "OPT3":cust_day="อังคารที่3";break;
																				case "OPT4":cust_day="อังคารที่4";break;
																					default:cust_day="";
																				}
																	}else{
																		cust_day='';
	...
	function browsDocStatus(){
		/**
		*@desc เลือกประเภทบิลหรือเปลี่ยนประเภทบิล กำหนดค่าตั้งต้นสำหรับการขาย
		*@return
		*/
...
//								$("#csh_doc_tp_show").empty().html(seldoc.description);
								...
								if(seldoc.status_no=='01'){
									...
									//เปลี่ยนสินค้า CN เปิดบิลประกบใบลดหนี้
									initFormBySelDoc();
									selCN();
	...
	function initFormBySelDoc(){
		/**
		 *@desc clear for init cashier ตามการเลือกประเภทเอกสาร
		 *@desc แก้ปัญหา default bill 00
		 *@return null
		 *@modify 15022013
		 */
		//get lock status
		...
		$('#csh_doc_tp').val('SL');		
		$('#flgbill').html('');
		$('#csh_discount_member').val('N');//ได้รับส่วนลดสมาชิกหรือไม่
		$('#csh_get_point').val('N');//ได้รับคะแนนหรือไม่
		//vat total
		$('#csh_doc_tp_vt').val('');
		$('#vt_name_val').val('');
		$('#vt_address_val1').val('');
		...
		$('#csh_free_product').val('');//ได้สินค้าฟรีหรือไม่
		$('#free_product_amount').val('');//มูลค่าสินค้าแถมฟรี
		$('#product_amount_type').val('');//เงื่อนไขจำนวนเงินแถมฟรี G,L
		$('#csh_free_premium').val('');//ให้สินค้าพิเศษหรือไม่
		$('#free_premium_amount').val('');//จำนวนเงินสินค้าพิเศษ
		$('#premium_amount_type').val('');//เงื่อนไขจำนวนเงินแถมฟรี G,L
		//firstbuy
		$('#csh_first_sale').val('');
		$('#csh_first_percent').val('');
		$('#csh_xpoint').val('1');
		$('#csh_play_main_promotion').val('');//มีผลกลับ select box  status_pro ประเภทเล่นโปรไม่เล่นโปร
		$('#csh_play_last_promotion').val('');//มีผลกลับ select box status_pro ประเภทเล่นโปรไม่เล่นโปร
	...
	function initFormCashier(){
		...
		$('#premium_amount_type').val('');//เงื่อนไขจำนวนเงินแถมฟรี G,L
		$('#csh_application_type').val('');//NEW=สมัครใหม่, ALL=ต่ออายุ
		$('#csh_card_type').val('');//CARD=บัตรสมาชิก, IDC=บัตรประจำตัวประชาชน
		//firstbuy
		$('#csh_first_sale').val('');
		$('#csh_first_percent').val('');
		$('#csh_xpoint').val('1');
	...
	function swapCashier(swap_cashier_id){
		...
		*/
		$.getJSON(
				...
				function(data){
					if(data==null){
						jAlert('ไม่พบรหัสนี้กรุณาตรวจสอบอีกครั้ง', 'Warning message',function(){
							return false;
						});	
					}else{
						$.each(data.emp, function(i,m){
							if(m.exist=='yes'){
								if(m.check_in_status=='Y'){
									...
									jAlert('รหัสนี้ไม่อยู่ในสถานะ Login ระบบ', 'Warning message',function(){
										return false;
									});	
								}
							}							
	...
	function printBill(str_doc_no){			
		...
		var point_total_today=$('#csh_point_total').html();//คแนนสะสม ณ วันนี้
		...
		var opts={
				...
				success:function(data){
					...
					if(parseInt(arr_data[0])>0){		
						...
						jAlert('Not found Sell ​​Items. Please check and try again.','Warning message',function(){
							return false;
		    			});
				    	ret= 'N';
					}
	...
	function selProductFree(promo_code){		
		/**
		 * @desc สินค้าแถมฟรี เช่น SAMPLING01
		 * @note :05092014 มีปัญหาเวลาสินค้าไม่มีในสต๊อก แล้วสแกนขายตามปกติสินค้าตัวนั้นจะกลายเป็นสินค้าแถมฟรี
		 * แก้ปัญหาเบื้องต้นด้วยการตรวจสอบว่าสินค้าตัวนั้นมีในสต๊อกหรือไม่ ไม่มีไม่โชว์
		 * @return NULL
		 */
		$("#dlgMemberProductFree").dialog({
	        ...
	        modal: true,
	        closeOnEscape: true,
	        title:"<span class='ui-icon ui-icon-person'></span>เลือกสินค้า",
	        width:'60%',
	        height:350,
	        overlay:{ backgroundColor:"#999",opacity:0.5},
	        open:function(event,ui){
	        	 		...
	        	 		$.ajax({
	        	 				...
	        	 				success:function(data){
		        	 				...
		        	 				$('.tableNavProductFree ul li').not('.nokeyboard').navigate({
								    	...
								    	$("#csh_play_main_promotion").val('N');
								    	$("#csh_play_last_promotion").val('N');								    	
								    	//*WR 22032013 รอปรับให้บันทึกแบบ auto ลง temp ไม่ต้องผ่าน key input csh_product_id
						...
						$.each(data.member, function(i,m){
							if(m.exist=='yes'){
								...
								if(m.status=='1'){
									...
									if(m.mem_status=='N'){
										$("#sch_desc").append('\nรหัสสมาชิกนี้ถูกกำหนดให้มีสถานะเป็น non active เพราะสมัครบัตรใหม่');
									}else if(m.mem_status=='F'){
										$("#sch_desc").append('\nรหัสสมาชิกนี้ถูกกำหนดให้มีสถานะเป็น non active \n เพราะเป็นบัตรเสีย\n Please check and try again.');
									}else if(m.mem_status=='T'){
										$("#sch_desc").append('\nรหัสสมาชิกนี้ถูกกำหนดให้มีสถานะเป็น non active \n เพราะเป็นบัตรหาย');
									}									
								}
							...
							if($('#status_couponpromo').val()=='Y'){
								...
								if(csh_net > 0 && buy_status=='G' && csh_net < start_baht ){
									...
									if($('#csh_promo_amt_type').val()=='G' && csh_sum_amount < csh_promo_amt){											
										jAlert('ยอดซื้อน้อยกว่า ' + $('#csh_promo_amt').val() + ' ไม่สามารถชำระเงินได้ Please check and try again.','Warning message',function(){
											$("#btn_cal_promotion").removeAttr("disabled");
											$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
											return false;
										});
...
//			}
		...
		shortcut.add("ESC",function(e){
			//*WR10022014 Revised ใช้สิทธิวันเกิดสมาชิก 4 ปีรับฟรี 25096
...
//			}
		...
		$("#btnBwsMemberScoreOfDay").click(function(e){
			...
			$("<div id='dlgMemberPoint'></div>").dialog({
			     ...
			     modal: true,
			     width: 600,
			     title:"<span class='ui-icon ui-icon-person'></span>รายการใช้คะแนนวันนี้",
			     open:function(){
			    	 $(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
						$(this).dialog('widget')
				            .find('.ui-dialog-titlebar')
...
//		$("#csh_product_id").numericFilter();
		...
		$("#btn_cal_promotion").click(function(e){
			...
			if(chk_percent_discount.length>0 && chk_member_no.length==0){
				jAlert('ไม่พบรหัสสมาชิก กรุณาป้อนรหัสสมาชิก Please check and try again.','Warning message',function(){					
					return false;
    			});
				return false;
			}			
			...
			if(jQuery.trim($("#csh_status_no").val())=='01'){
				...
				if(parseInt(csh_net)>0){					
					//*WR29012015 for ของแถม First Purchase OPPN300
								...
								data:{
									...
									setTimeout(function(){																			
										...
										var net_amt=$('#csh_net').val().replace(/[^\d\.\-\ ]/g,'');
								 		...
								 		var amount=$('#csh_sum_amount').val().replace(/[^\d\.\-\ ]/g,'');
								 		amount=parseFloat(amount);
								 		//*WR26082014 ตรวจสอบยอดเงินใน method selCoPromotion
				...
				if(ecoupon_amount_balance>csh_net){
					//ยอดสิทธิมากกว่ายอดซื้อ					
					ecp_amount_bal=csh_net;
				}else{
					//ยอดสิทธิน้อยกว่ายอดซื้อ					
			   						...
			   						$('#ecp_amount').keypress(function(evt){
					   			        ...
					   			        if(key == 13){
						   			        ...
						   			        if(parseFloat(chk_ecp_amount_bal)>parseFloat(ecoupon_amount_balance)){
						   			        	jAlert('ไม่สามารถใช้สิทธิมากกว่าวงเงินที่เหลือได้ Please check and try again.','Warning message',function(){
						   			        		$('#ecp_amount').val(ecp_amount_bal).focus();
						   							return false;
						   		    			});
							   			        return false;
				...
				if($promo_tp.val()=='N'){
					...
					if($csh_net<$start_baht){
						jAlert('ยอดเงินสุทธิน้อยกว่า '+$start_baht+' Please check and try again.','Warning message',function(){
							$("#btn_cal_promotion").removeAttr("disabled");
							$('#btn_cal_promotion').removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
							$("#csh_product_id").focus();
		    			});
								...
								$.ajax({
										...
										success:function(data){
											if(data=='3'){
												...
												jAlert('!!! ยอดเงินสุทธิจากการคำนวณส่วนลด พบข้อผิดพลาด !!!\nกรุณาลบรายการสินค้าที่ราคาสูงสุดออก และสแกนสินค้าชิ้นนั้นอีกครั้ง','Warning message',function(){	
													rePromoPointDiscount();
													$("#btn_cal_promotion").removeAttr("disabled");
				    								$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
													return false;
            							...
            							success:function(data){
            								...
            								$('.tableNavPercentRange ul li').not('.nokeyboard').navigate({
											    ...
											    if(point>point_total){
											    	jAlert('คะแนนสะสมไม่พอ Please check and try again.','Warning message',function(){													
														return false;
									    			});
											    	return false;
											    }
											    ...
											    if(chkDiaryTemp()=='Y'){
											    	...
											    	if($play_last_pro=='N'){
														...
														$.ajax({
																...
																success:function(data){
																	if(data=='3'){
																		//*WR30012014 OX24171113=The first purchase 1 คะแนนส่วนลด 5 บาท รับสิทธิส่วนลด 30-50%
				...
				//*WR08042015 promo_other โปร Mobile เล่นร่วมกับโปรหลัก
				var play_last_promotion=$('#csh_play_last_promotion').val();
				...
				//alert("promo_type=>" + promo_type);
				
				//*WR22072014 for support last promotion
		...
		$('#btnCoupon').button({
	    	...
	    	var onsales=onSales();
		    if(onsales>0){
			 	jAlert('You make a pending sale. Can not select the promotion after the sale.', 'Warning message',function(){	            	
	            	return false;
		        });
			 	return false;
			}
		    			    		 ...
		    			    		 $('.tableNavCouponPro ul li').not('.nokeyboard').navigate({
										  ...
										  //*WR27102014
										    ...
										    if(sel_promo.member_tp == 'Y' && member_no.length<1){
										    	jAlert('กรุณาสแกนรหัสสมาชิกก่อนเล่นโปรโมชั่น', 'Warning message',function(){
						    					...
						    					jAlert('ลูกค้า WALK IN กรุณากดคีย์ ENTER ที่ช่องรหัสสมาชิก \nเพื่อแสดงสถานะลูกค้าประเภท WALK IN ก่อนเล่นโปรโมชั่น', 'Warning message',function(){
										    		$('#dlgProCoupon').dialog('close');
										    		initTblTemp();
													initFormCashier();
													initFieldOpenBill();
							    			...
							    			if(sel_promo.promo_tp=='LINE' || sel_promo.promo_tp=='MCOUPON'){	
							    				...
							    				if(sel_promo.promo_code=='OK27230415'){
						    						var id_card=$('#csh_id_card').val();
						    						var mobile_no=$('#csh_mobile_no').val();
						    						fromreadprofile_idcard(sel_promo.promo_code,member_no,id_card,mobile_no,'idcard'); 						    						
						    						$('#dlgProCoupon').dialog('close');
						    						return false;
						    					}
							    				if(sel_promo.promo_code=='OX02230315' || sel_promo.promo_code=='OX02031215'){
							    					//MGM member get member
						    						listfriend(member_no);
						    						$('#dlgProCoupon').dialog('close');
						    						return false;							    					
						    					}
							    				fromreadprofile(sel_promo.promo_code,otp_status,member_no,id_card,mobile_no,chk_mem_idcard,sel_promo.promo_des);	
						    				...
						    				if(sel_promo.promo_tp=='COUPON' && sel_promo.type_discount == 'PERCENT'){
						    					...
						    					$('#dlgProCoupon').dialog('close');	
							    				 //check coupon discount for match pro ส่วนลด coupon percent
							    				$('<div id="dlgChkCouponDiscount"><input type="text" size="15" id="coupon_code2" class="keybarcode"/></div>').dialog({
														   ...
														   resizable:false,
														   closeOnEscape:false,		
												           title: "ตรวจสอบ Coupon " + parseInt(sel_promo.discount) + "%",
												           position:"center",
												           open:function(){
																$(this).dialog('widget')
														            .find('.ui-dialog-titlebar')
														    	            ...
														    	            { 
														    	                ...
														    	                click: function(evt){ 
																		    		...
																		    		if(coupon_code.length != 12){
																	    				jAlert('กรุณาป้อนรหัส Coupon ให้ครบ 12 หลัก', 'Warning message',function(){
																		    		...
																		    		var chk_coupon_disocunt=coupon_code.substring(4,6);
																		    		//alert(chk_coupon_disocunt + "<=>" + parseInt(sel_promo.discount));																		    		
																		    		if(chk_coupon_disocunt != parseInt(sel_promo.discount)){
																		    			jAlert('รหัส Coupon ' + coupon_code + ' ไม่ตรงกับโปรโมชั่นส่วนลด ' +  parseInt(sel_promo.discount) + "%", 'Warning message',function(){
						  			  ...
						  			  //*WR27102014
						  			    ...
						  			    $.each(arr_keyinput , function(i, val) { 
						  			    	if(arr_keyinput[i]=='I'){
						  			    		j_mc+=1;
						  			    		key_idcad='<tr><td>รหัสบัตรประชาชน :</td><td><input type="text" id="coupon_idcard" size="20" index="'+j_mc+'" class="input-text-pos keymbinput ui-corner-all"></td></tr>';
						  			    	}
						  			    	if(arr_keyinput[i]=='R'){
						  			    		j_mc+=1;
						  			    	...
						  			    	if(arr_keyinput[i]=='M'){
						  			    		j_mc+=1;
						  			    		key_mobile='<tr><td>เบอร์โทรศัพท์ : </td><td><input type="text" id="mobile_code" size="20" index="'+j_mc+'" class="input-text-pos keymbinput ui-corner-all"><td></tr>' ;
						  			    	}
						  				});
											    	                	...
											    	                	evt.preventDefault();
																 		...
																 		$.ajax({
																 			...
																 			data:{
																 				...
																 				var objJson=$.parseJSON(data);
																				if(objJson.status=='OK'){
																					//assume check coupon is complete.
																					$('#status_couponpromo').val('Y');
																					$('#csh_coupon_code').val(coupon_code);
																					$('#csh_id_card').val(idcard);//*WR21012015 for coupon
																					//*WR16102014-27102014
																					if(promo_code!='OK04421014' && promo_code!='OK01251114' && 
																							promo_code!='OK04241114' && promo_code!='OK06451214' && $('#csh_promo_tp').val()=='MCOUPON'){
																					//if(promo_code!='OK04421014' && $('#csh_promo_tp').val()=='MCOUPON'){
																					//if(promo_code=='OK02411014' || promo_code=='OS02401014' || promo_code=='OK02431014'){
																						
																						 /////////////////////////////////////// FORM KEYPRODUCT 16102014 \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
															  							 //สแกนรหัสสินค้าที่ร่วมรายการโปรโมชั่น 
																	  					$('<div id="dlgPdtMCoupon"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span><input type="text" size="15" id="pdt_mcoupon"/></div>').dialog({
																							           autoOpen:true,
																									   width:'400',
																									   height:'200',	
																									   modal:true,
																									   resizable:false,
																									   closeOnEscape:false,		
																							           title: "สแกนรายการสินค้าที่ร่วมโปร",
																							           position:"center",
																							           open:function(){
																											$(this).dialog('widget')
																									            .find('.ui-dialog-titlebar')
																							  			  ...
																							  			  $(this).dialog("widget").find(".ui-widget-overlay").live('click', function(){
																								  			...
																								  			$('#pdt_mcoupon').keypress(function(evt){																    	
																											    	var key = evt.keyCode ? evt.keyCode : evt.which ? evt.which : evt.charCode; 
																								   			        if(key == 13){	
																										    			evt.preventDefault();
																										    			evt.stopPropagation();
																					    				...
																					    				buttons: [ 
																									    	            { 
																									    	                ...
																									    	                click: function(evt){ 
																									    	                	evt.preventDefault();
																														 		evt.stopPropagation();
																														 		var product_id=$('#pdt_mcoupon').val();
																														 		product_id=$.trim(product_id);																		 			
																													    		if(product_id.length<1){
																												    				jAlert('กรุณาสแกนรหัสสินค้าที่ร่วมรายการโปรโมชั่น', 'Warning message',function(){
																												    					$('#pdt_mcoupon').val('').focus();
																												    					return false;
																												    				});	
																												    				return false;
																													    		}
																						...
																						//*WR27102014
																						 jAlert('ท่านสามารถใช้สิทธิเล่นโปรโมชั่นได้', 'ผลการตรวจสอบ',function(){																						
																							initFieldPromt();
																							return false;
																						});
...
//																					 jAlert('ท่านสามารถใช้สิทธิเล่นโปรโมชั่น Mobile Coupon ได้', 'ผลการตรวจสอบ',function(){																						
//																						initFieldPromt();
//																						return false;
//																					});
																					...
																					$('#status_couponpromo').val('N');
																					$('#csh_coupon_code').val('');
																					 jAlert(objJson.status_msg, 'ผลการตรวจสอบ',function(){
																						$('#csh_member_no').focus(); 
																						return false;
																					});	
																					initTblTemp()
		...
		$('#btnClosePageCashier').button({
			...
			e.preventDefault();
			jConfirm('คุณต้องการปิดหน้าขายใช่หรือไม่?', 'Confirm message', function(r) {
			    if(r){	
		           ...
		           position:"center",
		            open:function(){		
		            	...
		            	var objReqUp2Data=$.ajax({
							...
							success:function(){
								objReqUp2Data=null;
//								 jAlert('อัพเดทข้อมูลสมบูรณ์', 'ผลการทำงาน',function(){			
//										return false;
//									});	
								 return false;
		...
		$('#btnPromotionData').click(function(e){
			...
			var opts_dlgPromoItems = {
					...
					open: function(){  
					    ...
					    $(this).parents(".ui-dialog:first")
					    .find(".ui-dialog-content")
					    .css({"background-color":"#BCDCD7","font-size":"22px","color":"#0000FF","padding":"0"});						  
								...
								$('#dlgCoOperation').empty().html(data);	
					    		 $('.tableNavSelCooperation ul li').not('.nokeyboard').navigate({
								    ...
								    if(promo_play=='TOUR01'){				    					
				    					...
				    					if(member_type!=='WALK IN'){
				    						jAlert('This promotion is for WALK IN customers only.', 'Warning message',function(){										    	
										    	return false;
										    });	
										    return false;
				    					}else{
																		...
																		objReq=null;
																			$.each(data.member, function(i,m){
																				if(m.exist=='yes'){
																					...
																					member_fullname +" \n"+
																				 	"บัตร" + jQuery.trim(m.special_day)+" \n"+
...
//		    			                      .css({"padding":".1em .1em .1em 0","margin":"0 0 0 0","background-color":"#C7D9DC"} )
		 ...
		 $("#btnSwapCashier").click(function(e){
			    ...
			    var dialogOpts_swapCashier = {
						...
						showOpt: {direction: 'up'},		
						closeOnEscape:true,	
						title:"<span class='ui-icon ui-icon-person'></span>เปลี่ยนผู้เปิดบิล",
						open: function(){     						
							$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
							 $(this).dialog('widget')
					            .find('.ui-dialog-titlebar')
								...
								$("#vt_name").keypress( function(evt) {
							        ...
							        if(key == 13) {
							            ...
							            if(jQuery.trim($("#vt_name").val())==''){
							            	 jAlert('กรุณาระบุชื่อ','Warning message',function(){
									            	$("#vt_name").focus();
									            	return false;
										      });
								        }else{
								...
								"OK":function(){ 	
									...
									if(taxid_branch=='b'){
										...
										if(vt_name!='' && taxid_branch_seq==''){
											jAlert('PLEASE SPECIFY BRANCH NO.','Warning message',function(){
						            ...
						            if(csh_member_no.length>0 && vt_passport_no.length>0){
						            	 jAlert('ลูกค้าที่เป็นสมาชิกอยู่แล้ว ไม่สามารถคีย์ Passport ได้','Warning message',function(){		
						            		 $('#vt_passport_no').val('');
								             return false;
									      });
						            	 return false;
		 ...
		 $('#btnGreenBag').click(function(e){	    	
		    	...
		    	var net_amt=$('#csh_net').val().replace(/[^\d\.\-\ ]/g,'');
		 		...
		 		var promo_tp=$('#csh_promo_tp').val();
		 		if(status_no=='02'){
					jAlert('บิล 02 (ซื้อครั้งแลกส่วนลดพิเศษ) ไม่ร่วมโปรโมชั่น', 'Warning message',function(){	            
						$('#csh_product_id').focus();
		            	return false;
			        });
					return false;
				}else if(status_no=='04' && net_amt<1){
					jAlert('บิล 04 (สมาชิกแลกคะแนน) กรุณาสแกน Barcode ถุงที่จะใช้\nหลังจากสแกนรายการสินค้าที่ร่วมโปรโมชั่นแล้ว', 'Warning message',function(){	            
						$('#csh_product_id').focus();
		            	return false;
			        });
					return false;
				}else if(promo_tp=='NEWBIRTH' && net_amt<1){
					jAlert('สมาชิกใช้สิทธิวันเกิด (NEWBIRTH) กรุณาสแกน Barcode ถุงที่จะใช้\nหลังจากสแกนรายการสินค้าที่ร่วมโปรโมชั่นแล้ว', 'Warning message',function(){	            
						$('#csh_product_id').focus();
		            	return false;
			        });
					return false;
...
//		    	var onsales=onSales();
//			    if(onsales>0){
//				 	jAlert('คุณทำรายการขายค้างอยู่ ไม่สามารถเลือกโปรโมชั้นหลังทำรายการขายได้', 'Warning message',function(){	            	
//		            	return false;
//			        });
//				 	return false;
//				}
			    ...
			    $("#dlgGreenBag").dialog({
						   ...
						   modal:true,
						   resizable:false,
				           title: "กรุณาสแกน Barcode ถุงที่จะใช้",		
				           position:"center",
				            open:function(){		
				            	$(this).dialog('widget')
					            .find('.ui-dialog-titlebar')
			    			    ...
			    			    $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"padding":"5px 15px","margin-top":"0",
			    					"background-color":"#BCDCD7","font-size":"20px","color":"#666"});
							 		...
							 		if(g1!=''){
							 			...
							 			$.ajax({
							 				...
							 				data:{
							 					...
							 					if(data=='O'){
							 					 	jAlert('ไม่สามารถเล่นโปรนี้ได้เกิน 5 ครั้งต่อวัน', 'Warning message',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
							 					}else if(data=='N'){
							 					 	jAlert('ไม่พบรหัสถุงหรือรหัสถุงนี้ไม่ใช่ของสมาชิก', 'Warning message',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
							 					}else if(data=='R'){
							 						jAlert('พบรายการรหัสถุงแล้ว ไม่สามารถใช้มากกว่า 1 ถุงต่อบิลได้', 'Warning message',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
	    ...
	    $('#btnSmsPro').click(function(e){
	    			    		...
	    			    		data:{
	    			    			...
	    			    			$('.tableNavSmsPro ul>li').not('.nokeyboard').navigate({
									    ...
									    if(sel_promo.member_tp=='Y' && $.trim(csh_member_no).length==0){
									    	jAlert('กรุณาระบุรหัสสมาชิกก่อน','Warning message',function(){
												 return false;
											  });
									    	return false;
									    }
	    	...
	    	var opts_ctemp={
					...
					success:function(data){
						var arr_data=data.split('#');
						if(parseInt(arr_data[0])>0){		
							jAlert('CAN NOT PLAY PROMOTION BECAUSE IT FOUND ITEM.','Warning message',function(){
								 return false;
							  });
						}else{
							///////////////
	    ...
	    $('#btnDelRowSelect').live('click', {}, function(ev) {
	       ...
	       if(del_list==''){
	        	 jAlert('Please select the item you want to delete.','Warning message',function(){
	        		 $('#btnDelRowSelect').removeClass('ui-state-focus ui-state-access').addClass('ui-state-default');
					 return false;
				  });
		    }else{
	            ...
	            if(member_no.length>0){
	            	var opts_ontrans={
	            			...
	            			data:{
	            				...
	            				rmd:Math.random()
	            					...
	            					//---------------------------------------------------------------------
	            					if(mdata[0]=='Y'){
	            	            		jAlert("Members can not do more than one.","Warning message",function(){
	            			    			$("#csh_member_no").focus();
	            			            	return false;
	            				        });
	            	            	}else if(mdata[0]=='T'){
	            	            		...
	            	            		//satart old
	            	            		 if(csh_status_no=='01'){
		            	     		            //bill สมัครสมาชิกใหม่ 
		            	     		            checkMemberExist();
		            	     		        }else if(csh_status_no=='02'){
		            	     			        //bill ซื้อครั้งแรกหลังสมัครสมาชิก
		            	     		        	firstBuy();		            	     		        	
		            	     			    	...
		            	     			    	if(getOnlineStatus()===1){	
		            	     			    		...
		            	     			    		//callMemberOffline();
		            	     			    		jAlert("ไม่สามารถแลกคะแนนแบบ OFFLINE ได้","Warning message",function(){
	            	     				    			$("#csh_member_no").focus();
	            	     				            	return false;
	            	     					        });
	            	     				    		return false;
		            	     			        ...
		            	     			        //เปลี่ยนสินค้า ลดหนี้
		            	     			    	if($("#cn_member_no").val()!=''){
		            	     			    		if($("#csh_member_no").val()==''){
		            	     				    		jAlert("กรุณาระบุ รหัสสมาชิก","Warning message",function(){
		            	     				    			$("#csh_member_no").focus();
		            	     				            	return false;
		            	     					        });
		            	     				    		return false;
		            	     				    	}else if($("#csh_member_no").val()!=$("#cn_member_no").val()){
		            	     				    		jAlert("รหัสสมาชิกไม่ตรงกับรหัสมาชิกในบิลลดหนี้(CN)","Warning message",function(){
		            	     				    			$("#csh_member_no").focus();
		            	     				            	return false;
		            	     					        });
		            	     				    		return false;
	            	...
	            	if(csh_status_no=='02'){
	            		var key_employee=$.trim($("#csh_member_no").val());
			            	...
			            	var key_employee=$.trim($("#csh_member_no").val());
		     			   	if(key_employee.length==0){
						    		jAlert("กรุณาระบุ รหัสพนักงาน","Warning message",function(){
						    			$("#csh_member_no").focus();
						            	return false;
							        });
						    		return false;
			            	...
			            	var key_employee=$.trim($("#csh_member_no").val());
					    		...
					    		jAlert("กรุณาระบุ รหัสบัตรเสีย/บัตรหาย","Warning message",function(){
					    			$("#csh_member_no").focus();
					            	return false;
						        });
					    	
 			 	}else if(csh_status_no=='06'){
	 			 		 //เปลี่ยนสินค้า ลดหนี้ ป้องกันปัญหา event bubling on key enter 			 			
     			    	if($("#cn_member_no").val()!=''){
     			    		if($("#csh_member_no").val()==''){
     				    		jAlert("กรุณาระบุ รหัสสมาชิก","Warning message",function(){
     				    			$("#csh_member_no").focus();
		...
		$("#btnBrowsProduct").click(function(e){
			...
			var dialogOpts_brows_product = {
					...
					resizable:true,
					position:['center','center'],
					title:"<span class='ui-icon ui-icon-cart'></span>รายการสินค้า",
					...
					open: function(){     	
		    			    ...
		    			    $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"height":"50px","padding-top":"10","margin-top":"0","background-color":"#BCDCD7",
			    			    										"font-size":"27px","color":"#000"});		    			    
		    			    $(this).dialog("widget").find(".ui-dialog-buttonpane")
		    			                      .css({"padding":".1em .1em .1em 0","margin":"0 0 0 0","background-color":"#C7D9DC"});
		    			    // button style		
								...
								success:function(data){
									...
									$('.tableNavOtherPro ul li').not('.nokeyboard').navigate({
									  ...
									  //*WR02062016 for support mini bueaty book 2016
									    ...
									    /// *WR20120906  case promotion 50BTO1P ปกติจะเล่นกรณีหลังมีรายการขายสินค้าก่อนหน้าเท่านั้น
									    if(sel_promo.promo_code=='50BTO1P'){
									    	...
									    	if(mp_status_no=='02'){
									    		jAlert("โปรโมชั่น More Point ไม่สามารถเล่นร่วมกับบิล 02(First Purchase) ได้","Warning message",function(){	
									            	return false;
										        });
									    		return false;
									    	}
										...
										/////////////////////////////// Temp  play only one ////////////////////////////////////////
									    /*WR10012013 ทำแก้ปัญหาเฉพาะหน้าใช้ครั้งเดียว short product*/	
									    if(sel_promo.promo_code.substring(0,2)=='R_'){
									    	$('#csh_promo_code').val(sel_promo.promo_code);			
									    	$('#csh_trn_diary2_sl').val('Y');									    	
									    	var title_returns="";
									    	if(sel_promo.promo_code.substring(0,6)=='R_STOR'){
									    		title_returns="เลขที่บิล";
									    	}else{
									    		title_returns="เลขที่บิล(แนะนำใช้ปุ่มค้นหา)";
									    	}
													...
													open: function(){   
														...
														if(sel_promo.promo_code.substring(0,6)=='R_STOR'){
																$('#dlgDocNoTemp').append("<input type='text' id='doc_no_temp' size='15'></input>" +
																		"<button id='btnSubmitTemp' style='font-size:14px;'>ตกลง</button>");
														}else{
															$('#dlgDocNoTemp').append("<input type='text' id='doc_no_temp' size='15' readonly style='border:1pt solid#999;background:#ccc;'></input>" +
																	"&nbsp;<button id='btnBwsMemBill' style='font-size:14px;'>ค้นหา</button>" +
																	"<div style='text-align:left;'><button id='btnSubmitTemp' style='font-size:14px;'>ตกลง</button></div>"
																	);
														}
														...
														$('#btnBwsMemBill').click(function(e){
															...
															var opts_dlgbillProfiles={
																	...
																	position:'center',
																	closeOnEscape: true,
																	title:"<span class='ui-icon ui-icon-document-b'></span>รายการบิลสมาชิก",
																	open:function(){
																		$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
																		$(this).dialog('widget')
																            .find('.ui-dialog-titlebar')
																				       			...
																				       			$(this).click(function(){
																				       				...
																				       				if(strJson!=''){
																						       				...
																						       				if(seldocno.status_receive=='Y'){
																						       					jAlert("รับสินค้าแล้ว","Warning message",function(){																					
																													return false;
																										        });
																						       					return false;
																						       				}
																						       				if(seldocno.flag=='C'){
																						       					jAlert("เอกสารนี้ถูกยกเลิก","Warning message",function(){																					
																													return false;
																										        });
																							       				return false;
																						       				}
														...
														$('#btnSubmitTemp').click(function(e){
														    ...
														    if(onsales>0){
																...
																$('#dlgDocNoTemp').dialog('close');
															 	jAlert('ไม่สามารถคืนสินค้าซ้ำได้', 'Warning message',function(){															 		
																	getCshTemp('N');
																	paymentBill('00');
													            	return false;
														        });
															    	  ...
															    	  $("#key_quantity_return").keypress(function(evt){	
															    		  ...
															    		  if(key==13){
																    		  ...
																    		  if(key_quantity_return.length==0 || key_quantity_return=='0'){
															    				  jAlert("กรุณาคีย์จำนวนสินค้า","Warning message",function(){				
															    			  ...
															    			  if(key_quantity_return>limited_qty){
															    				  jAlert("ไม่สามารถคีย์จำนวนสินค้าได้มากกว่า " + limited_qty + " รายการ","Warning message",function(){				
															    					   $("#key_quantity_return").val('1').focus();
																						return false;
																			        });
															    				  return false;
															    	  ...
															    	  $("#key_product_return").keypress(function(evt){															    		 
															    		  ...
															    		  if(key==13){
															    			  ...
															    			  if(key_product_return.length==0){
															    				  jAlert("กรุณาคีย์รหัสสินค้า","Warning message",function(){				
															    					   $("#key_product_return").val('').focus();
																						return false;
																			        });
															    			  }
															...
															var opts_chk={
																	...
																	data:{
																		...
																		if(data=='1'){
																			//ไม่พบ
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบเลขที่เอกสารนี้ หรือเลขที่เอกสารนี้ไม่ร่วมโปรโมชั่น","Warning message",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='2'){
																			...
																			jAlert("รหัสบัตรสมาชิกไม่ตรงตามเลขที่เอกสารนี้","Warning message",function(){																						
																				return false;
																	        });
																			return false;
																		}else if(data=='3'){			
																			//รับไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("บิลนี้ไม่ได้ติดสินค้า short <br>ไม่สามารถคืนได้","Warning message",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='4'){
																			...
																			jAlert("ยอดซื้อน้อยกว่า 2000 ไม่สามารถรับโปรนี้ได้","Warning message",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='5'){	
																			...
																			jAlert("ยอดซื้อน้อยกว่า 3000 ไม่สามารถเล่นโปรนี้ได้","Warning message",function(){																						
																				return false;
																	        });
																			return false;
																		}else if(data=='6'){																				
																			...
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบโปรโมชั่นที่ร่วมโปร","Warning message",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='8'){			
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่ใช่บิล 01 บัตรใหม่หรือบัตรต่ออายุ","Warning message",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='9'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("bill ทีมีการตรวจสอบยอดซื้อ","Warning message",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='10'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบเลขที่เอกสาร หรือเลขที่เอกสารไม่อยู่ในช่วงเวลาที่กำหนด","Warning message",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='11'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบโปรในตาราง promo_short","Warning message",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='12'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("เลขที่เอกสารนี้ถูกยกเลิกแล้ว","Warning message",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='13'){																				
																			//บิลนี้ไม่เป็นไปตามเงื่อนไขโปรโมชั่น
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("บิลนี้ไม่เป็นไปตามเงื่อนไขโปรโมชั่น ไม่สามารถคืนสินค้าได้","Warning message",function(){																				
																				return false;
																	        });
																			return false;
																		}else{
											  		 ...
											  		 $.ajax({
											  			 ...
											  			 data:{
											  				 ...
											  				 if(data=='Y'){
											  					...
											  					jAlert("ไม่พบข้อมูลหรือลูกค้ารับสินค้าไปแล้ว", 'Warning message',function(){
											  						initFieldOpenBill();
											  		            	return false;
											  			        });
											  				 }
	    ...
	    $("#btnDocType").click(function(evt){
		    ...
		    var onsales=onSales();
		    if(onsales>0){
			 	jAlert('You make a pending transaction. Document type can not be changed.', 'Warning message',function(){
	            	$("#csh_product_id").focus();
	            	return false;
		        });
			}else if(onsales==0){			
		...
		$(".ui-accordion .ui-accordion-content").css({
			'height':hacc,
			'background':'#619387 url(/sales/img/op/bg_acor_content.png) repeat-x bottom left',

...
//        							});//keypress
		...
		if(!ajaxShipping) {
			...
			$.ajax({
				...
				success:function(data){
					...
					alert(member_type+" :: "+$('#csh_net').val());
					if(member_type=='WALK IN' && $('#csh_net').val()>0){
						billmemberregister();
					}else{
	...
	function billmemberregister(){
		alert("billmemberregister");
...
//	    		    			  alert("index of a is " + a + "\n and length of a is " + b);
		...
		$('#dlgSmsMobile').html('');
		var url='/sales/member/billmemberregister';
		  $.ajax({
			type:'post',
		    url:url,
		    data:{

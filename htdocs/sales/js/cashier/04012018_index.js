				    	            ...
				    	            { 
				    	                ...
				    	                click: function(evt){ 
								    		...
								    		if(product_id.length<1){
							    				jAlert('กรุณาสแกนรหัสสินค้า', 'WARNING!',function(){
						...
						if(buy_type=='N' ){
							...
							var start_baht=start_baht.replace(/[^\d\.\-\ ]/g,'');
							 ...
							 if(net_amt<start_baht){
								 jAlert("ยอดเงินสุทธิอย่างน้อยต้อง " + start_baht + " บาท", 'WARNING!',function(){																						
									 $('#pdt_otherpro').val('').focus();
										return false;
								});
								 return false;																		    										 
							  										    	            ...
							  										    	            { 
							  										    	                ...
							  										    	                click: function(evt){ 
							  										    	                	evt.preventDefault();
							  															 		...
							  															 		product_id=$.trim(product_id);																		 			
							  														    		if(product_id.length<1){
							  													    				jAlert('กรุณาสแกนรหัสสินค้าฟรีที่ร่วมรายการโปรโมชั่น', 'WARNING!',function(){
							  											...
							  											if(promo_code=='OK01160115'){
							  												//if(buy_type=='N' ){
							  													...
							  													var start_baht=start_baht.replace(/[^\d\.\-\ ]/g,'');
							  													 ...
							  													 if(net_amt>end_baht){
							  														 jAlert("ยอดเงินสุทธิต้องไม่เกิน " + end_baht + " บาท กรุณาทำรายการใหม่", 'WARNING!',function(){																						
							  															$('#pdt_detail_id').val('').focus();
							  															return false;
							  														});
							  														 return false;																		    										 
					        ...
					        if(key == 13){
					            ...
					            //*WR 12052015
					           	...
					           	if(chk_member_th_old!='1120'){
					           		jAlert("กรุณาระบุรหัสบัตรเก่าเป็นบัตรวันพฤหัสบดีเท่านั้น", "WARNING!",function(){       									
       									$('#old_card_thu').focus();
       									return false;
       								});
					           		return false;
					           	...
					           	var objReqChkCardToChange=$.getJSON(
					           						...
					           						function(data){							
					           							...
					           							if(data=='2'){
					           								jAlert('ไม่พบรหัสสมาชิกนี้กรุณาตรวจสอบอีกครั้ง', 'WARNING!',function(){
					           									initFieldOpenBill();
					           									return false;
					           								});
					           							}else{
					           								$.each(data.member, function(i,m){
					           									if(m.exist=='yes'){					           										
					           										if(m.age_card!='4'){
					           											jAlert('ต้องเป็นสมาชิก 4 ปีเท่านั้น', 'WARNING!',function(){
								           									$('#old_card_thu').focus();
								           									return false;
								           								});
					           											return false;
					           								...
					           								//------------------ zone alert --------------------
						           								...
						           								if(m.forgot_card=='Y'){
					           										jAlert('รหัสนี้ ถูกแจ้งลืม ในระบบแล้ว<br>สอบถามรหัสรับบัตรคืน(SMS) จากลูกค้า<br>หรือติดต่อ Admin', 'WARNING!',function(){
					           											initFieldOpenBill();
					           											return false;
					           										});	
					           										return false;	
					           									...
					           									if(m.expire_status=='Y'){
					           										jAlert('รหัสสมาชิกนี้บัตรหมดอายุกรุณาตรวจสอบอีกครั้ง', 'WARNING!',function(){
					           											initFieldOpenBill();
					           											return false;
					           										});	
					           										return false;				
...
//									}
	...
	function chkCnMember(){
		...
		if(member_no!=cn_member_no){
			jAlert('รหัสสมาชิกไม่ตรงกับรหัสสมาชิกในเอกสารลดหนี้ (CN) <br>Please check and try again.','WARNING!',function(b){
					 return false;
	   		 });
		}else{
			...
			success:function(data){
				...
				if(obj_json.status_ck=='Y'){
					...
					if(obj_json.month_enable=='Y' && obj_json.outofday=='Y' && obj_json.outtime=='Y'){
						...
						jAlert('เดือนนี้ไม่สามารถใช้สิทธิได้','WARNING!',function(b){
							initTblTemp();
					        initFormCashier();
							initFieldOpenBill();
		    				 return false;
	...
	function chkEmpPrivilage(){
		...
		*/
		...
		if(employee_id.length==0){
			jAlert('กรุณาระบุรหัสพนักงาน','WARNING!',function(b){
				$("#csh_member_no").focus();
				 return false;
    		 });		
	   		 return false;	
				...
				success:function(data){
					if(data!=''){
						...
						if(ecp.error_opsday=='Y'){
							jAlert('พนักงานไม่สามารถใช้สิทธิในวัน OPS Day ได้','WARNING!',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.error_cam=='Y'){
							jAlert('กรุณา Scan บัตรพนักงาน หรือถ่ายรูปขอสิทธิก่อน','WARNING!',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.error_empty=='Y'){
							jAlert('พนักงานท่านนี้ใช้สิทธิไม่ได้ กรุณาพยามอีกครั้งหรือติดต่อ HR','WARNING!',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.is_used=='Y'){							
							jAlert('พนักงานท่านนี้ใช้สิทธิไปแล้ว','WARNING!',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						...
						jAlert('ไม่สามารถใช้สิทธิได้ Please check and try again.','WARNING!',function(b){
							$("#csh_member_no").focus();
		    				 return false;
			    		 });
					}
		 ...
		 jConfirm('DO YOU WANT TO DELETE ITEM NO :\r\n '+del_list_show,'Confirm message', function(r){
		        if(r){
					var opts={
							   ...
							   success: function(data){
								    if(data=='LOCKDEL'){
								    	 jAlert('ไม่สามารถลบรายการขณะเล่นโปรนี้ได้ ท่านสามารถเลือกยกเลิกรายการขายแทน','WARNING!',function(b){
								    		 initFieldPromt();
						    				 return false;
							    		 });
		...
		if(flgstop=='Y'){
			...
			if($('#status_couponpromo').val()=='Y'){
				...
				if(csh_net > 0 && buy_status=='G' && csh_net < start_baht ){
					jAlert('ยอดซื้อน้อยกว่า ' + start_baht + ' ไม่สามารถชำระเงินได้ Please check and try again.','WARNING!',function(){
	    				           ...
	    				           position:"center",
	    				            ...
	    				            buttons: {
									 	"OK":function(evt){
										 	...
										 	if(chk_keyin == 0){
										 			jAlert('กรุณาคีย์บาร์โคดให้ครบ','WARNING!',function(b){			
	...
	function saveTransaction(){
		...
		if($.trim(status_no)=='01'){
			...
			if(member_no.length==0 && $('#csh_application_type').val()=="NEW" && $('#csh_card_type').val()=="MBC"){
				jAlert('ไม่พบรหัสบัตรใหม่ กรุณายกเลิกเอกสารแล้วทำรายการใหม่','WARNING!',function(){			
					return false;
				});		
				return false;
			}else if(refer_member_id.length==0 && $('#csh_application_type').val()=="ALL"){
				jAlert('ไม่พบรหัสบัตรเก่าที่ใช้อ้างอิง กรุณายกเลิกเอกสารแล้วทำรายการใหม่','WARNING!',function(){			
					return false;
				});	
				return false;
			}//end if			
						        ...
						        if(key == 13) {
						            ...
						            var opts={
							            	...
							            	success:function(data){
								            	var arr_data=data.split('#');
												if($.trim(arr_data[0])==""){
													jAlert('ไม่พบรหัสผู้ขาย Please check and try again.','WARNING!',function(){
														$("#chk_saleman_id").val('').focus();
														return false;
									    			});
												}else if($.trim(arr_data[3])=='P'){
													jAlert('ขณะนี้พนักงานขายไม่อยู่ในระบบ Please check and try again.','WARNING!',function(){
														$("#chk_saleman_id").val('').focus();
														return false;
									    			});
												}else if($.trim(arr_data[3])=='N'){
													jAlert('ขณะนี้พนักงานขายไม่ได้ลงเวลาเข้าระบบ Please check and try again.','WARNING!',function(){
														$("#chk_saleman_id").val('').focus();
														return false;
									    			});
												}else{
    						                ...
    						                success:function(data){
    						                        ...
    						                        if(arr_data[0]=='Y'){
    						                        	...
    						                        	if(cmp_member_no==confirm_member_no){
        		    						            	...
        		    						            	jAlert("รหัสสมาชิกที่ยืนยันไม่ตรงกัน Please check and try again.", 'WARNING!',function(){
        							            				$('#confirm_member_no').focus();
        			           									return false;
        			           								});
        		    						            	return false;
    						                        	...
    						                        	jAlert("Code หมดเวลากรุณาขอ Code อีกครั้ง","WARNING!",function(){	                                       
    				                                        $("#confirm_member_no").val('');
    				                                        $("#confirm_member_no").focus();
    				                                        return false;
    				                                    });
...
//    						            }
	...
	function callEDC(credit_net_amt){
		...
		if(parseFloat(credit_net_amt)<1){
			jAlert('กรุณาระบุจำนวนเงินชำระบัตรเครดิต','WARNING!',function(){
				$("#txt_credit_value").focus();
				return false;
			});
		}else if(parseFloat(credit_net_amt)>curr_total_payment){
			jAlert('จำนวนเงินชำระบัตรเครดิตมากกว่ายอดสุทธิที่ต้องชำระ','WARNING!',function(){
				$("#txt_credit_value").focus();
				return false;
			});
		}else{
			var opts_edc={
					...
					data:{
						...
						//////////////////////\\\\\alert\\\\\/////////////////////////
						jConfirm('ตรวจสอบยอดชำระเงินบัตรเครดิต','WARNING!', function(r){
					        if(r){
			...
			jAlert('กรุณารูดหรือเสียบบัตรเครดิต','WARNING!',function(r){
				$('#popup_ok').focus();
				if(r){
					$.ajax(opts_edc);
				}			
											  ...
											  return true;
											    ...
											    if (key == 13) {
		        									...
		        									if(credit_no.length<4 || credit_no.length>16){
		        										jAlert('กรุณาระบุเลขที่บัตรเครดิตให้ถูกต้อง','WARNING!',function(r){
		        											if(r){
		        												$("#txt_credit_no_value").focus();
		        												return false;
		        											}	
				 	...
				 	"ตกลง":function(evt){
				 		...
				 		if(status_no=='01' && application_id=='OPKTC300'){				 			
				 			...
				 			if(credit_no_value.length<6 ){
					 			jAlert('กรุณาระบุเลขที่บัตรเครดิตด้านหน้า 6 หลักเฉพาะชุดสมัคร OPKTC300','WARNING!',function(r){
									if(r){
										$("#txt_credit_no_value").focus();
										return false;
									}	
		            	...
		            	return false;
					 	...
					 	"OK":function(evt){
	            			...
	            			if($chk_over_bag_qty=='Y'){
	            				jAlert('กรุณาระบุจำนวนไม่เกิน 99 ','WARNING!',function(){
									return false;
				    			});
	            				return false;
	            			}
	            			...
	            			if( $chk_none_item=='Y'){
	            				jAlert('กรุณาระบุจำนวนถุงอย่างน้อย 1 รายการ (ถ้าไม่สะดวกให้คีย์ 0 ถุง) ','WARNING!',function(){
									return false;
				    			});
	            				return false;
	            			}
						...
						function(){
						    ...
						    $("#btn_payment_confirm").click( function(e){
								...
								$.ajax({
									...
									data:{
										...
										if(data=='Y'){												
											if($('#payment_channel').val()!="Alipay"){
												jAlert('โปรโมชั่นนี้(OX07140317)ต้องชำระเงินผ่าน Alipay เท่านั้น','WARNING!',function(){														
													return false;
								    			});
												return false;
											}else{
							...
							$("#btn_payment_alipay").click( function(evt){
								...
								if(member_type!=='WALK IN'){
									jAlert('การชำระเงินผ่าน Alipay ใช้สำหรับลูกค้า WALK IN เท่านั้น', 'WARNING!',function(){									    	
								    	return false;
								    });	
								    return false;
								}else{
									    	                	...
									    	                	evt.preventDefault();
														 		...
														 		$.ajax({
														 			...
														 			data:{
														 				...
														 				if(data=='Y'){
														 					jAlert('เลขที่การเปิดบิลมือซ้ำไม่สามารถบันทึกได้ Please check and try again.', 'WARNING!',function(){
	...
	function cancelBill(){
		...
		*/
		var opts={
					...
					success:function(data){
						if(data=='0'){
							jAlert('ไม่สามารถยกเลิกรายการได้ Please check and try again.', 'WARNING!',function(){
								return false;
							});	
						}else{						
	...
	function getProduct(){
		...
		*/	
		...
		if(product_id.length==0){
			jAlert('กรุณาระบุ รหัสสินค้า', 'WARNING!',function(){
				return false;
			});	
				...
				success:function(data){		
					if(data==1){
						...
						if(pdt_lost_id=='23983'){
							...
							jAlert('รหัสสินค้านี้สต๊อกไม่พอ Please check and try again.', 'WARNING!',function(){			
								if($('#csh_gap_promotion').val()=='Y'){								
									//*WR22072014
									if($('#csh_promo_code').val()=='50BTO1P'){
										endOfProOther();//uncomment on 05012013
						...
						jAlert('รหัสสินค้านี้ถูกล๊อก Please check and try again.', 'WARNING!',function(){	
							if($('#csh_gap_promotion').val()=='Y'){
								//endOfProOther();								
							}
							initFieldPromt();		
						...
						jAlert('สินค้า Tester ห้ามขาย Please check and try again.', 'WARNING!',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
					}else if(data==5){
						jAlert('สินค้า ไม่ร่วมโปรโมชั่น Please check and try again.', 'WARNING!',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
					}else if(data==6){
						jAlert('ไม่สามารถซื้อสินค้าเกินกว่าจำนวนที่กำหนดได้  Please check and try again.', 'WARNING!',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
										...
										var opts_chk_first_lmt={
													...
													data:{
														...
														if(data=='Y'){
															...
															jAlert('ยอดซื้อต้องไม่เกิน ' + parseFloat(csh_first_limited).toFixed(2) + ' Please check and try again.', 'WARNING!',function(){						
																initFieldPromt();		
																return false;							
															});	
														}
		...
		*/
		...
		var opts={
				...
				success:function(data){
					var arr_data=data.split('#');	
					if(arr_data[0]=='0'){
						jAlert('ไม่สามารถทำรายการสินค้านี้ได้ Please check and try again.', 'WARNING!',function(){
							initFieldPromt();
							return false;
						});	
					}else{		
	...
	function setSmsProduct(sms_promo_code){
		...
		var opts_smsproduct={
				...
				data:{
					...
					var arr_data=data.split('#');
					if(arr_data[0]=='0'){
						jAlert('ไม่สามารถบันทึกรายการได้ Please check and try again.', 'WARNING!',function(){							
							return false;
						});	
					}else{
	...
	function callSmsPromo(sms_promo_code,sms_mobile,redeem_code,member_tp,idcard){
		...
		var opts_smspromo={
				...
				data:{
					...
					if(data=='null'){
						jAlert("ไม่พบข้อมูล Please check and try again.", 'WARNING!',function(){
						...
						$('#id_redeem_code').val(redeem_code);
						 //*WR23012013						
						  if(prefix_member_no_idcard!='ID' && member_tp=='Y'  && objJson.member!=$('#csh_member_no').val()){
							  jAlert("รหัสบัตรสมาชิกไม่ตรง Please check and try again.", 'WARNING!',function(){
			    				   $("#sms_mobile").focus();
									return false;
								});	
							  return false;
...
//	    		    			  alert("index of a is " + a + "\n and length of a is " + b);
	    		    			  if((arr_tp.indexOf("T") != -1) && (sms_mobile.length!=10)){
	    		    				  jAlert('กรุณาระบุเบอร์โทรศัพท์จำนวน 10 หลัก', 'WARNING!',function(){
					    				   $("#sms_mobile").focus();
											return false;
										});	    				  
	    		    				  return false;
									    	   ...
									    	   data:{
									    		  ...
									    		  var arr_data=data.split('#');
									    		   if(arr_data[0]=='1'){
									    			   ...
									    			   jAlert('สินค้าไม่ร่วมโปรโมชั่น Please check and try again.', 'WARNING!',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='1'){
									    			   jAlert('ไม่พบสินค้าในทะเบียน Please check and try again.', 'WARNING!',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='2'){
									    			   jAlert('ไม่พบสินค้าในสต๊อก Please check and try again.', 'WARNING!',function(){
									    				   $("#co_product_id").select().focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='3'){
									    			   jAlert('สินค้าอยู่ในสถานะ Lock Please check and try again.', 'WARNING!',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='4'){
									    			   jAlert('ราคาสินค้าร่วมโปรโมชั่นมากว่าสินค้าปกติ Please check and try again.', 'WARNING!',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }
		...
		var opts={
				...
				success:function(data){					
					...
					if(arr_data[0]=='0'){
						...
						jAlert('ไม่สามารถบันทึกสินค้าซ้ำได้ หรือ กด ESC Key เพื่อออกจากโปรโมชั่นก่อนคีย์สินค้าทั่วไป', 'WARNING!',function(){
							initFieldPromt();
							return false;
						});	
					}else{						
	...
	function setCshValTemp(){
		...
		*/
		...
		if(quantity.length==0){
			jAlert('กรุณาระบุ จำนวนสินค้า', 'WARNING!',function(){
				$("#csh_quantity").focus();
				return false;
			});	
			return false;
		...
		if(member_percent1 != '' && member_no==''){
			jAlert('ไม่ใช่สมาชิกไม่ได้รับส่วนลด Please cancel the sale and re-open the bill.', 'WARNING!',function(){
				$("#csh_product_id").focus();
				return false;
			});	
			return false;
			...
			var opts_vip={
						...
						success:function(data){
							if(data=='Y'){
								...
								jAlert('ยอดซื้อมากกว่าวงเงินคงเหลือ '+addCommas(vip_balance)+' บาท','WARNING!',function(){
									initFieldPromt();
									return false;
								});	
								return false;
							    ...
							    $('#dlgSelCoPromotion').empty().html(data);	
	    			    		 $('.tableNavSelCopromotion ul li').not('.nokeyboard').navigate({
								    ...
								    if(promo_play=='OX07320814' || promo_play=='OX07561014'){
								    	...
								    	if(chk_oppn != 'OPPN300'){
								    		jAlert('เฉพาะชุดสมัครใหม่ OPPN300 เท่านั้น','WARNING!',function(){
												return false;
											});	
								    		return false;
								    	}
											    	            ...
											    	            { 
											    	                ...
											    	                click: function(evt){ 
											    	                	evt.preventDefault();
																 		...
																 		product_id=$.trim(product_id);																		 			
															    		if(product_id.length<1){
														    				jAlert('กรุณาสแกนรหัสสินค้าที่ร่วมรายการโปรโมชั่น', 'WARNING!',function(){
		...
		if(member_no.length==0){
							...
							$.each(data.member, function(i,m){
								if(m.exist=='yes'){
									...
									if(status_no=='04' && m.link_status=='OFFLINE'){										
										jAlert("ไม่พบการตอบสนองจาก Server ไม่สามารถแลกคะแนน(OFFLINE)ได้","WARNING!",function(){
											initFieldOpenBill();
    	     				            	return false;
    	     					        });
    	     				    		return false;										
			...
			setTimeout(function(){
				if(opts_redeempoint){
					...
					jAlert('ไม่พบการตอบสนองจาก Server กรุณาลองใหม่อีกครั้ง','WARNING!',function(){
						initFieldOpenBill();
						return false;
					});	
				}
						    ...
						    evt.preventDefault();		
						       ...
						       var $csh_newcard=$("#csh_new_card");
								...
								if($csh_newcard_val.length==0){
									jAlert('กรุณาระบุรหัสบัตรสมาชิก', 'WARNING!',function(){
										$csh_newcard.focus();
										return false;
									});	
								}else{
									...
									var opts_member={
											...
											success:function(data){
												if(data=='1'){
													jAlert('รหัสบัตรนี้ถูกใช้สมัครสมาชิกแล้ว Please check and try again.', 'WARNING!',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='2'){
													jAlert('ไม่พบรหัสบัตรนี้ หรือรหัสนี้ถูกใช้สมัครแล้ว Please check and try again.', 'WARNING!',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='3'){
													jAlert('กรุณาเลือก ' + msg_show, 'WARNING!',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='4'){
													jAlert('กรุณาเลือกบัตรวันพฤหัสบดี', 'WARNING!',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='5'){
													jAlert('กรุณาสแกนบัตร ' + msg_show_sel, 'WARNING!',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else if(data=='6'){
													jAlert('กรุณาเปลี่ยนคีย์บอดเป็นภาษาอังกฤษ', 'WARNING!',function(){
														$csh_newcard.focus();
														return false;
													});	
												}else{
										...
										if($input_giftset_quantity_val.length==0){
											...
											var opts={
													...
													success:function(data){	
														var arr_data=data.split('#');
														if(arr_data[0]=='3'){
															jAlert('สินค้านี้ถูก lock ไม่สารมารถทำรายการได้ Please check and try again.', 'WARNING!',function(){
																$input_giftset_product.val('').focus();
																return false;
															});	
														}else if(arr_data[0]=='2'){
															jAlert('จำนวนสินค้าไม่พอที่จะทำรายการ Please check and try again.', 'WARNING!',function(){
																$input_giftset_product.val('').focus();
																return false;
															});	
														}else if(arr_data[0]=='0'){
				...
				close: function(evt,ui) {
					...
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){
						if($csh_net_val<$gift_set_amount_val){
							jAlert("ยอดซื้อน้อยกว่า " + parseFloat($("#csh_gift_set_amount").val()).toFixed(2) + " Please check and try again.","WARNING!",function(){
					    	...
					    	var key = evt.keyCode ? evt.keyCode : evt.which ? evt.which : evt.charCode; 
		   			        if(key == 13){	
					    		...
					    		if(keyin_mstquantity_id.length<1){
				    				jAlert('กรุณาระบุจำนวนสินค้า', 'WARNING!',function(){
														...
														if(application_id=='OPPC300'){				
																	...
																	data:{
																		...
																		rnd:Math.random()
																			...
																			if(price_check > parseFloat($free_product_amount.val())){
																				jAlert("ยอดซื้อมากกว่า " + parseFloat($free_product_amount.val()).toFixed(2) + " Please check and try again.","WARNING!",function(){
																					window.parent.$(".ui-dialog").remove();
																...
																$.ajax({
																	...
																	data:{
																		...
																		if(data=='N'){
																			jAlert("กรุณาเลือกสินค้าในกลุ่ม Body/Hair ","WARNING!",function(){
																				$("#input_free_product").val('').focus();
																				return false;
																			});	
																			return false;
				...
				close: function(evt,ui) {
					...
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){
						if($csh_net_val<parseFloat($free_product_amount.val())){
							jAlert("ยอดซื้อน้อยกว่า " + parseFloat($free_product_amount.val()).toFixed(2) + " Please check and try again.","WARNING!",function(){
								window.parent.$(".ui-dialog").remove();
								$('#dlgFreeProduct').dialog('open');
								$("#input_free_product").focus();
								return false;
								...
								var opts_freepremium={
											...
											success:function(data){
												...
												if(c_free_premium_amount>parseFloat($free_premium_amount.val())){
													c_free_premium_amount=parseFloat(c_free_premium_amount)-parseFloat(arr_fp[1]);
													jAlert('มูลค่าสินค้ามากกว่าจำนวนเงินสินค้าพิเศษ Please check and try again.', 'WARNING!',function(){
												...
												if(arr_fp[0]=='4'){
													jAlert('สินค้ารหัสนี้ไม่ร่วมโปรโมชั่น Please check and try again.', 'WARNING!',function(){
														$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
												if(arr_fp[0]=='1'){
													jAlert('ไม่พบข้อมูลรหัสสินค้านี้ในทะเบียน Please check and try again.', 'WARNING!',function(){
								           				$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
												if(arr_fp[0]=='2'){
													jAlert('สินค้าสต๊อกไม่พอ Please check and try again.', 'WARNING!',function(){
								           				$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
													if(arr_fp[0]=='3'){
														jAlert('สินค้าถูกล๊อก Please check and try again.', 'WARNING!',function(){
									           				$input_free_premium.focus().select();
									        				return false;
									        			});	
												}else if(arr_fp[0]=='5'){
						...
						success:function(data){
							...
							$('.tableNavCatList ul li').not('.nokeyboard').navigate({
							  ...
							  //*WR05022014 OPPN300 Revised
								...
								if(chk_ops=='Y'){
									jAlert('ลูกค้าถือบัตร ' + arr_data[9] + ' อยู่ไม่สามาถถือบัตรซ้ำได้', 'WARNING!',function(){				           				
				        				return false;
				        			});	
									return false;
								}
								...
								if(arr_data[1]!=''){
										var opts={
												...
												success:function(data){	
													...
													if(arr_res[0]=='2'){
														jAlert('จำนวนสินค้าไม่พอ Please check and try again.', 'WARNING!',function(){															
															//TEST ON 13052012
															$('div.tableNavCatList ul>li').navigate('destroy');
															$("#dlgMemberCatalogList").dialog("close");
...
//																	}
										...
										if(status_no!='01'){
											$("#dlgMemberCatalogList").dialog("close");
											jAlert('เกิดปัญหาการตรวจสอบสถานะบิล \nกรุณายกเลิกการขาย แล้วเลือกบิล 01 อีกครั้ง', 'WARNING!',function(){
												initTblTemp();
												initFormCashier();
												getCshTemp('N');
												browsDocStatus();
	        ...
	        NewDialog.dialog({
	            ...
	            buttons: [
	                {text: "ต่อไป >>", click: function(e) {
	                	 ...
	                	 var chk_idcard_exist=$('#chk_idcard_exist').val();
			             if(chk_idcard_exist.length==0){
			            	 jAlert('กรุณาระบุรหัสบัตรประชาชน', 'WARNING!',function(){
			    	                ...
			    	                click: function(evt){ 
			    	                	 ...
			    	                	 var member_no_by_comp=$('#check_member_no_by_comp').val();
			 				             if(member_no_by_comp.length==0){
			 				            	jAlert('กรุณาระบุรหัสสมาชิกเพื่อตรวจสอบ', 'WARNING!',function(){
			 				            		$('#check_member_no_by_comp').focus();	
												return false;
											});	
			 				            	return false;
									...
									$('div.tableNavMemberPromotion2 ul>li').not('.nokeyboard').navigate({
									  ...
									  //*WR06122013 New OPPC300 2014
									    ...
									    if(selpromo.application_id=='OPPC300' && mp_point_sum < 30){
									    	 jAlert('คะแนนไม่พอ Please check and try again.', 'WARNING!',function(){
									    		 $("#dlgRegPromo").dialog("close");
									    		 initTblTemp();
											     initFormCashier();
											     initFieldOpenBill();
				            ...
				            if($refer_member_st_val.length==0){
					            ...
					            if(getOnlineStatus()===1){
					           	...
					           	var opts_memexpire={
						           		...
						           		success:function(data){						           			
							           		if(data==''){
							           			jAlert('ไม่พบรหัสบัตรสมาชิกเก่า Please check and try again.', 'WARNING!',function(){
							           				initMemberCatalog();//*WR25092012
							           				$refer_member_st.val('').focus();
							        				return false;
							        			});	
									           	...
									           	/*######################################*/
													...
													if(m.link_status=='ONLINE'){
														...
														if(m.exist_status=='NO'){
															jAlert('ไม่พบข้อมูลรหัสสมาชิก Please check and try again.', 'WARNING!',function(){
																initFormCashier();
																initFieldOpenBill();
																return false;
															});	
											           	...
											           	var opts={
												           		...
												           		success:function(data){												           			
													           		...
													           		if(arr_app[0]=='2'){														           			
													           			...
													           			jAlert('ไม่สามารถต่อชุดสมัครหลังเดือนหมดอายุได้ Please check and try again.', 'WARNING!',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='4'){
													           			jAlert('ไม่สามารถต่อชุดสมัครได้ Please check and try again.', 'WARNING!',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='5'){
													           			jAlert('ยอดเงินน้อยกว่า '+addCommas(arr_app[2])+' ไม่สามารถต่อชุดสมัครได้ Please check and try again.', 'WARNING!',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='6'){														           			
													           			jAlert('ไม่พบรายการชุดสมัคร Please check and try again.', 'WARNING!',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='7'){														           			
													           			jAlert('คะแนนสะสมน้อยกว่า '+arr_cat[8]+' Please check and try again.', 'WARNING!',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
													           			...
													           			jAlert('ยอดเงินเท่ากับหรือมากกว่า '+addCommas(arr_app[2])+' ไม่สามารถต่อชุดสมัครได้ Please check and try again.', 'WARNING!',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='10'){
													           			jAlert('สามารถต่อก่อนเดือนหมดอายุ 1 เดือนหรือภายในเดือนหมดอายุเท่านั้น Please check and try again.', 'WARNING!',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
		...
		*/
		...
		if(member_id.length==0){
			...
			$.getJSON(
					...
					function(data){
						if(data==1){
							jAlert('ไม่พบรหัสสมาชิกในทะเบียน หรือวันที่สมัครไม่ตรงกับวันที่เปิดบิล','WARNING!',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
							...
							jAlert('รหัสสมาชิกเคยใช้สิทธิแล้ว ไม่สามารถใช้สิทธิซ้ำได้','WARNING!',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
							...
							jAlert('เกิดผิดพลาด ไม่สามารถใช้สิทธิซ้ำได้','WARNING!',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
	...
	function cardLostStock(){
		...
		*/
		...
		var opts={
					...
					success:function(data){
						if(data=='0'){
							jAlert('รหัสบัตรนี้ไม่พบในทะเบียน หรือต้องเป็นบัตรเสีย-บัตรหายเท่านั้น','WARNING!',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='2'){
							jAlert('รหัสสมาชิกนี้ถูกใช้สมัครแล้ว','WARNING!',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='3'){
							jAlert('กรุณาเลือกบัตร ' + msg_show,'WARNING!',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='4'){
							jAlert('รหัสบัตรนี้ไม่พบในทะเบียน หรือท่านยังไม่ได้ลงทะเบียนบัตร','WARNING!',function(){
								initFieldOpenBill();
								return false;
							});	
						}else{
		...
		var opts={
					...
					success:function(data){
						if(data=='0'){
							...
							jAlert('รหัสบัตรนี้สินค้าคงเหลือไม่พอ Please check and try again.','WARNING!',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='6'){
							jAlert('กรุณาเลือกบัตร OPS White Card','WARNING!',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='7'){
							jAlert('กรุณาเลือกบัตร OPS Gold Card','WARNING!',function(){
								initFieldOpenBill();
								return false;
							});	
						}else{
	...
	function getMemberProfile(col,txtsearch){
		...
		*/
		 $.getJSON(
					...
					function(data){
						if(data=='0'){
							jAlert('ไม่พบข้อมูลสมาชิก', 'WARNING!',function(){
					    ...
					    if(selpromo.promo_code=='NEWBIRTH'){
						    ...
						    $.ajax({
							    ...
							    data:{
								    ...
								    if(data=='0'){
								    	jAlert('ไม่พบข้อมูลโปรโมชั่น Please check and try again.', 'WARNING!',function(){
								    		$("#other_promotion_cmd").html('');
									    	$('.tableNavMemberPriv ul').navigate('destroy');
...
//									}
	...
	function stockLost(){
		...
		*/
		...
		if(employee_id.length==0){
			jAlert('กรุณาป้อนรหัสพนักงาน', 'WARNING!',function(){
				initFieldOpenBill();
				return false;
			});	
			return false;
			...
			//check employee info
			 var opts={
		            	...
		            	success:function(data){
			            	var arr_data=data.split('#');
							if($.trim(arr_data[0])==""){
								jAlert('ไม่พบรหัสพนักงาน Please check and try again.','WARNING!',function(){
									initFieldOpenBill();
									return false;
				    			});
							}else if(data!=""){
						   			        ...
						   			        if(key == 13){
							   			        $.ajax({
										            	...
										            	success:function(data){
										            		var arr_data=data.split('#');
															if($.trim(arr_data[0])==""){
																jAlert('ไม่พบข้อมูล Audit Please check and try again.','WARNING!',function(){
																	$("#audit_password").focus();
																	return false;
												    			});
															}else if(data!=""){
	...
	function swapCashier(swap_cashier_id){
		...
		*/
		$.getJSON(
				...
				function(data){
					if(data==null){
						jAlert('ไม่พบรหัสนี้กรุณาตรวจสอบอีกครั้ง', 'WARNING!',function(){
							return false;
						});	
					}else{
						$.each(data.emp, function(i,m){
							if(m.exist=='yes'){
								if(m.check_in_status=='Y'){
									...
									jAlert('รหัสนี้ไม่อยู่ในสถานะ Login ระบบ', 'WARNING!',function(){
										return false;
									});	
								}
							}							
							...
							if($('#status_couponpromo').val()=='Y'){
								...
								if(csh_net > 0 && buy_status=='G' && csh_net < start_baht ){
									...
									if($('#csh_promo_amt_type').val()=='G' && csh_sum_amount < csh_promo_amt){											
										jAlert('ยอดซื้อน้อยกว่า ' + $('#csh_promo_amt').val() + ' ไม่สามารถชำระเงินได้ Please check and try again.','WARNING!',function(){
											$("#btn_cal_promotion").removeAttr("disabled");
											$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
											return false;
										});
...
//		$("#csh_product_id").numericFilter();
		...
		$("#btn_cal_promotion").click(function(e){
			...
			if(chk_percent_discount.length>0 && chk_member_no.length==0){
				jAlert('ไม่พบรหัสสมาชิก กรุณาป้อนรหัสสมาชิก Please check and try again.','WARNING!',function(){					
					return false;
    			});
				return false;
			}			
			...
			if(jQuery.trim($("#csh_status_no").val())=='01'){
			   						...
			   						$('#ecp_amount').keypress(function(evt){
					   			        ...
					   			        if(key == 13){
						   			        ...
						   			        if(parseFloat(chk_ecp_amount_bal)>parseFloat(ecoupon_amount_balance)){
						   			        	jAlert('ไม่สามารถใช้สิทธิมากกว่าวงเงินที่เหลือได้ Please check and try again.','WARNING!',function(){
						   			        		$('#ecp_amount').val(ecp_amount_bal).focus();
						   							return false;
						   		    			});
							   			        return false;
				...
				if($promo_tp.val()=='N'){
					...
					if($csh_net<$start_baht){
						jAlert('ยอดเงินสุทธิน้อยกว่า '+$start_baht+' Please check and try again.','WARNING!',function(){
							$("#btn_cal_promotion").removeAttr("disabled");
							$('#btn_cal_promotion').removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
							$("#csh_product_id").focus();
		    			});
								...
								$.ajax({
										...
										success:function(data){
											if(data=='3'){
												...
												jAlert('!!! ยอดเงินสุทธิจากการคำนวณส่วนลด พบข้อผิดพลาด !!!\nกรุณาลบรายการสินค้าที่ราคาสูงสุดออก และสแกนสินค้าชิ้นนั้นอีกครั้ง','WARNING!',function(){	
													rePromoPointDiscount();
													$("#btn_cal_promotion").removeAttr("disabled");
				    								$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
													return false;
            							...
            							success:function(data){
            								...
            								$('.tableNavPercentRange ul li').not('.nokeyboard').navigate({
											    ...
											    if(point>point_total){
											    	jAlert('คะแนนสะสมไม่พอ Please check and try again.','WARNING!',function(){													
														return false;
									    			});
											    	return false;
											    }
		...
		$('#btnCoupon').button({
	    	...
	    	var onsales=onSales();
		    if(onsales>0){
			 	jAlert('You make a pending sale. Can not select the promotion after the sale.', 'WARNING!',function(){	            	
	            	return false;
		        });
			 	return false;
			}
		    			    		 ...
		    			    		 $('.tableNavCouponPro ul li').not('.nokeyboard').navigate({
										  ...
										  //*WR27102014
										    ...
										    if(sel_promo.member_tp == 'Y' && member_no.length<1){
										    	jAlert('กรุณาสแกนรหัสสมาชิกก่อนเล่นโปรโมชั่น', 'WARNING!',function(){
						    					...
						    					jAlert('ลูกค้า WALK IN กรุณากดคีย์ ENTER ที่ช่องรหัสสมาชิก \nเพื่อแสดงสถานะลูกค้าประเภท WALK IN ก่อนเล่นโปรโมชั่น', 'WARNING!',function(){
										    		$('#dlgProCoupon').dialog('close');
										    		initTblTemp();
													initFormCashier();
													initFieldOpenBill();
														    	            ...
														    	            { 
														    	                ...
														    	                click: function(evt){ 
																		    		...
																		    		if(coupon_code.length != 12){
																	    				jAlert('กรุณาป้อนรหัส Coupon ให้ครบ 12 หลัก', 'WARNING!',function(){
																		    		...
																		    		if(chk_coupon_disocunt != parseInt(sel_promo.discount)){
																		    			jAlert('รหัส Coupon ' + coupon_code + ' ไม่ตรงกับโปรโมชั่นส่วนลด ' +  parseInt(sel_promo.discount) + "%", 'WARNING!',function(){
...
//		    			                      .css({"padding":".1em .1em .1em 0","margin":"0 0 0 0","background-color":"#C7D9DC"} )
								...
								$("#vt_name").keypress( function(evt) {
							        ...
							        if(key == 13) {
							            ...
							            if(jQuery.trim($("#vt_name").val())==''){
							            	 jAlert('กรุณาระบุชื่อ','WARNING!',function(){
									            	$("#vt_name").focus();
									            	return false;
										      });
								        }else{
						...
						buttons: {							
								"OK":function(){ 	
						            ...
						            if(csh_member_no.length>0 && vt_passport_no.length>0){
						            	 jAlert('ลูกค้าที่เป็นสมาชิกอยู่แล้ว ไม่สามารถคีย์ Passport ได้','WARNING!',function(){		
						            		 $('#vt_passport_no').val('');
								             return false;
									      });
						            	 return false;
		 ...
		 $('#btnGreenBag').click(function(e){	    	
		    	...
		    	var net_amt=$('#csh_net').val().replace(/[^\d\.\-\ ]/g,'');
		 		...
		 		var promo_tp=$('#csh_promo_tp').val();
		 		if(status_no=='02'){
					jAlert('บิล 02 (ซื้อครั้งแลกส่วนลดพิเศษ) ไม่ร่วมโปรโมชั่น', 'WARNING!',function(){	            
						$('#csh_product_id').focus();
		            	return false;
			        });
					return false;
				}else if(status_no=='04' && net_amt<1){
					jAlert('บิล 04 (สมาชิกแลกคะแนน) กรุณาสแกน Barcode ถุงที่จะใช้\nหลังจากสแกนรายการสินค้าที่ร่วมโปรโมชั่นแล้ว', 'WARNING!',function(){	            
						$('#csh_product_id').focus();
		            	return false;
			        });
					return false;
...
//		    	var onsales=onSales();
//			    if(onsales>0){
//				 	jAlert('คุณทำรายการขายค้างอยู่ ไม่สามารถเลือกโปรโมชั้นหลังทำรายการขายได้', 'WARNING!',function(){	            	
//		            	return false;
//			        });
//				 	return false;
//				}
							 		...
							 		if(g1!=''){
							 			...
							 			$.ajax({
							 				...
							 				data:{
							 					...
							 					if(data=='O'){
							 					 	jAlert('ไม่สามารถเล่นโปรนี้ได้เกิน 5 ครั้งต่อวัน', 'WARNING!',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
							 					}else if(data=='N'){
							 					 	jAlert('ไม่พบรหัสถุงหรือรหัสถุงนี้ไม่ใช่ของสมาชิก', 'WARNING!',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
							 					}else if(data=='R'){
							 						jAlert('พบรายการรหัสถุงแล้ว ไม่สามารถใช้มากกว่า 1 ถุงต่อบิลได้', 'WARNING!',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
	    			    		...
	    			    		data:{
	    			    			...
	    			    			$('.tableNavSmsPro ul>li').not('.nokeyboard').navigate({
									    ...
									    if(sel_promo.member_tp=='Y' && $.trim(csh_member_no).length==0){
									    	jAlert('กรุณาระบุรหัสสมาชิกก่อน','WARNING!',function(){
												 return false;
											  });
									    	return false;
									    }
	            ...
	            if(member_no.length>0){
	            	            		...
	            	            		//satart old
	            	            		 if(csh_status_no=='01'){
		            	     			    	...
		            	     			    	if(getOnlineStatus()===1){	
		            	     			    		...
		            	     			    		//callMemberOffline();
		            	     			    		jAlert("ไม่สามารถแลกคะแนนแบบ OFFLINE ได้","WARNING!",function(){
	            	     				    			$("#csh_member_no").focus();
	            	     				            	return false;
	            	     					        });
	            	     				    		return false;
		            	     			    	...
		            	     			    	if($("#cn_member_no").val()!=''){
		            	     			    		if($("#csh_member_no").val()==''){
		            	     				    		jAlert("กรุณาระบุ รหัสสมาชิก","WARNING!",function(){
		            	     				    			$("#csh_member_no").focus();
		            	     				            	return false;
		            	     					        });
		            	     				    		return false;
		            	     				    	}else if($("#csh_member_no").val()!=$("#cn_member_no").val()){
		            	     				    		jAlert("รหัสสมาชิกไม่ตรงกับรหัสมาชิกในบิลลดหนี้(CN)","WARNING!",function(){
		            	     				    			$("#csh_member_no").focus();
		            	     				            	return false;
		            	     					        });
		            	     				    		return false;
	            	...
	            	if(csh_status_no=='02'){
	            		var key_employee=$.trim($("#csh_member_no").val());
			            	...
			            	var key_employee=$.trim($("#csh_member_no").val());
		     			   	if(key_employee.length==0){
						    		jAlert("กรุณาระบุ รหัสพนักงาน","WARNING!",function(){
						    			$("#csh_member_no").focus();
						            	return false;
							        });
						    		return false;
			            	...
			            	var key_employee=$.trim($("#csh_member_no").val());
					    		...
					    		jAlert("กรุณาระบุ รหัสบัตรเสีย/บัตรหาย","WARNING!",function(){
					    			$("#csh_member_no").focus();
					            	return false;
						        });
									...
									$('.tableNavOtherPro ul li').not('.nokeyboard').navigate({
									  ...
									  //*WR02062016 for support mini bueaty book 2016
									    ...
									    if(sel_promo.promo_code=='50BTO1P'){
									    	...
									    	if(mp_status_no=='02'){
									    		jAlert("โปรโมชั่น More Point ไม่สามารถเล่นร่วมกับบิล 02(First Purchase) ได้","WARNING!",function(){	
									            	return false;
										        });
									    		return false;
									    	}
																				       			...
																				       			$(this).click(function(){
																				       				...
																				       				if(strJson!=''){
																						       				...
																						       				if(seldocno.status_receive=='Y'){
																						       					jAlert("รับสินค้าแล้ว","WARNING!",function(){																					
																													return false;
																										        });
																						       					return false;
																						       				}
																						       				if(seldocno.flag=='C'){
																						       					jAlert("เอกสารนี้ถูกยกเลิก","WARNING!",function(){																					
																													return false;
																										        });
																							       				return false;
																						       				}
														...
														$('#btnSubmitTemp').click(function(e){
														    ...
														    if(onsales>0){
																...
																$('#dlgDocNoTemp').dialog('close');
															 	jAlert('ไม่สามารถคืนสินค้าซ้ำได้', 'WARNING!',function(){															 		
																	getCshTemp('N');
																	paymentBill('00');
													            	return false;
														        });
															    	  ...
															    	  $("#key_quantity_return").keypress(function(evt){	
															    		  ...
															    		  if(key==13){
																    		  ...
																    		  if(key_quantity_return.length==0 || key_quantity_return=='0'){
															    				  jAlert("กรุณาคีย์จำนวนสินค้า","WARNING!",function(){				
															    			  ...
															    			  if(key_quantity_return>limited_qty){
															    				  jAlert("ไม่สามารถคีย์จำนวนสินค้าได้มากกว่า " + limited_qty + " รายการ","WARNING!",function(){				
															    					   $("#key_quantity_return").val('1').focus();
																						return false;
																			        });
															    				  return false;
															    	  ...
															    	  $("#key_product_return").keypress(function(evt){															    		 
															    		  ...
															    		  if(key==13){
															    			  ...
															    			  if(key_product_return.length==0){
															    				  jAlert("กรุณาคีย์รหัสสินค้า","WARNING!",function(){				
															    					   $("#key_product_return").val('').focus();
																						return false;
																			        });
															    			  }
															...
															var opts_chk={
																	...
																	data:{
																		...
																		if(data=='1'){
																			//ไม่พบ
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบเลขที่เอกสารนี้ หรือเลขที่เอกสารนี้ไม่ร่วมโปรโมชั่น","WARNING!",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='2'){
																			...
																			jAlert("รหัสบัตรสมาชิกไม่ตรงตามเลขที่เอกสารนี้","WARNING!",function(){																						
																				return false;
																	        });
																			return false;
																		}else if(data=='3'){			
																			//รับไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("บิลนี้ไม่ได้ติดสินค้า short <br>ไม่สามารถคืนได้","WARNING!",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='4'){
																			...
																			jAlert("ยอดซื้อน้อยกว่า 2000 ไม่สามารถรับโปรนี้ได้","WARNING!",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='5'){	
																			...
																			jAlert("ยอดซื้อน้อยกว่า 3000 ไม่สามารถเล่นโปรนี้ได้","WARNING!",function(){																						
																				return false;
																	        });
																			return false;
																		}else if(data=='6'){																				
																			...
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบโปรโมชั่นที่ร่วมโปร","WARNING!",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='8'){			
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่ใช่บิล 01 บัตรใหม่หรือบัตรต่ออายุ","WARNING!",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='9'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("bill ทีมีการตรวจสอบยอดซื้อ","WARNING!",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='10'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบเลขที่เอกสาร หรือเลขที่เอกสารไม่อยู่ในช่วงเวลาที่กำหนด","WARNING!",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='11'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบโปรในตาราง promo_short","WARNING!",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='12'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("เลขที่เอกสารนี้ถูกยกเลิกแล้ว","WARNING!",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='13'){																				
																			...
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("บิลนี้ไม่เป็นไปตามเงื่อนไขโปรโมชั่น ไม่สามารถคืนสินค้าได้","WARNING!",function(){																				
																				return false;
																	        });
																			return false;
																		}else{
											  		 ...
											  		 $.ajax({
											  			 ...
											  			 data:{
											  				 ...
											  				 if(data=='Y'){
											  					...
											  					jAlert("ไม่พบข้อมูลหรือลูกค้ารับสินค้าไปแล้ว", 'WARNING!',function(){
											  						initFieldOpenBill();
											  		            	return false;
											  			        });
											  				 }

	$(function(){	
				...
				$('#btnBwsIdCard').click(function(e){
					...
					if(chk_member_no.length>0){
						jAlert('พบการทำรายการค้างอยู่ กรุณายกเลิกเอกสารแล้วทำรายการใหม่', 'แจ้งเตือน',function(){
							$('#btnBwsIdCard').focus();
							return false;
						});	
						return false;
	...
	function formTour(promo_code){
		...
		$("<div id='dlgFormTour'></div>").dialog({
	       	   autoOpen:true,
	  				...
	  				closeOnEscape: false,
	  				title:"<span class='ui-icon ui-icon-person'></span>กรุณายืนยันข้อมูลลูกค้า",
	  				open: function(){ 
    							...
    							$('#tour_barcode').keypress(function(evt){
    						        ...
    						        if(key == 13){
    						            ...
    						            if(tour_barcode.length<1){
    						            	jAlert('กรุณาระบุทัวร์ Barcode','ข้อความแจ้งเตือน',function(){
    				    						$('#popup_ok').focus();
    				    						setTimeout(function(){				    							
    						            ...
    						            //alert("chk_tour_digit=>" + chk_tour_digit);    						            
    						           	if(chk_tour_digit!=='PRCTG'){
    						           		...
    						           		jAlert('รหัสคูปอง Barcode ไม่ถูกต้อง','ข้อความแจ้งเตือน',function(){
    				    						$('#popup_ok').focus();
    				    						setTimeout(function(){				    							
    				    							if($('#popup_ok').not(":focus")){
    						    						$('#popup_ok').focus();
    							...
    							$('#tour_passport_no').keypress(function(evt){
    						        ...
    						        if(key == 13){
    						            ...
    						            if(tour_passport_no.length<1){
    						            	jAlert('กรุณาระบุเลขที่ Passport','ข้อความแจ้งเตือน',function(){
    				    						$('#popup_ok').focus();
    				    						setTimeout(function(){				    							
    				    							if($('#popup_ok').not(":focus")){
    						    						$('#popup_ok').focus();
		    	                ...
		    	                click: function(evt){ 
						        	...
						        	//*WR modify 22052017
						           	...
						           	if(tour_country.length<1){							 			
							 			jAlert('กรุณาระบุประเทศ','ข้อความแจ้งเตือน',function(){		
							 				$('#popup_ok').focus();
				    						setTimeout(function(){				    							
				    							if($('#popup_ok').not(":focus")){
						    						$('#popup_ok').focus();
				...
				,beforeClose:function(evt){
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){	
						//evt.stopPropagation();
						evt.preventDefault();
						jConfirm('คุณต้องการยกเลิกการเล่นโปรโมชั่นใช่หรือไม่','ข้อความแจ้งเตือน', function(r){
					        if(r){
					        	initFormCashier();
								initTblTemp();
								getCshTemp('Y');
	...
	function mstMcpPromotion(promo_code,chk_status,line_msg_error,idcard,coupon_code){
	   ...
	   //alert(promo_code +","+chk_status+","+line_msg_error +","+ idcard +","+ coupon_code);
		if(chk_status=='Y'){
			...
			$('<div id="dlgPdtOtherPro"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span><input type="text" size="15" id="pdt_otherpro"/></div>').dialog({
		           ...
		           open:function(){
				    	            ...
				    	            { 
				    	                ...
				    	                click: function(evt){ 
								    		...
								    		if(product_id.length<1){
							    				jAlert('กรุณาสแกนรหัสสินค้า', 'แจ้งเตือน',function(){
				  							 ...
				  							 $.ajax({
				  								 ...
				  								 data:{
				  									...
				  									if(arr_data[0]=='1'){	
								    					...
								    					//*WR04092045 Mcoupon
								    					jAlert(arr_data[1],'ข้อความแจ้งเตือน',function(){
								    						$('#popup_ok').focus();
								    						setTimeout(function(){
						...
						if(buy_type=='N' ){
							...
							var start_baht=start_baht.replace(/[^\d\.\-\ ]/g,'');
							 ...
							 if(net_amt<start_baht){
								 jAlert("ยอดเงินสุทธิอย่างน้อยต้อง " + start_baht + " บาท", 'แจ้งเตือน',function(){																						
									 $('#pdt_otherpro').val('').focus();
										return false;
								});
								 return false;																		    										 
					...
					,close:function(evt){
						...
						if(onSales()==0){
							jAlert('ไม่พบการทำรายการขายไม่สามารถชำระเงินได้', 'แจ้งเตือน',function(){
								initFormCashier();
								initFieldOpenBill();
								$("#csh_member_no").focus();
		    					return false;
			...
			jAlert(line_msg_error, 'แจ้งเตือน',function(){
				initFormCashier();
				initFieldOpenBill();
				$("#csh_member_no").focus();
				return false;
	...
	function lineApp3(application_id,a,b,c,d,e,idcard,coupon_code,friend_id_card,friend_mobile_no,promo_code,reg_mobile_no){
	 ...
	 function setPdt2Temp(promo_code){
			...
			$('<div id="dlgPdtMaster"><span style="color:#336666;font-size:20px;">รหัสสินค้า : </span><input type="text" size="15" id="pdt_master_id"/></div>').dialog({
					   ...
					   resizable:false,
					   closeOnEscape:false,		
			           title: "สแกนรายการสินค้าหลักที่ร่วมโปรโมชั่น",
			           position:"center",
			           open:function(){
							$(this).dialog('widget')
					            .find('.ui-dialog-titlebar')
					    	            ...
					    	            { 
					    	                ...
					    	                click: function(evt){ 
					    	                	evt.preventDefault();
										 		...
										 		product_id=$.trim(product_id);																		 			
									    		if(product_id.length<1){
								    				jAlert('กรุณาสแกนรหัสสินค้าหลักที่ร่วมรายการโปรโมชั่น', 'แจ้งเตือน',function(){
							  										    	            ...
							  										    	            { 
							  										    	                ...
							  										    	                click: function(evt){ 
							  										    	                	evt.preventDefault();
							  															 		...
							  															 		product_id=$.trim(product_id);																		 			
							  														    		if(product_id.length<1){
							  													    				jAlert('กรุณาสแกนรหัสสินค้าฟรีที่ร่วมรายการโปรโมชั่น', 'แจ้งเตือน',function(){
							  											...
							  											if(promo_code=='OK01160115'){
							  												//if(buy_type=='N' ){
							  													...
							  													var start_baht=start_baht.replace(/[^\d\.\-\ ]/g,'');
							  													 ...
							  													 if(net_amt>end_baht){
							  														 jAlert("ยอดเงินสุทธิต้องไม่เกิน " + end_baht + " บาท กรุณาทำรายการใหม่", 'แจ้งเตือน',function(){																						
							  															$('#pdt_detail_id').val('').focus();
							  															return false;
							  														});
							  														 return false;																		    										 
					        ...
					        if(key == 13){
					            ...
					            $.ajax({
					            	...
					            	data:{
					            		...
					            		if(data!='Y'){
					            			jAlert(data, 'ข้อความแจ้งเตือน',function(){
					            				$('#new_card_tues').focus();
	           									return false;
	           								});
					            		}else{					            		
					        ...
					        if(key == 13){
					            ...
					            //*WR 12052015
					           	...
					           	if(chk_member_th_old!='1120'){
					           		jAlert("กรุณาระบุรหัสบัตรเก่าเป็นบัตรวันพฤหัสบดีเท่านั้น", "ข้อความแจ้งเตือน",function(){       									
       									$('#old_card_thu').focus();
       									return false;
       								});
					           		return false;
					           						...
					           						function(data){							
					           							...
					           							if(data=='2'){
					           								...
					           								$.each(data.member, function(i,m){
					           									if(m.exist=='yes'){					           										
					           										if(m.age_card!='4'){
					           											jAlert('ต้องเป็นสมาชิก 4 ปีเท่านั้น', 'ข้อความแจ้งเตือน',function(){
								           									$('#old_card_thu').focus();
								           									return false;
								           								});
					           											return false;
					           								...
					           								//------------------ zone alert --------------------
						           								...
						           								if(m.forgot_card=='Y'){
					           										jAlert('รหัสนี้ ถูกแจ้งลืม ในระบบแล้ว<br>สอบถามรหัสรับบัตรคืน(SMS) จากลูกค้า<br>หรือติดต่อ Admin', 'ข้อความแจ้งเตือน',function(){
					           											initFieldOpenBill();
					           											return false;
					           										});	
					           										return false;	
...
//									}
	...
	function chkCnMember(){
		...
		if(member_no!=cn_member_no){
			jAlert('รหัสสมาชิกไม่ตรงกับรหัสสมาชิกในเอกสารลดหนี้ (CN) <br>กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(b){
					 return false;
	   		 });
		}else{
...
//		return false;
			...
			success:function(data){
				...
				if(obj_json.status_ck=='Y'){
					...
					if(obj_json.month_enable=='Y' && obj_json.outofday=='Y' && obj_json.outtime=='Y'){
						...
						jAlert('เดือนนี้ไม่สามารถใช้สิทธิได้','ข้อความแจ้งเตือน',function(b){
							initTblTemp();
					        initFormCashier();
							initFieldOpenBill();
		    				 return false;
	...
	function chkEmpPrivilage(){
		...
		*/
		...
		if(employee_id.length==0){
			jAlert('กรุณาระบุรหัสพนักงาน','ข้อความแจ้งเตือน',function(b){
				$("#csh_member_no").focus();
				 return false;
    		 });		
	   		 return false;	
				...
				success:function(data){
					if(data!=''){
						...
						if(ecp.error_opsday=='Y'){
							jAlert('พนักงานไม่สามารถใช้สิทธิในวัน OPS Day ได้','ข้อความแจ้งเตือน',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.error_cam=='Y'){
							jAlert('กรุณา Scan บัตรพนักงาน หรือถ่ายรูปขอสิทธิก่อน','ข้อความแจ้งเตือน',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.error_empty=='Y'){
							jAlert('พนักงานท่านนี้ใช้สิทธิไม่ได้ กรุณาพยามอีกครั้งหรือติดต่อ HR','ข้อความแจ้งเตือน',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
						}else if(ecp.is_used=='Y'){							
							jAlert('พนักงานท่านนี้ใช้สิทธิไปแล้ว','ข้อความแจ้งเตือน',function(b){
								$("#csh_member_no").focus();
			    				 return false;
				    		 });
							return false;
							...
							var amount_balance=parseFloat(ecp.amount_balance);
							 ...
							 $('#other_promotion_title').html(" "+ecp.name+" "+ecp.surname+
								...
								jAlert('ผู้มีส่วนร่วมในการใช้สิทธิ์พนักงานของบุคคลอื่น \nจะถูกตัดสิทธิ์สวัสดิการทุกประเภท<br>' +
										msg_emp_tp,'ข้อความแจ้งเตือน',function(b){
									initFieldPromt();
				    				 return false;
					    		 });	
						}
					}else{
						jAlert('ไม่สามารถใช้สิทธิได้ กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(b){
							$("#csh_member_no").focus();
		    				 return false;
			    		 });
					}
	...
	function deleteFromGrid(del_list,del_list_show){
		...
		*/
		 ...
		 jConfirm('คุณต้องการลบรายการลำดับที่\r\n '+del_list_show+' ใช่หรือไม่?','ยืนยันการลบรายการ', function(r){
		        if(r){
					var opts={
							   type:"POST",
							   ...
							   success: function(data){
								    if(data=='LOCKDEL'){
								    	 jAlert('ไม่สามารถลบรายการขณะเล่นโปรนี้ได้ ท่านสามารถเลือกยกเลิกรายการขายแทน','ข้อความแจ้งเตือน',function(b){
								    		 initFieldPromt();
						    				 return false;
							    		 });
	...
	function saveTransaction(){
		...
		if($.trim(status_no)=='01'){
			...
			if(member_no.length==0 && $('#csh_application_type').val()=="NEW" && $('#csh_card_type').val()=="MBC"){
				jAlert('ไม่พบรหัสบัตรใหม่ กรุณายกเลิกเอกสารแล้วทำรายการใหม่','ข้อความแจ้งเตือน',function(){			
					return false;
				});		
				return false;
			}else if(refer_member_id.length==0 && $('#csh_application_type').val()=="ALL"){
				jAlert('ไม่พบรหัสบัตรเก่าที่ใช้อ้างอิง กรุณายกเลิกเอกสารแล้วทำรายการใหม่','ข้อความแจ้งเตือน',function(){			
					return false;
				});	
				return false;
			}//end if			
    						                ...
    						                success:function(data){
    						                        ...
    						                        if(arr_data[0]=='Y'){
    						                        	...
    						                        	if(cmp_member_no==confirm_member_no){
        		    						            	...
        		    						            	jAlert("รหัสสมาชิกที่ยืนยันไม่ตรงกัน กรุณาตรวจสอบอีกครั้ง", 'ข้อความแจ้งเตือน',function(){
        							            				$('#confirm_member_no').focus();
        			           									return false;
        			           								});
        		    						            	return false;
    						                        	...
    						                        	jAlert("Code หมดเวลากรุณาขอ Code อีกครั้ง","ข้อความแจ้งเตือน",function(){	                                       
    				                                        $("#confirm_member_no").val('');
    				                                        $("#confirm_member_no").focus();
    				                                        return false;
    				                                    });
		...
		if(parseFloat(credit_net_amt)<1){
			...
			var opts_edc={
					...
					data:{
						...
						//////////////////////\\\\\alert\\\\\/////////////////////////
						jConfirm('ตรวจสอบยอดชำระเงินบัตรเครดิต','ข้อความแจ้งเตือน', function(r){
					        if(r){
...
//        							});//keypress
				 	...
				 	"ตกลง":function(evt){
				 		...
				 		if(status_no=='01' && application_id=='OPKTC300'){				 			
				 			...
				 			if(jQuery.inArray(chk_ktc300, arr_ktc300) != -1) {
				 				...
				 				jAlert('กรุณาระบุเลขที่บัตรเครดิต KTC ด้านหน้า 6 หลัก\nเฉพาะชุดสมัคร OPKTC300 เท่านั้น','ข้อความแจ้งเตือน',function(r){
									if(r){
										$("#txt_credit_no_value").focus();
										return false;
									}	
				 			...
				 			if(credit_no_value.length<6 ){
					 			jAlert('กรุณาระบุเลขที่บัตรเครดิตด้านหน้า 6 หลักเฉพาะชุดสมัคร OPKTC300','ข้อความแจ้งเตือน',function(r){
									if(r){
										$("#txt_credit_no_value").focus();
										return false;
									}	
	...
	function chkBeforeConfirmPayment(){
		...
		if(chk_txt_cash>0 && chk_txt_credit>0 ){
        	jAlert('กรุณาเลือกวิธีการชำระเงินอย่างใดอย่างหนึ่งเท่านั้น กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){
				$("#txt_cash").focus();
				return false;
			});
        ...
        if(chk_net_curr!=0){
	    	...
	    	$("#btn_payment_confirm").removeClass("ui-state-focus ui-state-hover ui-state-active");
        	jAlert('จำนวนเงินชำระมากเกินปกติ กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){
				$("#txt_cash").focus();
			});
	    	return false;
	    }else{
	    	var csh_bill_manual_no=$('#csh_bill_manual_no').val();
	 		...
	 		if(csh_bill_manual_no.length>0){
	 			jConfirm('คุณต้องการบันทึกการเปิดบิลมือ\nเลขที่บิล : ' + csh_bill_manual_no +
	 					...
	 					'\nใช่หรือไม่','ยืนยันการบันทึก', function(r){
	 		        if(r){
	 		        	setBag2Temp();
	 		        }else{
	 		        	return false;
						...
						function(){
						    ...
						    $("#btn_payment_confirm").click( function(e){
								...
								$.ajax({
									...
									data:{
										...
										if(data=='Y'){												
											if($('#payment_channel').val()!="Alipay"){
												jAlert('โปรโมชั่นนี้(OX07140317)ต้องชำระเงินผ่าน Alipay เท่านั้น','ข้อความแจ้งเตือน',function(){														
													return false;
								    			});
												return false;
											}else{
							...
							$("#btn_payment_alipay").click( function(evt){
								...
								if(member_type!=='WALK IN'){
									jAlert('การชำระเงินผ่าน Alipay ใช้สำหรับลูกค้า WALK IN เท่านั้น', 'แจ้งเตือน',function(){									    	
								    	return false;
								    });	
								    return false;
								}else{
							  				...
							  				buttons: [ 			
									    	                	...
									    	                	evt.preventDefault();
														 		...
														 		$.ajax({
														 			...
														 			data:{
														 				...
														 				if(data=='Y'){
														 					jAlert('เลขที่การเปิดบิลมือซ้ำไม่สามารถบันทึกได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
								    	                ...
								    	                { 
									    	                ...
									    	                click: function(evt){ 
									    	                	evt.preventDefault();
														 		evt.stopPropagation();	
														 		jConfirm('คุณต้องการยกเลิกรายการบิลมือ ใช่หรือไม่?','ยืนยันการยกเลิก', function(r){
															        if(r){
															        	$('.frmKeymanualFixedTextbox').val('');
																 		$('.frmKeymanualFixedTextboxTicket').val('');
																 		$('#csh_bill_manual_no').val('');
	...
	function paymentBillOrg31052017(status_no){
		/**
		*@desc last modify : 12062014
		*@param String status_no :
		*@return void
		*/	
				...
				open: function(){ 
						...
						function(){
							...
							$("#btn_payment_credit").click( function(evt){
					        	...
					        	var online_mode=false;
					        	if(!online_mode){		
					        		$('#txt_cash').val('0.00');//WR20052013 เลือกปรเภทการชำระได้อย่างใดอย่างหนึ่งเท่านั้น ณ ตอนนี้
					        		$('#txt_cash_back').val('0.00');
					        		$('#txt_cash_back_2').val('0.00');					        		
					        		calPaid();
						        	dlgCredit();
					        		return false;
						        }							       
	...
	function cancelBill(){
		...
		*/
		var opts={
					...
					success:function(data){
						if(data=='0'){
							jAlert('ไม่สามารถยกเลิกรายการได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
								return false;
							});	
						}else{						
		...
		var opts={
				...
				success:function(data){		
					if(data==1){
						...
						jAlert('สินค้า ไม่ร่วมโปรโมชั่น กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
					}else if(data==6){
						jAlert('ไม่สามารถซื้อสินค้าเกินกว่าจำนวนที่กำหนดได้  กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){								
							initFieldPromt();		
							return false;							
						});			
						return false;		
...
//								       return false;
									...
									if($('#csh_status_no').val()=='02' && parseFloat(csh_first_limited)>0){			
										...
										var opts_chk_first_lmt={
													...
													data:{
														...
														if(data=='Y'){
															...
															jAlert('ยอดซื้อต้องไม่เกิน ' + parseFloat(csh_first_limited).toFixed(2) + ' กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){						
																initFieldPromt();		
																return false;							
															});	
														}
										...
										if(status_no==''){
											jAlert('สถานะเอกสารเอกสารไม่ถูกต้อง กรุณายกเลิกการขายแล้วเปิดบิลใหม่', 'ข้อความแจ้งเตือน',function(){
												$("#csh_product_id").focus();
												return false;
											});	
											return false;
		...
		*/
		...
		var opts={
				...
				success:function(data){
					var arr_data=data.split('#');	
					if(arr_data[0]=='0'){
						jAlert('ไม่สามารถทำรายการสินค้านี้ได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
							initFieldPromt();
							return false;
						});	
					}else{		
	...
	function setSmsProduct(sms_promo_code){
		...
		var opts_smsproduct={
				...
				data:{
					...
					var arr_data=data.split('#');
					if(arr_data[0]=='0'){
						jAlert('ไม่สามารถบันทึกรายการได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){							
							return false;
						});	
					}else{
	...
	function callSmsPromo(sms_promo_code,sms_mobile,redeem_code,member_tp,idcard){
		...
		var opts_smspromo={
				...
				data:{
					...
					if(data=='null'){
						jAlert("ไม่พบข้อมูล กรุณาตรวจสอบอีกครั้ง", 'ข้อความแจ้งเตือน',function(){
					...
					if(objJson.status=='OK'){
						...
						$('#id_redeem_code').val(redeem_code);
						 //*WR23012013						
						  if(prefix_member_no_idcard!='ID' && member_tp=='Y'  && objJson.member!=$('#csh_member_no').val()){
							  jAlert("รหัสบัตรสมาชิกไม่ตรง กรุณาตรวจสอบอีกครั้ง", 'ข้อความแจ้งเตือน',function(){
			    				   $("#sms_mobile").focus();
									return false;
								});	
							  return false;
						...
						//*WR20092016 fix code for mcs promotion						  
						 ...
						 jAlert(objJson.status_msg, 'ข้อความแจ้งเตือน',function(){
							  //---------------- 25072013 --------------------------------
							 $('#status_smspromo').val('');
							 $('#id_smspromo').val('');
									    	   ...
									    	   data:{
									    		  ...
									    		  var arr_data=data.split('#');
									    		   if(arr_data[0]=='1'){
									    			   ...
									    			   jAlert('สินค้าไม่ร่วมโปรโมชั่น กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    			   ...
									    			   jAlert('ไม่พบสินค้าในสต๊อก กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
									    				   $("#co_product_id").select().focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='3'){
									    			   jAlert('สินค้าอยู่ในสถานะ Lock กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }else if(arr_data[3]=='4'){
									    			   jAlert('ราคาสินค้าร่วมโปรโมชั่นมากว่าสินค้าปกติ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
									    				   $("#co_product_id").focus();
															return false;
														});	
									    		   }
		...
		var opts={
				...
				success:function(data){					
					...
					if(arr_data[0]=='0'){
						...
						jAlert('ไม่สามารถบันทึกสินค้าซ้ำได้ หรือ กด ESC Key เพื่อออกจากโปรโมชั่นก่อนคีย์สินค้าทั่วไป', 'ข้อความแจ้งเตือน',function(){
							initFieldPromt();
							return false;
						});	
					}else{						
	...
	function setCshValTemp(){
		...
		*/
		...
		if(quantity.length==0){
			jAlert('กรุณาระบุ จำนวนสินค้า', 'ข้อความแจ้งเตือน',function(){
				$("#csh_quantity").focus();
				return false;
			});	
			return false;
		...
		if(member_percent1 != '' && member_no==''){
			jAlert('ไม่ใช่สมาชิกไม่ได้รับส่วนลด กรุณายกเลิกการขายแล้วเปิดบิลใหม่', 'ข้อความแจ้งเตือน',function(){
				$("#csh_product_id").focus();
				return false;
			});	
			return false;
			...
			var opts_vip={
						...
						success:function(data){
							if(data=='Y'){
								...
								jAlert('ยอดซื้อมากกว่าวงเงินคงเหลือ '+addCommas(vip_balance)+' บาท','ข้อความแจ้งเตือน',function(){
									initFieldPromt();
									return false;
								});	
								return false;
							    ...
							    $('#dlgSelCoPromotion').empty().html(data);	
	    			    		 $('.tableNavSelCopromotion ul li').not('.nokeyboard').navigate({
								    ...
								    if(promo_play=='OX07320814' || promo_play=='OX07561014'){
								    	...
								    	if(chk_oppn != 'OPPN300'){
								    		jAlert('เฉพาะชุดสมัครใหม่ OPPN300 เท่านั้น','ข้อความแจ้งเตือน',function(){
												return false;
											});	
								    		return false;
								    	}
															    		...
															    		$.ajax({
															    			...
															    			data:{
															    				...
															    				if(arr_data[0]=='1'){															    					
															    					...
															    					jAlert(msg_result,'ข้อความแจ้งเตือน',function(){
																						$("#btn_cal_promotion").removeAttr("disabled");
																						$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
											    	            ...
											    	            { 
											    	                ...
											    	                click: function(evt){ 
											    	                	evt.preventDefault();
																 		...
																 		product_id=$.trim(product_id);																		 			
															    		if(product_id.length<1){
														    				jAlert('กรุณาสแกนรหัสสินค้าที่ร่วมรายการโปรโมชั่น', 'แจ้งเตือน',function(){
			    	            ...
			    	            { 
			    	                ...
			    	                click: function(evt){ 
			    	                	evt.preventDefault();
								 		...
								 		jConfirm('คุณต้องการยกเลิกรายการขาย ใช่หรือไม่?','ยืนยันการใช้สิทธิ', function(r){
									        if(r){
									        	$('#dlgProRedeemSpecial').dialog('close');
										 		initTblTemp()
												initFormCashier();
		...
		if(member_no.length==0){
							...
							$.each(data.member, function(i,m){
								if(m.exist=='yes'){
									...
									if(status_no=='04' && m.link_status=='OFFLINE'){										
										jAlert("ไม่พบการตอบสนองจาก Server ไม่สามารถแลกคะแนน(OFFLINE)ได้","ข้อความแจ้งเตือน",function(){
											initFieldOpenBill();
    	     				            	return false;
    	     					        });
    	     				    		return false;										
			...
			setTimeout(function(){
				if(opts_redeempoint){
					...
					jAlert('ไม่พบการตอบสนองจาก Server กรุณาลองใหม่อีกครั้ง','ข้อความแจ้งเตือน',function(){
						initFieldOpenBill();
						return false;
					});	
				}
										...
										if($input_giftset_quantity_val.length==0){
											...
											var opts={
													...
													success:function(data){	
														var arr_data=data.split('#');
														if(arr_data[0]=='3'){
															jAlert('สินค้านี้ถูก lock ไม่สารมารถทำรายการได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
																$input_giftset_product.val('').focus();
																return false;
															});	
														}else if(arr_data[0]=='2'){
															jAlert('จำนวนสินค้าไม่พอที่จะทำรายการ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
																$input_giftset_product.val('').focus();
																return false;
															});	
														}else if(arr_data[0]=='0'){
				...
				close: function(evt,ui) {
					...
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){
						if($csh_net_val<$gift_set_amount_val){
							jAlert("ยอดซื้อน้อยกว่า " + parseFloat($("#csh_gift_set_amount").val()).toFixed(2) + " กรุณาตรวจสอบอีกครั้ง","ข้อความแจ้งเตือน",function(){
					    	...
					    	var key = evt.keyCode ? evt.keyCode : evt.which ? evt.which : evt.charCode; 
		   			        if(key == 13){	
					    		...
					    		if(keyin_mstquantity_id.length<1){
				    				jAlert('กรุณาระบุจำนวนสินค้า', 'แจ้งเตือน',function(){
							 		...
							 		$.ajax({
							 			...
							 			success:function(data){	
											if(data=='Y'){
												...
												jAlert(data, 'แจ้งเตือน',function(){																						
													 $('#pdt_otherpro').val('').focus();
														return false;
												});
											}
														...
														if(application_id=='OPPC300'){				
																	...
																	data:{
																		...
																		rnd:Math.random()
																			...
																			if(price_check > parseFloat($free_product_amount.val())){
																				jAlert("ยอดซื้อมากกว่า " + parseFloat($free_product_amount.val()).toFixed(2) + " กรุณาตรวจสอบอีกครั้ง","ข้อความแจ้งเตือน",function(){
																					window.parent.$(".ui-dialog").remove();
																...
																$.ajax({
																	...
																	data:{
																		...
																		if(data=='N'){
																			jAlert("กรุณาเลือกสินค้าในกลุ่ม Body/Hair ","ข้อความแจ้งเตือน",function(){
																				$("#input_free_product").val('').focus();
																				return false;
																			});	
																			return false;
				...
				close: function(evt,ui) {
					...
					if(evt.originalEvent && $(evt.originalEvent.target).closest(".ui-dialog-titlebar-close").length){
						if($csh_net_val<parseFloat($free_product_amount.val())){
							jAlert("ยอดซื้อน้อยกว่า " + parseFloat($free_product_amount.val()).toFixed(2) + " กรุณาตรวจสอบอีกครั้ง","ข้อความแจ้งเตือน",function(){
								window.parent.$(".ui-dialog").remove();
								$('#dlgFreeProduct').dialog('open');
								$("#input_free_product").focus();
								return false;
								...
								var opts_freepremium={
											...
											success:function(data){
												...
												if(c_free_premium_amount>parseFloat($free_premium_amount.val())){
													c_free_premium_amount=parseFloat(c_free_premium_amount)-parseFloat(arr_fp[1]);
													jAlert('มูลค่าสินค้ามากกว่าจำนวนเงินสินค้าพิเศษ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
												...
												if(arr_fp[0]=='4'){
													jAlert('สินค้ารหัสนี้ไม่ร่วมโปรโมชั่น กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
														$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
												if(arr_fp[0]=='1'){
													jAlert('ไม่พบข้อมูลรหัสสินค้านี้ในทะเบียน กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
								           				$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
												if(arr_fp[0]=='2'){
													jAlert('สินค้าสต๊อกไม่พอ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
								           				$input_free_premium.focus().select();
								        				return false;
								        			});	
												}else
													if(arr_fp[0]=='3'){
														jAlert('สินค้าถูกล๊อก กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
									           				$input_free_premium.focus().select();
									        				return false;
									        			});	
												}else if(arr_fp[0]=='5'){
						...
						success:function(data){
							...
							$('.tableNavCatList ul li').not('.nokeyboard').navigate({
							  ...
							  //*WR05022014 OPPN300 Revised
								...
								if(chk_ops=='Y'){
									jAlert('ลูกค้าถือบัตร ' + arr_data[9] + ' อยู่ไม่สามาถถือบัตรซ้ำได้', 'ข้อความแจ้งเตือน',function(){				           				
				        				return false;
				        			});	
									return false;
								}
								...
								if(arr_data[1]!=''){
										var opts={
												...
												success:function(data){	
													...
													if(arr_res[0]=='2'){
														jAlert('จำนวนสินค้าไม่พอ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){															
															//TEST ON 13052012
															$('div.tableNavCatList ul>li').navigate('destroy');
															$("#dlgMemberCatalogList").dialog("close");
...
//																	}
										...
										if(status_no!='01'){
											$("#dlgMemberCatalogList").dialog("close");
											jAlert('เกิดปัญหาการตรวจสอบสถานะบิล \nกรุณายกเลิกการขาย แล้วเลือกบิล 01 อีกครั้ง', 'ข้อความแจ้งเตือน',function(){
												initTblTemp();
												initFormCashier();
												getCshTemp('N');
												browsDocStatus();
	        ...
	        NewDialog.dialog({
	            ...
	            buttons: [
	                {text: "ต่อไป >>", click: function(e) {
	                	 ...
	                	 var chk_idcard_exist=$('#chk_idcard_exist').val();
			             if(chk_idcard_exist.length==0){
			            	 jAlert('กรุณาระบุรหัสบัตรประชาชน', 'ข้อความแจ้งเตือน',function(){
			    	                ...
			    	                click: function(evt){ 
			    	                	 ...
			    	                	 var member_no_by_comp=$('#check_member_no_by_comp').val();
			 				             if(member_no_by_comp.length==0){
			 				            	jAlert('กรุณาระบุรหัสสมาชิกเพื่อตรวจสอบ', 'ข้อความแจ้งเตือน',function(){
			 				            		$('#check_member_no_by_comp').focus();	
												return false;
											});	
			 				            	return false;
			 					         ...
			 					         var opts_chkops={
			 					            		...
			 					            		data:{
			 					            			...
			 					            			if(objMb.status=='Y'){	
			 					            				...
			 					            				jConfirm(str_show,'ยืนยันการใช้สิทธิ', function(r){
			 											        if(r){
			 					            				...
			 					            				str_show=str_show + "<hr> ไม่สามารถใช้โปรโมชั่นนี้ได้ กรุณาตรวจสอบอีกครั้ง \n";			 					            				
			 					            				jAlert(str_show, 'ข้อความแจ้งเตือน',function(){
		 									    				$("#dlgCheckMemberByCompany").dialog('close');
		 														return false;
		 													});	
									...
									$('div.tableNavMemberPromotion2 ul>li').not('.nokeyboard').navigate({
									  ...
									  //*WR06122013 New OPPC300 2014
									    ...
									    if(selpromo.application_id=='OPPC300' && mp_point_sum < 30){
									    	 jAlert('คะแนนไม่พอ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
									    		 $("#dlgRegPromo").dialog("close");
									    		 initTblTemp();
											     initFormCashier();
											     initFieldOpenBill();
				            ...
				            if($refer_member_st_val.length==0){
					            ...
					            if(getOnlineStatus()===1){
					           	...
					           	var opts_memexpire={
						           		...
						           		success:function(data){						           			
							           		if(data==''){
							           			jAlert('ไม่พบรหัสบัตรสมาชิกเก่า กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
							           				initMemberCatalog();//*WR25092012
							           				$refer_member_st.val('').focus();
							        				return false;
							        			});	
									           	...
									           	/*######################################*/
													...
													if(m.link_status=='ONLINE'){
														...
														if(m.exist_status=='NO'){
															jAlert('ไม่พบข้อมูลรหัสสมาชิก กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
																initFormCashier();
																initFieldOpenBill();
																return false;
															});	
											           	...
											           	var opts={
												           		...
												           		success:function(data){												           			
													           		...
													           		if(arr_app[0]=='2'){														           			
													           			...
													           			jAlert('ไม่สามารถต่อชุดสมัครหลังเดือนหมดอายุได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='4'){
													           			jAlert('ไม่สามารถต่อชุดสมัครได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='5'){
													           			jAlert('ยอดเงินน้อยกว่า '+addCommas(arr_app[2])+' ไม่สามารถต่อชุดสมัครได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='6'){														           			
													           			jAlert('ไม่พบรายการชุดสมัคร กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='7'){														           			
													           			jAlert('คะแนนสะสมน้อยกว่า '+arr_cat[8]+' กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='8'){														           			
													           			jAlert('AFTER CASE COMMING SOON.', 'ข้อความแจ้งเตือน',function(){
													           				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='9'){
													           			jAlert('ยอดเงินเท่ากับหรือมากกว่า '+addCommas(arr_app[2])+' ไม่สามารถต่อชุดสมัครได้ กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
														           	}else if(arr_app[0]=='10'){
													           			jAlert('สามารถต่อก่อนเดือนหมดอายุ 1 เดือนหรือภายในเดือนหมดอายุเท่านั้น กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
													        				getMemberCatalog();
													        				return false;
													        			});	
													           			return false;
		...
		*/
		...
		if(member_id.length==0){
			...
			$.getJSON(
					...
					function(data){
						if(data==1){
							jAlert('ไม่พบรหัสสมาชิกในทะเบียน หรือวันที่สมัครไม่ตรงกับวันที่เปิดบิล','ข้อความแจ้งเตือน',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
							...
							jAlert('รหัสสมาชิกเคยใช้สิทธิแล้ว ไม่สามารถใช้สิทธิซ้ำได้','ข้อความแจ้งเตือน',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
							...
							jAlert('เกิดผิดพลาด ไม่สามารถใช้สิทธิซ้ำได้','ข้อความแจ้งเตือน',function(){
								initFormCashier();
								initTblTemp();
								getCshTemp('Y');
								initFieldOpenBill();
	...
	function cardLostStock(){
		...
		*/
		...
		var opts={
					...
					success:function(data){
						if(data=='0'){
							jAlert('รหัสบัตรนี้ไม่พบในทะเบียน หรือต้องเป็นบัตรเสีย-บัตรหายเท่านั้น','ข้อความแจ้งเตือน',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='2'){
							jAlert('รหัสสมาชิกนี้ถูกใช้สมัครแล้ว','ข้อความแจ้งเตือน',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='3'){
							jAlert('กรุณาเลือกบัตร ' + msg_show,'ข้อความแจ้งเตือน',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='4'){
							jAlert('รหัสบัตรนี้ไม่พบในทะเบียน หรือท่านยังไม่ได้ลงทะเบียนบัตร','ข้อความแจ้งเตือน',function(){
								initFieldOpenBill();
								return false;
							});	
						}else{
		...
		var opts={
					...
					success:function(data){
						if(data=='0'){
							...
							jAlert('รหัสบัตรนี้สินค้าคงเหลือไม่พอ กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='6'){
							jAlert('กรุณาเลือกบัตร OPS White Card','ข้อความแจ้งเตือน',function(){
								initFieldOpenBill();
								return false;
							});	
						}else if(data=='7'){
							jAlert('กรุณาเลือกบัตร OPS Gold Card','ข้อความแจ้งเตือน',function(){
								initFieldOpenBill();
								return false;
							});	
						}else{
	...
	function getMemberProfile(col,txtsearch){
		...
		*/
		 $.getJSON(
					...
					function(data){
						if(data=='0'){
							jAlert('ไม่พบข้อมูลสมาชิก', 'ข้อความแจ้งเตือน',function(){
	...
	function getProMemPriv(member_no,birthday,apply_date,expire_date,tabs){
					    ...
					    if(selpromo.promo_code=='NEWBIRTH'){
						    ...
						    $.ajax({
							    ...
							    data:{
								    ...
								    if(data=='0'){
								    	jAlert('ไม่พบข้อมูลโปรโมชั่น กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
								    		$("#other_promotion_cmd").html('');
									    	$('.tableNavMemberPriv ul').navigate('destroy');
		...
		setTimeout(function(){
			if(objReqMemPriv){
				...
				objReqMemPriv.abort();
				objReqMemPriv=null;
				jConfirm('ไม่พบข้อมูลที่ร้องขอจาก Server ลูกค้ายืนยันใช้สิทธิประโยชน์สมาชิก ใช่หรือไม่?','ยืนยันการใช้สิทธิ', function(r){
			        if(r){
...
//									}
								...
								if($('#csh_member_vip').val()!='1' && status_no!='01' && status_no!='02' && status_no!='04' && status_no!='05'  && status_no!='06'){
									jConfirm('คุณต้องการใช้สิทธิประโยชน์สมาชิก ใช่หรือไม่?','ยืนยันการใช้สิทธิ', function(r){
										 if(r){
			...
			setTimeout(function(){
			  ...
			  if (objReqMemberInfo){
				  ...
				  objReqMemberInfo=null;
				  jConfirm('ไม่มีการตอบสนองจากการร้องขอข้อมูล ONLINE\n คุณต้องการเปลี่ยนเป็นโหมด	OFFLINE หรือไม่?','ยืนยันการขอข้อมูล', function(r){
				        if(r){
				        	callMemberOffline();
				        	return false;
				        }else{
	...
	function stockLost(){
		...
		*/
		...
		if(employee_id.length==0){
			jAlert('กรุณาป้อนรหัสพนักงาน', 'ข้อความแจ้งเตือน',function(){
				initFieldOpenBill();
				return false;
			});	
			return false;
	...
	function browsDocStatus(){
		...
		*/
		var dialogOpts_dlgBrowsDocStatus = {
				...
				showOpt: {direction: 'down'},		
				closeOnEscape: true,	
				title:"<span class='ui-icon ui-icon-document'></span>ประเภทบิล",
				open: function(){  
					  $(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
					  $(this).dialog('widget')
			            .find('.ui-dialog-titlebar')
	...
	function initFormCashier(){
		...
		//init doc_tp to status_no 00 SL default
		$('#csh_doc_tp').val('SL');
		$('#csh_doc_tp_show').html('บิลเงินสดอย่างย่อ');
	...
	function swapCashier(swap_cashier_id){
		...
		*/
		$.getJSON(
				...
				function(data){
					if(data==null){
						jAlert('ไม่พบรหัสนี้กรุณาตรวจสอบอีกครั้ง', 'ข้อความแจ้งเตือน',function(){
							return false;
						});	
					}else{
						$.each(data.emp, function(i,m){
							if(m.exist=='yes'){
								if(m.check_in_status=='Y'){
									...
									jAlert('รหัสนี้ไม่อยู่ในสถานะ Login ระบบ', 'ข้อความแจ้งเตือน',function(){
										return false;
									});	
								}
							}							
		...
		var opts={
				...
				success:function(data){
					...
					if(parseInt(arr_data[0])>0){		
						...
						jAlert('ไม่พบรายการขาย กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){
							return false;
		    			});
				    	ret= 'N';
					}
...
//			}
		...
		$("#btnBwsMemberBuy").click(function(e){
			...
			$("<div id='dlgMemberBuy'></div>").dialog({
			     ...
			     width:'50%',
			     height:350,
			     title:"<span class='ui-icon ui-icon-person'></span>รายการซื้อ",
			     open:function(){
			    	 $(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
						$(this).dialog('widget')
				            .find('.ui-dialog-titlebar')
...
//		$("#csh_product_id").numericFilter();
		...
		$("#btn_cal_promotion").click(function(e){
			...
			if(chk_percent_discount.length>0 && chk_member_no.length==0){
				jAlert('ไม่พบรหัสสมาชิก กรุณาป้อนรหัสสมาชิก กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){					
					return false;
    			});
				return false;
			}			
			...
			if(jQuery.trim($("#csh_status_no").val())=='01'){
			   						...
			   						$('#ecp_amount').keypress(function(evt){
					   			        ...
					   			        if(key == 13){
						   			        ...
						   			        if(parseFloat(chk_ecp_amount_bal)>parseFloat(ecoupon_amount_balance)){
						   			        	jAlert('ไม่สามารถใช้สิทธิมากกว่าวงเงินที่เหลือได้ กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){
						   			        		$('#ecp_amount').val(ecp_amount_bal).focus();
						   							return false;
						   		    			});
							   			        return false;
				...
				if($promo_tp.val()=='N'){
					...
					if($csh_net<$start_baht){
						jAlert('ยอดเงินสุทธิน้อยกว่า '+$start_baht+' กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){
							$("#btn_cal_promotion").removeAttr("disabled");
							$('#btn_cal_promotion').removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
							$("#csh_product_id").focus();
		    			});
								...
								$.ajax({
										...
										success:function(data){
											if(data=='3'){
												...
												jAlert('!!! ยอดเงินสุทธิจากการคำนวณส่วนลด พบข้อผิดพลาด !!!\nกรุณาลบรายการสินค้าที่ราคาสูงสุดออก และสแกนสินค้าชิ้นนั้นอีกครั้ง','ข้อความแจ้งเตือน',function(){	
													rePromoPointDiscount();
													$("#btn_cal_promotion").removeAttr("disabled");
				    								$("#btn_cal_promotion").removeClass('ui-state-focus ui-state-access ui-state-disabled').addClass('ui-state-default');
													return false;
            							...
            							success:function(data){
            								...
            								$('.tableNavPercentRange ul li').not('.nokeyboard').navigate({
											    ...
											    if(point>point_total){
											    	jAlert('คะแนนสะสมไม่พอ กรุณาตรวจสอบอีกครั้ง','ข้อความแจ้งเตือน',function(){													
														return false;
									    			});
											    	return false;
											    }
		...
		$("#tbl_cashier").flexigrid(
				{
					...
					colModel : [
						...
						{display: 'สถานะโปร', name : 'promo_st', width : 70, sortable : true, align: 'center'},
						{display: 'โปรโมชั่น', name : 'promo_code', width : 90, sortable : true, align: 'center'},
						...
						{display: 'รวมเงิน', name : 'amount', width :90, sortable : false, align: 'center'}
						],
					sortname: "id",
					sortorder: "asc",
					action:'gettmp',
		...
		$('#btnCoupon').button({
	    	...
	    	var onsales=onSales();
		    if(onsales>0){
			 	jAlert('คุณทำรายการขายค้างอยู่ ไม่สามารถเลือกโปรโมชั้นหลังทำรายการขายได้', 'ข้อความแจ้งเตือน',function(){	            	
	            	return false;
		        });
			 	return false;
			}
		    			    		 ...
		    			    		 $('.tableNavCouponPro ul li').not('.nokeyboard').navigate({
										  ...
										  //*WR27102014
										    ...
										    if(sel_promo.member_tp == 'Y' && member_no.length<1){
										    	jAlert('กรุณาสแกนรหัสสมาชิกก่อนเล่นโปรโมชั่น', 'แจ้งเตือน',function(){
														    	            ...
														    	            { 
														    	                ...
														    	                click: function(evt){ 
																		    		...
																		    		if(coupon_code.length != 12){
																	    				jAlert('กรุณาป้อนรหัส Coupon ให้ครบ 12 หลัก', 'แจ้งเตือน',function(){
																		    		...
																		    		if(chk_coupon_disocunt != parseInt(sel_promo.discount)){
																		    			jAlert('รหัส Coupon ' + coupon_code + ' ไม่ตรงกับโปรโมชั่นส่วนลด ' +  parseInt(sel_promo.discount) + "%", 'แจ้งเตือน',function(){
																		    		...
																		    		$.ajax({
																		    			...
																		    			data:{
																		    				...
																		    				if(arr_data[0]=='Y'){
																		    					//is used
																		    					jAlert(arr_data[1], 'แจ้งเตือน',function(){
																		    						initTblTemp();
																									initFormCashier();
																									initFieldOpenBill();
																									$('#coupon_code2').focus();
...
//																					});
		...
		$('#btnClosePageCashier').button({
			...
			e.preventDefault();
			jConfirm('คุณต้องการปิดหน้าขายใช่หรือไม่?', 'ยืนยันการปิดหน้าขาย', function(r) {
			    if(r){	
...
//									});	
		...
		$('#btn_trn_alipay').button({
            ...
            dlgTrnAlipay.dialog({
			   ...
			   modal:true,
			   resizable:false,
	           title: "รายการชำระผ่าน Aplipay วันนี้",		
	           position:"center",
	            open:function(){		
	            	$(this).dialog('widget')
		            .find('.ui-dialog-titlebar')
		...
		$('#btnPromotionData').click(function(e){
			...
			var opts_dlgPromoItems = {
					...
					showOpt: {direction: 'up'},		
					closeOnEscape: true,	
					title:"<span class='ui-icon ui-icon-cart'></span>รายการโปรโมชั่น",
					open: function(){  
	    ...
	    */
								...
								$('#dlgCoOperation').empty().html(data);	
					    		 $('.tableNavSelCooperation ul li').not('.nokeyboard').navigate({
								    ...
								    if(promo_play=='TOUR01'){				    					
				    					...
				    					if(member_type!=='WALK IN'){
				    						jAlert('โปรโมชั่นนี้สำหรับลูกค้า WALK IN เท่านั้น', 'แจ้งเตือน',function(){										    	
										    	return false;
										    });	
										    return false;
				    					}else{
		...
		$('#btnBillDetail').click(function(evt){
			...
			var opts_dlgBillDetail={
					...
					position:'center',
					closeOnEscape: true,
					title:"<span class='ui-icon ui-icon-document-b'></span>บิลประจำวัน",
					open:function(){
						$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
						$(this).dialog('widget')
				            .find('.ui-dialog-titlebar')
		...
		$('#btnContact').click(function(evt){
			...
			var opts_dlgContact={
					...
					position:'center',
					closeOnEscape: true,
					title:"<span class='ui-icon ui-icon-person'></span>เบอร์ติดต่อ",
					open:function(){
						$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
						$(this).dialog('widget')
				            .find('.ui-dialog-titlebar')
		...
		$('#btnMemberData').click(function(evt){
			...
			var opts_dlgMemberData={
					...
					position:'center',
					closeOnEscape: true,
					title:"<span class='ui-icon ui-icon-person'></span>ข้อมูลสมาชิก",
					open:function(){
						$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
						$(this).dialog('widget')
				            .find('.ui-dialog-titlebar')
		...
		$("#btnQstPdtOfBill").click(function(e){
			...
			$("<div id='dlgQstPdtOfBill'></div>").dialog({
		     	   autoOpen:true,
						...
						closeOnEscape: true,
						title:"<span class='ui-icon ui-icon-cart'></span>สอบถามข้อมูลสินค้ารายบิล",
						open: function(){    
							$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
							$(this).dialog('widget')
					            .find('.ui-dialog-titlebar')
						    ...
						    $(this).parents(".ui-dialog:first").find(".ui-dialog-content").css({"background-color":"#ebebeb",
							    																			...
							    																			"padding":"5 0 0 0"});	
			   				$("#dlgQstPdtOfBill").append("&nbsp;&nbsp;รหัสสินค้า&nbsp;:&nbsp;<input type='text' id='adt_product_id' size='20' class='input-text-pos'/><hr style='border:0pt solid#e0e0e0;'>");
			   				...
			   				$("#adt_product_id").keypress(function(evt){
					   			        ...
					   			        if(key == 13){	
						   			        ...
						   			        if(product_id.length==0){
						   			        	jAlert('กรุณาระบุรหัสสินค้า', 'กรุณาตรวจสอบอีกครั้ง',function(){		
						   			        		$("#adt_product_id").focus();																				
													return false;
												});	
							   			    }else{
		...
		$("#btnCheckStock").click(function(e){
		    ...
		    var dialogOpts_CheckStock = {
					...
					showOpt: {direction: 'up'},		
					closeOnEscape:false,	
					title:"<span class='ui-icon ui-icon-cart'></span>สินค้าคงเหลือ",
					open: function(){     	
							$(this).parents(".ui-dialog:first").css({"padding":"3px","margin":"0 0 0 0","border-color":"#C6D5DC"});
							$(this).dialog('widget')
					            .find('.ui-dialog-titlebar')
...
//		    			                      .css({"padding":".1em .1em .1em 0","margin":"0 0 0 0","background-color":"#C7D9DC"} )
								...
								"ตกลง":function(){ 	
									...
									if(taxid_branch=='b'){
										...
										if(vt_name!='' && taxid_branch_seq==''){
											jAlert('กรุณาระบุลำดับสาขา','ข้อความแจ้งเตือน',function(){
						            ...
						            if(csh_member_no.length>0 && vt_passport_no.length>0){
						            	 jAlert('ลูกค้าที่เป็นสมาชิกอยู่แล้ว ไม่สามารถคีย์ Passport ได้','ข้อความแจ้งเตือน',function(){		
						            		 $('#vt_passport_no').val('');
								             return false;
									      });
						            	 return false;
		 ...
		 $('#btnGreenBag').click(function(e){	    	
		    	...
		    	var net_amt=$('#csh_net').val().replace(/[^\d\.\-\ ]/g,'');
		 		...
		 		var promo_tp=$('#csh_promo_tp').val();
		 		if(status_no=='02'){
					jAlert('บิล 02 (ซื้อครั้งแลกส่วนลดพิเศษ) ไม่ร่วมโปรโมชั่น', 'ข้อความแจ้งเตือน',function(){	            
						$('#csh_product_id').focus();
		            	return false;
			        });
					return false;
				}else if(status_no=='04' && net_amt<1){
					jAlert('บิล 04 (สมาชิกแลกคะแนน) กรุณาสแกน Barcode ถุงที่จะใช้\nหลังจากสแกนรายการสินค้าที่ร่วมโปรโมชั่นแล้ว', 'ข้อความแจ้งเตือน',function(){	            
						$('#csh_product_id').focus();
		            	return false;
			        });
					return false;
...
//				}
							 		...
							 		if(g1!=''){
							 			...
							 			$.ajax({
							 				...
							 				data:{
							 					...
							 					if(data=='O'){
							 					 	jAlert('ไม่สามารถเล่นโปรนี้ได้เกิน 5 ครั้งต่อวัน', 'ข้อความแจ้งเตือน',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
							 					}else if(data=='N'){
							 					 	jAlert('ไม่พบรหัสถุงหรือรหัสถุงนี้ไม่ใช่ของสมาชิก', 'ข้อความแจ้งเตือน',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
							 					}else if(data=='R'){
							 						jAlert('พบรายการรหัสถุงแล้ว ไม่สามารถใช้มากกว่า 1 ถุงต่อบิลได้', 'ข้อความแจ้งเตือน',function(){	       
							 					 		initFieldPromt();
										            	return false;
											        });
							 					 	return false;
	    ...
	    $('#btnSmsPro').click(function(e){
	    			    		...
	    			    		data:{
	    			    			...
	    			    			$('.tableNavSmsPro ul>li').not('.nokeyboard').navigate({
									    ...
									    if(sel_promo.member_tp=='Y' && $.trim(csh_member_no).length==0){
									    	jAlert('กรุณาระบุรหัสสมาชิกก่อน','ข้อความแจ้งเตือน',function(){
												 return false;
											  });
									    	return false;
									    }
	    	...
	    	var opts_ctemp={
					...
					success:function(data){
						var arr_data=data.split('#');
						if(parseInt(arr_data[0])>0){		
							jAlert('ท่านกำลังทำรายการขาย ไม่สามารถใช้ SMS Pro ได้','ข้อความแจ้งเตือน',function(){
								 return false;
							  });
						}else{
							///////////////
		...
		$('#btnCancle').click(function(e){
			e.preventDefault();
			jConfirm('คุณต้องการยกเลิกรายการขายใช่หรือไม่?', 'ยืนยันการยกเลิกบิล', function(r) {
			    if(r){	
	            ...
	            if(member_no.length>0){
	            	var opts_ontrans={
	            			...
	            			data:{
	            				...
	            				rmd:Math.random()
	            					...
	            					//---------------------------------------------------------------------
	            					if(mdata[0]=='Y'){
	            	            		jAlert("สมาชิกไม่สามารถทำรายการพร้อมกันมากกว่า 1 เครื่องได้","ข้อความแจ้งเตือน",function(){
	            			    			$("#csh_member_no").focus();
	            			            	return false;
	            				        });
	            	            	}else if(mdata[0]=='T'){
	            	            		...
	            	            		//satart old
	            	            		 if(csh_status_no=='01'){
		            	     			    	...
		            	     			    	if(getOnlineStatus()===1){	
		            	     			    		...
		            	     			    		//callMemberOffline();
		            	     			    		jAlert("ไม่สามารถแลกคะแนนแบบ OFFLINE ได้","ข้อความแจ้งเตือน",function(){
	            	     				    			$("#csh_member_no").focus();
	            	     				            	return false;
	            	     					        });
	            	     				    		return false;
		            	     			    	...
		            	     			    	if($("#cn_member_no").val()!=''){
		            	     			    		if($("#csh_member_no").val()==''){
		            	     				    		...
		            	     				    		jAlert("รหัสสมาชิกไม่ตรงกับรหัสมาชิกในบิลลดหนี้(CN)","ข้อความแจ้งเตือน",function(){
		            	     				    			$("#csh_member_no").focus();
		            	     				            	return false;
		            	     					        });
		            	     				    		return false;
	            	...
	            	if(csh_status_no=='02'){
	            		var key_employee=$.trim($("#csh_member_no").val());
			            	...
			            	var key_employee=$.trim($("#csh_member_no").val());
		     			   	if(key_employee.length==0){
						    		jAlert("กรุณาระบุ รหัสพนักงาน","ข้อความแจ้งเตือน",function(){
						    			$("#csh_member_no").focus();
						            	return false;
							        });
						    		return false;
			            	...
			            	var key_employee=$.trim($("#csh_member_no").val());
					    		...
					    		jAlert("กรุณาระบุ รหัสบัตรเสีย/บัตรหาย","ข้อความแจ้งเตือน",function(){
					    			$("#csh_member_no").focus();
					            	return false;
						        });
									  ...
									  //*WR02062016 for support mini bueaty book 2016
									    ...
									    if(sel_promo.promo_code=='50BTO1P'){
									    	...
									    	if(csh_net_to_check<1){
									    		jAlert("ไม่พบรายการขาย ไม่สามารถเล่นโปรโมชั่นนี้ได้","ข้อความแจ้งเตือน",function(){	
													 initFieldPromt();												
									            	return false;
										        });
									    		return false;
									    	...
									    	if(mp_status_no=='02'){
									    		jAlert("โปรโมชั่น More Point ไม่สามารถเล่นร่วมกับบิล 02(First Purchase) ได้","ข้อความแจ้งเตือน",function(){	
									            	return false;
										        });
									    		return false;
									    	}
																				       			...
																				       			$(this).click(function(){
																				       				...
																				       				if(strJson!=''){
																						       				...
																						       				if(seldocno.status_receive=='Y'){
																						       					jAlert("รับสินค้าแล้ว","ข้อความแจ้งเตือน",function(){																					
																													return false;
																										        });
																						       					return false;
																						       				}
																						       				if(seldocno.flag=='C'){
																						       					jAlert("เอกสารนี้ถูกยกเลิก","ข้อความแจ้งเตือน",function(){																					
																													return false;
																										        });
																							       				return false;
																						       				}
														...
														$('#btnSubmitTemp').click(function(e){
														    ...
														    if(onsales>0){
																...
																$('#dlgDocNoTemp').dialog('close');
															 	jAlert('ไม่สามารถคืนสินค้าซ้ำได้', 'ข้อความแจ้งเตือน',function(){															 		
																	getCshTemp('N');
																	paymentBill('00');
													            	return false;
														        });
															    	  ...
															    	  $("#key_quantity_return").keypress(function(evt){	
															    		  ...
															    		  if(key==13){
																    		  ...
																    		  if(key_quantity_return.length==0 || key_quantity_return=='0'){
															    				  jAlert("กรุณาคีย์จำนวนสินค้า","ข้อความแจ้งเตือน",function(){				
															    			  ...
															    			  if(key_quantity_return>limited_qty){
															    				  jAlert("ไม่สามารถคีย์จำนวนสินค้าได้มากกว่า " + limited_qty + " รายการ","ข้อความแจ้งเตือน",function(){				
															    					   $("#key_quantity_return").val('1').focus();
																						return false;
																			        });
															    				  return false;
															    	  ...
															    	  $("#key_product_return").keypress(function(evt){															    		 
															    		  ...
															    		  if(key==13){
															    			  ...
															    			  if(key_product_return.length==0){
															    				  jAlert("กรุณาคีย์รหัสสินค้า","ข้อความแจ้งเตือน",function(){				
															    					   $("#key_product_return").val('').focus();
																						return false;
																			        });
															    			  }
															...
															var opts_chk={
																	...
																	data:{
																		...
																		if(data=='1'){
																			//ไม่พบ
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบเลขที่เอกสารนี้ หรือเลขที่เอกสารนี้ไม่ร่วมโปรโมชั่น","ข้อความแจ้งเตือน",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='2'){
																			...
																			jAlert("รหัสบัตรสมาชิกไม่ตรงตามเลขที่เอกสารนี้","ข้อความแจ้งเตือน",function(){																						
																				return false;
																	        });
																			return false;
																		}else if(data=='3'){			
																			//รับไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("บิลนี้ไม่ได้ติดสินค้า short <br>ไม่สามารถคืนได้","ข้อความแจ้งเตือน",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='4'){
																			...
																			jAlert("ยอดซื้อน้อยกว่า 2000 ไม่สามารถรับโปรนี้ได้","ข้อความแจ้งเตือน",function(){																					
																				return false;
																	        });
																			return false;
																		}else if(data=='5'){	
																			...
																			jAlert("ยอดซื้อน้อยกว่า 3000 ไม่สามารถเล่นโปรนี้ได้","ข้อความแจ้งเตือน",function(){																						
																				return false;
																	        });
																			return false;
																		}else if(data=='6'){																				
																			...
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบโปรโมชั่นที่ร่วมโปร","ข้อความแจ้งเตือน",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='8'){			
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่ใช่บิล 01 บัตรใหม่หรือบัตรต่ออายุ","ข้อความแจ้งเตือน",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='9'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("bill ทีมีการตรวจสอบยอดซื้อ","ข้อความแจ้งเตือน",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='10'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบเลขที่เอกสาร หรือเลขที่เอกสารไม่อยู่ในช่วงเวลาที่กำหนด","ข้อความแจ้งเตือน",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='11'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("ไม่พบโปรในตาราง promo_short","ข้อความแจ้งเตือน",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='12'){																				
																			//รับสินค้าไปแล้ว
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("เลขที่เอกสารนี้ถูกยกเลิกแล้ว","ข้อความแจ้งเตือน",function(){																				
																				return false;
																	        });
																			return false;
																		}else if(data=='13'){																				
																			...
																			$('#dlgDocNoTemp').dialog('close');
																			jAlert("บิลนี้ไม่เป็นไปตามเงื่อนไขโปรโมชั่น ไม่สามารถคืนสินค้าได้","ข้อความแจ้งเตือน",function(){																				
																				return false;
																	        });
																			return false;
																		}else{
											  		 ...
											  		 $.ajax({
											  			 ...
											  			 data:{
											  				 ...
											  				 if(data=='Y'){
											  					...
											  					jAlert("ไม่พบข้อมูลหรือลูกค้ารับสินค้าไปแล้ว", 'ข้อความแจ้งเตือน',function(){
											  						initFieldOpenBill();
											  		            	return false;
											  			        });
											  				 }
	    ...
	    $("#btnDocType").click(function(evt){
		    ...
		    var onsales=onSales();
		    if(onsales>0){
			 	jAlert('คุณทำรายการค้างอยู่ ไม่สามารถเปลี่ยนประเภทเอกสารได้', 'ข้อความแจ้งเตือน',function(){
	            	$("#csh_product_id").focus();
	            	return false;
		        });
			}else if(onsales==0){			

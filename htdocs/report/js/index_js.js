...
$(function() {
	...
	$(".nexttxt").live("keydown",function(event){
		...
		if(keyCode==13){
			$(this).parents().parents().next().find('.nexttxt').focus();
			...
			$.ajax({type: "POST",url: "/stock/checkstock/getdoconprintlisttag",data:form,success:
				...
				var data = data.split(':::::');
					if(data[1]=="n"){
						...
						$("#shlef").val(data[1]);
						$("#doc_no").val(data[0]);
						var page="printlisttagold?doc_no_old="+data[0];
...
function print_doc_no_old(){
	...
	var page="printlisttagold?doc_no_old="+doc_no_lod;
	window.open(page, '_blank');
}
...
function search_product_listtag(event){
	...
	if(keyCode==13){
		/*if(s_product_id==""){
			jAlert('คุณยังไม่คีย์รหัสสินค้า!', 'แจ้งเตือน');

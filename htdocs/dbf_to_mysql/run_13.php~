...
if(!file_exists("$db_path")){
		system("touch /home/shopsetup/dbf_to_mysql/pook13.txt");
...
$sql_del="TRUNCATE TABLE pos_ssup.com_expense_head";
mysql_query($sql_del,$conn);

//--------------------------------------------------------------------------
$sql_uu="INSERT INTO pos_ssup.com_expense_head(`corporation_id`, `company_id`, `branch_id`, `branch_no`, `doc_dt`, `employee_id`, `amount`, `remark`, `reg_date`) 
SELECT   `BRAND`, `BRAND`,`SHOP`, `SHOP`,`DOC_DT`,`EMP_ID`, `AMOUNT`, `REMARK` 
FROM  `$tbl`  WHERE  1 ";

...
$sql="select * from trn_diary1_or where status_transfer_ji='' ";
...
if($num>0){
	while($dat=mysql_fetch_array($rs)){
		...
		";
		echo $sql_insert_or_head."<br>";
		$rs_or_head=mysql_db_query($db,$sql_insert_or_head,$local_ji);
		if($rs_or_head){
			
			$sql_up_1="update trn_diary1_or set status_transfer_ji='Y',date_transfer_ji=NOW() where doc_no='$dat[doc_no]' ";
			$rs_up_1=mysql_db_query($db,$sql_up_1,$local);
			
			if($rs_up_1){
				$sql_select_or_list="select * from trn_diary2_or where doc_no='$dat[doc_no]' ";
				$rs_select_or_list=mysql_db_query($db,$sql_select_or_list,$local);
				while($dat_list=mysql_fetch_array($rs_select_or_list)){
					$sql_insert_or_list="
					insert into 
						trn_diary2_or 
					set 
					...
					";
					echo $sql_insert_or_list."<br>";
					$rs_or_list=mysql_db_query($db,$sql_insert_or_list,$local_ji);
					if($rs_or_list){
						$sql_up_2="update trn_diary2_or set status_transfer_ji='Y',date_transfer_ji=NOW() where doc_no='$dat[doc_no]' and product_id='$dat_list[product_id]' ";
						echo $sql_up_2."<br>";

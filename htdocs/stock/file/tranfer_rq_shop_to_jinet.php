...
$sql="select * from trn_diary1_rq where status_transfer_ji='' and flag<>'C' and quantity>'0' ";
...
if($num>0){
	while($dat=mysql_fetch_array($rs)){
		...
		";
		echo $sql_insert_rq_head."<br>";
		$rs_rq_head=mysql_db_query($db,$sql_insert_rq_head,$local_ji);
		if($rs_rq_head){
			
			$sql_up_1="update trn_diary1_rq set status_transfer_ji='Y',date_transfer_ji=NOW() where doc_no='$dat[doc_no]' ";
			$rs_up_1=mysql_db_query($db,$sql_up_1,$local);
			
			if($rs_up_1){
				$sql_select_rq_list="select * from trn_diary2_rq where doc_no='$dat[doc_no]' ";
				$rs_select_rq_list=mysql_db_query($db,$sql_select_rq_list,$local);
				while($dat_list=mysql_fetch_array($rs_select_rq_list)){
					$sql_insert_rq_list="
					insert into 
						trn_diary2_rq 
					set 
					...
					";
					echo $sql_insert_rq_list."<br>";
					$rs_rq_list=mysql_db_query($db,$sql_insert_rq_list,$local_ji);
					if($rs_rq_list){
						$sql_up_2="update trn_diary2_rq set status_transfer_ji='Y',date_transfer_ji=NOW() where doc_no='$dat[doc_no]' and product_id='$dat_list[product_id]' ";
						echo $sql_up_2."<br>";
...
$sql="select * from trn_diary1_iq where status_transfer_ji='' and flag<>'C' and quantity>'0' ";
$rs=mysql_db_query($db,$sql,$local);
$num=mysql_num_rows($rs);
if($num>0){
	while($dat=mysql_fetch_array($rs)){

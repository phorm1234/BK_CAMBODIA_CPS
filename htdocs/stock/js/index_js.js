				...
				if(r==true){
					...
					function(data){
						if(data=="y"){
							...
							jAlert('โอนข้อมูลยอดขาดเกินเรียบร้อยแล้ว!', 'Alert');
						}else{
							$( "#dialog_process" ).dialog( "close" );
							jAlert('ไม่สามารถดำเนินการได้!', 'Alert');
						}
					}
					});
				}
...
function save_shelf(){
	...
	if(shelf_no==""){
		jAlert('You did not insert Shelf No.', 'Alert',function(r) {
			$("#shelf_no").focus();
			return  false;
		});
		
	}else{
		jConfirm("Save Data", 'Confirm Process', function(r) {
			if(r==true){
				...
				$.ajax({type: "POST",url: "/stock/checkstock/saveshelf",data:{chk_func_save:chk_func_save,shelf_desc:shelf_desc,shelf_no:shelf_no,shelf_no_up:shelf_no_up},success:
					function(data){
						if(data=="y"){
							...
							jAlert('This Shelf No. already exists!', 'Alert');
						}else{
							jAlert('Error Can not save!', 'Alert');
						}
					}
				}); 
			}
...
function save_shelf_ck(){
	...
	var shelf_no=$("#shelf_no").val();
	if(shelf_no==""){
		jAlert('You did not inser Shelf No.', 'Alert', function() {
			$("#shelf_no").focus();
			return false;
		});
	}else{
...
function del_shelf(shelf_no){
	var txt="Confirm Delete Shelf No. "+shelf_no;
	...
	jConfirm(txt, 'Confirm Process', function(r) {
		if(r==true){
			$.ajax({type: "POST",url: "/stock/checkstock/deleteshelf",data:{shelf_no:shelf_no},success:
				function(data){
					if(data=="n"){
						jAlert('Error Can not Delete!', 'Alert');
						//alert("เกิดความผิดพลาดไม่สามารถลบได้!");	
						return false;
					}else if(data=="w"){
						//alert("ยังมีชั้นวางสินค้าไม่สามารถลบได้!");
						jAlert('The Floor are available! Can not delete', 'Alert');
					}else{
						view();
					}
				}
...
function save_floor(){
	...
	if(floor_no==""){
		jAlert('You did not insert Floor No.', 'Alert');
		$("#floor_no").focus();
		return  false;
	}else{
		jConfirm('Save Data', 'Confirm Process', function(r) {
				...
				$.ajax({type: "POST",url: "/stock/checkstock/savefloor",
					...
					function(data){
						if(data=="y"){
							...
							jAlert('This Floor No. already exists!', 'Alert');
						}else{
							jAlert('Error Can not Save!', 'Alert');
						}
					}
...
function del_floor(shelf_no,floor_no){
	var txt="Confirm Delete Floor No. "+floor_no;
	jConfirm(txt, 'Confirm Process', function(r) {
		if(r==true){
...
function save_room(){
	...
	if(room_no==""){
		jAlert('You did not insert Room No.', 'Alert');
		$("#room_no").focus();
		return  false;
	}else{
		jConfirm('Save Data', 'Confirm Process', function(r) {
				...
				$.ajax({type: "POST",url: "/stock/checkstock/saveroom",
					...
					function(data){
						if(data=="y"){
							...
							jAlert('This Room No. already exists!', 'Alert');
						}else{	
							jAlert('Error can not Save!', 'Alert');
						}
					}
...
function del_room(shelf_no,floor_no,room_no){
	var txt="Confirm Delete Room No. "+room_no;
	jConfirm(txt, 'Confirm Process', function(r) {
		if(r==true){
			$.ajax({type: "POST",url: "/stock/checkstock/deleteroom",data:{shelf_no:shelf_no,floor_no:floor_no,room_no:room_no},success:
				function(data){
					if(data=="n"){
						...
						jAlert('There are products in the shelves can not be removed!', 'Alert');
						return false;
					}else{
						view();
					}
...
function print_shelf(){
			...
			$.ajax({type: "POST",url: "/stock/checkstock/printshelf",data:form,success:
				function(data){
					var data = data.split(':::::');
					if(data[1]=="n"){
						jAlert('You did not select Shelf!', 'Alert');
						return false;
					}else{
						var txt_confirm_print="Click OK to Print";
						jConfirm(txt_confirm_print, 'Confirm Process', function(r) {
							if(r==true){
								$("#shlef").val(data[1]);
								$("#doc_no").val(data[0]);
			...
			$.ajax({type: "POST",url: "/stock/checkstock/getdoconprintlisttag",data:form,success:
				function(data){
					var data = data.split(':::::');
					if(data[1]=="n"){
						jAlert('คุณยังไม่เลือก Shelf!', 'Alert');
						return false;
					}else{
	...
	$.ajax({type: "POST",url: "/stock/checkstock/checkrunck",data:{},success:
		function(data){
			if(data=="y"){
						...
						$.ajax({type: "POST",url: "/stock/checkstock/getdoconprintlisttag",data:form,success:
							...
							var data = data.split(':::::');
								if(data[1]=="n"){
									...
									$( "#dialog_process" ).dialog( "close" );
									var txt_confirm_print="doc. no. : "+data[0]+" click OK to print";
									jConfirm(txt_confirm_print, 'Confirm Process', function(r) {
										if(r==true){
											$("#shlef").val(data[1]);
											$("#doc_no").val(data[0]);
				...
				jAlert('Today the document number was run, can not process', 'Alert', function(r){return false;});
			}else{
...
function print_doc_no(){
	$.ajax({type: "POST",url: "/stock/checkstock/checkdocno",data:{},success:
		function(data){
			if(data=="n"){
				jAlert('No doc. no. has been run.', 'Alert');
			}else{
...
function tranfer_stock(){
	...
	jConfirm("Confirm Adjust Stock", 'Confirm Process', function(r) {
		if(r==true){
			$.ajax({type: "POST",url: "/stock/checkstock/tranferstock",data:{doc_no:doc_no},success:
				function(data){
					if(data=="y"){
						jAlert('Adjust Stock Success!', 'Alert');
					}else{
						jAlert('Can not process!', 'Alert');
						return false;
					}
				}
			});
...
function go_tranfer_room(){
	...
	if(new_room==""){
		jAlert('you did not select Room!', 'Alert');
	}else{
		var txt="Confirm Transfer!";
		jConfirm(txt, 'Confirm Process', function(r) {
		if(r==true){
			var form=$('form#frm_tranfer_product').serialize();

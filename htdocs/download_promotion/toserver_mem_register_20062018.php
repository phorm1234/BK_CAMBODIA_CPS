...
$ji_ip="10.100.59.151";
$ji_user="crm-cps-kh";
$ji_pass="kgPCZNjRWsAbG7feVcyPabJDyag9Y4sh";
$ji_db="pos_ssup_cpskh";

$ji_db_sevice="service_pos_cpskh";
...
$find="select * from member_register where sendtoserver_status='' and reg_date between date(now())- interval 10 day  and date(now()) ";
...
for($i=1; $i<=$rows_find; $i++){
	...
	set
		...
		`send_company`='$data_find[send_company]',
		`send_address`='$data_find[send_address]',
		`send_mu`='$data_find[send_mu]',
		`send_home_name`='$data_find[send_home_name]',
		`send_soi`='$data_find[send_soi]',
		`send_road`='$data_find[send_road]',
		`send_tambon_id`='$data_find[send_tambon_id]',
		`send_tambon_name`='$data_find[send_tambon_name]',
		`send_amphur_id`='$data_find[send_amphur_id]',
		`send_amphur_name`='$data_find[send_amphur_name]',
		`send_province_id`='$data_find[send_province_id]',
		`send_province_name`='$data_find[send_province_name]',
		`send_postcode`='$data_find[send_postcode]',
		`send_tel`='$data_find[send_tel]',
		`send_mobile`='$data_find[send_mobile]',
		`send_fax`='$data_find[send_fax]',
		`send_remark`='$data_find[send_remark]'		

	";
	mysql_select_db($ji_db_sevice);
	echo $add_register . "<br>";
	$run_add_register=mysql_query($add_register,$conn_ji) or die(mysql_error());
	if($run_add_register==true){
		//mark
		$mark_register="update member_register set sendtoserver_status='Y', sendtoserver_date=date(now()), sendtoserver_time=time(now()) where customer_id_new='$data_find[customer_id_new]' ";
		mysql_select_db($dbname);
		$run_mark_register=mysql_query($mark_register,$conn_shop);
	}	
...
mysql_query("SET character_set_client=utf8");
mysql_query("SET character_set_connection=utf8");
$find_card="select * from member_history where sendtoserver_status=''  and reg_date between date(now())- interval 10 day  and date(now()) ";
mysql_select_db($dbname);
...
for($x=1; $x<=$rows_find_card; $x++){
	...
	if($run_add_card==true){
		...
		//mark
		$mark_card="update member_history set sendtoserver_status='Y', sendtoserver_date=date(now()), sendtoserver_time=time(now()) where customer_id_new='$data_card[customer_id_new]' and member_no='$data_card[member_no]' ";
		mysql_select_db($dbname);
		$run_mark_card=mysql_query($mark_card,$conn_shop);
	}
}

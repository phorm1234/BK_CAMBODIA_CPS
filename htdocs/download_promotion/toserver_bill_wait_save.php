...
$conn_ji=mysql_connect($ji_ip,$ji_user,$ji_pass) or die("The connection has timed out");
...
for($i=1; $i<=$rows; $i++){
			...
			$chk_have="select * from trn_diary1 where doc_no='$doc_no' ";
			$conn_ji=mysql_connect($ji_ip,$ji_user,$ji_pass);
			mysql_query("SET character_set_results=utf8");
			mysql_query("SET character_set_client=utf8");
			mysql_query("SET character_set_connection=utf8");
			mysql_select_db($ji_db_sevice);
			$run=mysql_query($chk_have,$conn_ji);
			$rows_have=mysql_num_rows($run);
				...
				if($result_insert){
					$up="update service_log set rechk='Y',rechk_up=now() where type_case='HEAD' and rechk='' and doc_no='$doc_no'  ";
					$run_up=mysql_db_query($dbname,$up,$conn_shop);
				}
...
echo "<script>window.location='toserver_bill_wait.php';</script>";

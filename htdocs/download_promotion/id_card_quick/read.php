...
function onreadidcheck(){
	...
	document.getElementById("hbd_day").disabled = true;
	document.getElementById("hbd_month").disabled = true;
	document.getElementById("hbd_year").disabled = true;
...
function tab1() {
	...
	/*var pathajax=pathidcard+"from_search.php";
	$.get(pathajax,{
...
function showtambon(var_tambon) {
	var pathajax=pathidcard+"find_tambon.php";
	$.get(pathajax,{
					send_province_id:$("#send_province_id").val(),
					send_amphur_id:$("#send_amphur_id").val(),
					var_tambon:var_tambon,
					ran:Math.random()},function(data){
						$("#showsend_tambon").html(data);
						document.getElementById("send_tambon_id").value =var_tambon;
						

	}); 
}

function showamphur_edit(var_amphur,var_tambon) {
	var pathajax=pathidcard+"find_amphur.php";
	$.get(pathajax,{
					send_province_id:$("#send_province_id").val(),
					var_amphur:var_amphur,
					ran:Math.random()},function(data){
						$("#showsend_amphur").html(data);
						document.getElementById("send_amphur_id").value =var_amphur;
						showtambon(var_tambon);
...
function showprovince() {
	var pathajax=pathidcard+"find_province.php";
	$.get(pathajax,{
					ran:Math.random()},function(data){
						$("#showsend_province").html(data);
...
<ul id="tabs">
    <li ><a href="#" title="tab1" onclick="$('#tab_num').val('1');tab1();">ข้อมูลส่วนตัว</a></li>
    <li><a href="#" title="tab2" onclick="$('#tab_num').val('2');tab2();">ที่อยู่การจัดส่งเอกสาร</a></li>  
</ul>

<div id="content" > 
    <div id="tab1" ><?php include("from_search.php");?></div>
    <div id="tab2"><?php include("from_address.php");?></div>
</div>

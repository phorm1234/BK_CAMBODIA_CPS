<?php 
	class Model_Member extends Model_PosGlobal{
		...
		function setPdtBuyOneGetOne($seq_pro,$member_no,$status_no,$promo_code,$product_id,$quantity){
			/***
			 * @desc
			 * @modify : 20102014
			 */
		...
		function unLockMobileCoupon($doc_no){
			/**
			 * @desc unlock mobile coupon application
			 * * modify : 08082014
			 * @return
			 */
			$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
			$sql_chk="SELECT COUNT(*) FROM trn_diary1 WHERE doc_no='$doc_no' AND coupon_code='OK27190514'";
			...
			if($n_chk>0){
				...
				$act = "cancel";
				$src = $ws."brand=op&doc_no=".$doc_no."&shop=".$shop."&act=".$act."&_=1334128422190";
		...
		function selCoPromotion($promo_code,$net_amt,$amount){
			/***
			 * @desc 22072014
			 * ** for table promo_check co promotion
			 * ** create 22072014
			 * ** modify 16102014 append promo_st='F' 
		...
		function setPdtMCoupon($member_no,$status_no,$promo_code,$product_id,$quantity){
			/**
			 * @desc 16102014
			 * * for support mobile coupon 30% discount
			 * @return
			 */
			$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
			$msg="";
		...
		function setCshValTemp($member_no){
			/**
			 * @desc 22072014
			 * @return
			 */
		...
		function chkCouPonIsUsed($coupon_code){
			/**
			 * @desc 22072014
		...
		function getListCoupon(){
			...
			$sql_promo="SELECT 
								...
								WHERE
									corporation_id='$this->m_corporation_id' AND
									company_id='$this->m_company_id' AND
									promo_tp IN('COUPON','COUPONMOBILE') AND
									'$this->doc_date' BETWEEN start_date AND end_date	";
			$row_promo=$this->db->fetchAll($sql_promo);
			if(count($row_promo)>0){
				return $row_promo;
					...
					$doc_no_chk=$data['doc_no'];
						$sql_chkdup="SELECT COUNT(*) FROM trn_diary2
											WHERE 
												...
												promo_code IN('OX07571014','OX07581014')";
...
//					    $member_year=$ob_json->age_card;
				...
				//assume promotion OPPN300 สมัครสมาชิกใหม่
							...
							if(count($row_promo)<1){
								...
								foreach($row_promo as $dataPromo){
										...
										$timeStmpEndPro = mktime(0,0,0,'12','31','2014');
		...
		function checkMemberExist($member_no,$ops_day_old){
			/**
			...
			if($ops_cmp!='' && $sp_day_cmp!=$ops_cmp){
				return 3;
			}else if($ops_cmp==''){
				$sql_chk="SELECT COUNT(*) FROM `trn_tdiary2_sl` WHERE promo_code IN('OPPN300','OPPS300')";
				$n_chk=$this->db->fetchOne($sql_chk);
				if($n_chk>0){
...
//					}//end if
		...
		function payment(
			...
			if($status_no=='01' && $application_id!='OPPN300' && $application_id!='OPPS300' && $refer_member_id==''){					
			...
			if($status_no=='01'){
				...
				if($application_id=='OPPC300'){
					...
					//$total_point=$point_begin+$point1+$point2+$redeem_point;	
					if(substr($member_no,0,4)=='1520'){
						$point1=30;
					}
					$total_point=$point1+$point2+$redeem_point;	
				}else if($application_id=='OPPN300' || $application_id=='OPPS300'){
					$total_point=0;
				}else if(substr($member_no,0,4)=='1520'){
					//*WR 20032014 for support Tuesday Card receice 30 point expire 31-07-2014
					$point1=30;
					$total_point=30;
				}
			}
...
//				}
				...
				$strResult= "1#".$this->m_doc_no."#".$this->doc_tp."#".$this->m_thermal_printer."#".$this->net_amt."#".$status_no."#".$this->m_branch_tp."#".$this->m_cashdrawer."#".$application_id;	
				////////COMPLETE TRANSACTION///////////
			}else{

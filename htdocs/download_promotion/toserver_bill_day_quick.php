...
$conf="select doc_date  from com_doc_date";
$r_conf=mysql_db_query($dbname,$conf,$conn_shop) or die("NONO");
$rows=mysql_num_rows($r_conf);
if($rows>0){
	$data_conf=mysql_fetch_array($r_conf);
...
$conf="select rechk_up  from service_log order by rechk_up desc limit 1";
$r_conf=mysql_db_query($dbname,$conf,$conn_shop) or die("NONO");
$data_conf=mysql_fetch_array($r_conf);
$rechk_up=$data_conf['rechk_up'];
if($rechk_up==""){
	echo "rechk_up='' ";
	return false;
}
...
/*
$conf="select *  from trn_diary1 where  doc_date ='$doc_date'  and STR_TO_DATE( concat(doc_date, ' ',doc_time ) , '%Y-%m-%d %H:%i:%s' ) >= DATE_SUB(NOW(), INTERVAL 30 MINUTE)  order by doc_time desc";	
*/
$conf="select *  from trn_diary1 where  doc_date ='$doc_date'  and  STR_TO_DATE( concat( doc_date, ' ', doc_time ),'%Y-%m-%d %H:%i:%s')>=STR_TO_DATE('$rechk_up','%Y-%m-%d %H:%i:%s')";	
$conn_shop=mysql_connect($ipserver,$mysql_user,$mysql_pass) or die("no connect mysql");
mysql_query("SET character_set_results=utf8");
mysql_query("SET character_set_client=utf8");
mysql_query("SET character_set_connection=utf8");

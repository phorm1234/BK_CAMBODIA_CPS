...
where
a.member_id<>'' and
b.promo_code IN ('OM02071113', 'OM02081113', 'OM02091113') and
...
mysql_query("SET character_set_client=utf8");
mysql_query("SET character_set_connection=utf8");
$conf="select * from trn_diary1 WHERE doc_date = date(now()) and flag='C' ";
echo $conf;
$conn_ji=mysql_connect($ji_ip,$ji_user,$ji_pass) or id("Link down");
mysql_query("SET character_set_results=utf8");
mysql_query("SET character_set_client=utf8");
...
for($x=1; $x<=$rows; $x++){
	...
	$insert="update promo_play_history set flag='$flag' where doc_date='$doc_date' and doc_no='$doc_no'	";
	//echo "$insert<br>";
	$result_insert=mysql_db_query($ji_db_sevice,$insert,$conn_ji);
	
	$insert="update trn_diary1 set flag='$flag' where doc_date='$doc_date' and doc_no='$doc_no'	";
	//echo "$insert<br>";
	$result_insert=mysql_db_query($ji_db_sevice,$insert,$conn_ji);
	if($result_insert){
		echo "Up Bill Cancel $x $doc_no Update Complete<br>";
	}else{
		echo "Up Bill Cancel $x $doc_no Update None Complete<br>";
	}

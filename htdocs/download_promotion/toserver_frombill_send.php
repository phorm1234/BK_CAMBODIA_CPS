...
$doc_no=$_POST['doc_no'];
$doc_date="$bill_year-$bill_month-$bill_d";
echo "send_type:$send_type<br>";



if($send_type=="B" and $member_no==''){
	echo "<script>alert('กรุณาระบุเลขที่สมาชิกด้วยครับ');</script>";
	echo "<script>window.location='toserver_frombill.php'</script>";
}

if($send_type=="F" and $doc_no==""){
	echo "<script>alert('กรุณาระบุเลขที่บิลด้วยครับ');</script>";
	echo "<script>window.location='toserver_frombill.php'</script>";
}



$ipserver=$server_local;
$mysql_user=$user_local;
$mysql_pass=$pass_local;
$dbname=$db_local;

//JI

$ji_ip=$server_service;
$ji_user=$user_service;
$ji_pass=$pass_service;
$ji_db_sevice=$db_service;
...
if($send_type=="A"){
	$conf="select *  from trn_diary1 where  doc_date =date(now()) order by id desc limit 1 ";	
}else if($send_type=="B"){
	echo "member_no:$member_no<br>";
	echo "doc_date:$doc_date<br>";
	$conf="select *  from trn_diary1 where  doc_date ='$doc_date' and member_id='$member_no' ";	
}else if($send_type=="F"){
	$conf="select *  from trn_diary1 where  doc_no ='$doc_no'  ";	
}else{
	$conf="select *  from trn_diary1 where  doc_date =date(now()) ";	
}
echo $conf;	
$conn_shop=mysql_connect($ipserver,$mysql_user,$mysql_pass) or die("no connect mysql");
mysql_query("SET character_set_results=utf8");
...
echo "Find : ".$rows . "items<br>";

if($send_type=="F" && $rows==0){
	echo "<script>alert('ไม่พบเลขที่บิลนี้ในระบบ กรุณป้อนเลขที่บิลใหม่อีกครั้ง');</script>";
	echo "<script>window.location='toserver_frombill.php'</script>";
}

if($rows==0){
	echo "<script>alert('ไม่พบบิลที่จะส่งข้อมูล');</script>";
	echo "<script>window.location='toserver_frombill.php'</script>";
}
mysql_close($conn_shop);
for($i=1; $i<=$rows; $i++){
	...
	if($result_insert){
		echo "Bill : $doc_no at $doc_date Insert Complete<br>";
	}else{
		echo "Bill : $doc_no at $doc_date Insert Duplicate<br>";
	}

	if($flag=="C"){
		$up="update trn_diary1 set flag='$flag' where doc_date='$doc_date' and doc_no='$doc_no' ";
		echo "$up<br>";
		mysql_query($up,$conn_ji) ;
	}
...
<span class='style2'>ส่งข้อมูลเรียบร้อยแล้ว</span><br>
<a href="toserver_frombill.php" class='style2'>Back</a>
</center>
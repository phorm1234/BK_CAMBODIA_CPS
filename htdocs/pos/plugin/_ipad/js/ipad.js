...
$('#sleep').live('click',function(){
	$.get("/pos/admin/logout",{
	ran:Math.random()
	},function(data){
		   $('#loginbox').hide();
		   $('#loginbox').removeClass('editMode');
    ...
    $('#window').addClass('out')
            ...
            .animate({left:"0%",width:885,top:"0%",height:662,opacity:1},'easeInQuint',function(){
                ...
                if(!$('#'+app_id+'_app').length){
                    ...
                    $('#iframe_holder').append('<iframe id="'+app_id+'_app" class="app_iframe" src="/pos/admin/app'+_appToLaunch+'" scrolling="no" width="884" height="641"></iframe>');

<?php
	...
	function sent_data($BRAND,$SHOP){
					...
					system($comman);
					$comman= "ncftpput -u opkh -pGJNCPsMRcuQSzGjDKVKRZ9mrZALpHT6A transopkh.ssup.co.th / $SHOP.zip";
					system($comman);
					$BRAND=urlencode($BRAND);
					$SHOP=urlencode($SHOP);
					$fp = @fopen("http://transopkh.ssup.co.th/transfer_to_office/op/run_script.php?BRAND=$BRAND&SHOP=$SHOP", "r");
					$text=@fgetss($fp, 4096);
					return "Succeed";
	}//func
	//--------------------------------------------------------------------------------------------

<?php
	...
	function sent_data($BRAND,$SHOP){
					...
					$folder="$SHOP";
					$path="/var/www/pos/htdocs/transcps.ssup.co.th/$folder";
					if(is_dir($folder)){
							...
							$file_mysql=$arr['file_mysql'];
								...
								switch ($file_mysql) {
										case "com_dayend_time":
												$where="dayend_date BETWEEN date_add('$doc_date',interval -16 day)  AND date_add('$doc_date',interval -0 day) ";
										...
										break;
										case "com_doc_date":
												$where="upd_date BETWEEN date_add('$doc_date',interval -16 day)  AND date_add('$doc_date',interval -0 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										case "com_data_po":
												$where="doc_date BETWEEN date_add('$doc_date',interval -16 day)  AND date_add('$doc_date',interval -0 day) ";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										...
										break;
										case "trn_diary1":
												$where="doc_date BETWEEN date_add('$doc_date',interval -16 day)  AND date_add('$doc_date',interval -1 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										case "trn_diary2":
										...
										case "com_expense_head":
												//$where="doc_dt BETWEEN date_add('$doc_date',interval -16 day)  AND date_add('$doc_date',interval -1 day)";
										...
										break;
										case "crm_card":
												$where="apply_date BETWEEN date_add('$doc_date',interval -16 day)  AND date_add('$doc_date',interval -1 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										case "crm_profile":
												$where="upd_date BETWEEN date_add('$doc_date',interval -16 day)  AND date_add('$doc_date',interval -1 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\" >  $path/$file_mysql.sql";
												system($command);
										break;
										case "trn_diary1_rd":
					...
					system($comman);
					$comman= "ncftpput -u cps -p CPS###4910 transcps.ssup.co.th / $SHOP.zip";
					system($comman);
					$BRAND=urlencode($BRAND);
					$SHOP=urlencode($SHOP);
					$fp = @fopen("http://transcps.ssup.co.th/transfer_to_office/cps/run_script.php?BRAND=$BRAND&SHOP=$SHOP", "r");
					$text=@fgetss($fp, 4096);
					return "Succeed";
	}//func
	//--------------------------------------------------------------------------------------------

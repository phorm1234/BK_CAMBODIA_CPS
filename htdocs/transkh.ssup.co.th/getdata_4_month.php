<?php
  ...
  echo sent_data($BRAND,$SHOP);
	...
	function sent_data($BRAND,$SHOP){
							...
							$file_mysql=$arr['file_mysql'];
								switch ($file_mysql) {
										case "com_dayend_time":
												$where="dayend_date BETWEEN date_add('$doc_date',interval -124 day)  AND date_add('$doc_date',interval -0 day) ";
										...
										break;
										case "com_doc_date":
												$where="upd_date BETWEEN date_add('$doc_date',interval -124 day)  AND date_add('$doc_date',interval -0 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										case "com_data_po":
												$where="doc_date BETWEEN date_add('$doc_date',interval -124 day)  AND date_add('$doc_date',interval -0 day) ";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										...
										break;
										case "trn_diary1":
												$where="doc_date BETWEEN date_add('$doc_date',interval -124 day)  AND date_add('$doc_date',interval -1 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										case "trn_diary2":
										...
										case "com_expense_head":
												//$where="doc_dt BETWEEN date_add('$doc_date',interval -124 day)  AND date_add('$doc_date',interval -1 day)";
										...
										break;
										case "crm_card":
												$where="apply_date BETWEEN date_add('$doc_date',interval -124 day)  AND date_add('$doc_date',interval -1 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\">  $path/$file_mysql.sql";
												system($command);
										break;
										case "crm_profile":
												$where="upd_date BETWEEN date_add('$doc_date',interval -124 day)  AND date_add('$doc_date',interval -1 day)";
												$command = "mysqldump -u$user_name -p'$password' --no-create-db --no-create-info --replace pos_ssup $file_mysql --where=\"$where\" >  $path/$file_mysql.sql";
												system($command);
										break;
										case "trn_diary1_rd":
					...
					$SHOP=urlencode($SHOP);
					$fp = @fopen("http://transopkh.ssup.co.th/transfer_to_office/op/run_script_4_month.php?BRAND=$BRAND&SHOP=$SHOP", "r");
					$text=@fgetss($fp, 4096);
					return "Succeed";
	}//func

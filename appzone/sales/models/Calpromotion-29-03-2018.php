...
class Model_Calpromotion extends Model_PosGlobal
{
		...
		public $sv_api="crmopkh.ssup.co.th";
		public $sv_online="10.100.59.151";
		public $user_online="crm-op-kh";
		public $pass_online="BxcfpffA8Y98qsDqMyQCXY4bsPQSbZnp";
		public $db_online="service_pos_opkh";
		public $db_online_old="pos_opkh";
...
doc_date between 
		...
		function up_point($doc_no){
			...
			if($status_no=="05"){	
				echo "Missing";			
				$fp = @fopen("http://$this->sv_api/app_service_opkh/process/member_new3.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);
				$fp = @fopen("http://$this->sv_api/app_service_opkh/app_service_op/process/member_his_seq.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);
			}
			
			if($status_no=="01" && $refer_member_id!=""){
				echo "Renew";	
				$fp = @fopen("http://$this->sv_api/app_service_opkh/process/member_new2.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);
				$fp = @fopen("http://$this->sv_api/app_service_opkh/process/member_renew_seq.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);			
			}
		...
		function up_cancle_bill($doc_no){
			    ...
			    $result_up_cancle_promoplay=$this->db->query($up_cancle_promoplay, 2);
				 ...
				 if($application_id=="OPPN300"   || substr($application_id,0,3)=="OPN" ){
		...
		function read_point($member_no){
			$fp =fopen("http://$this->sv_api/app_service_opkh/process/member_point_all.php?member_no=$member_no", "r");
            $text=fgetss($fp, 4096);
            return $text;
		}
...
/*
		...
		function sale_all($member_no){
            $fp = @fopen("http://$this->sv_api/app_service_opkh/process/member_sale_all.php?member_no=$member_no", "r");
            $text=@fgetss($fp, 4096);
            return  $text;
		}

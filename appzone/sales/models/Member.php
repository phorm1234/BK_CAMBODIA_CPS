...
class Model_Member extends Model_PosGlobal
{
	...
	function chkPlaybillRegister()
	{
		...
		$sql = "SELECT branch_id, member_no FROM com_member_dummy ";
		$data = $this->db->fetchAll($sql);
		$memdummy = "ID" . $data[0]['branch_id'] . str_pad($data[0]['member_no'], 7, '0', STR_PAD_LEFT);
		return  array('regist' => $bill_regis, 'dummy' => $memdummy);
	} //func
	...
	function updDstCshTemp($promo_code, $type_discount, $discount)
	{
		...
		$where = "";
		if (
			$promo_code == 'OPPL300' || $promo_code == 'OPMGMC300' || $promo_code == 'OPMGMI300' ||
			...
			if (!empty($rows_chk)) {
				foreach ($rows_chk as $data) {
					...
					if ($n_chk < 1) {
						...
						$check_repeat = 'N';
						$objCsh = new Model_Cashier();
						$str_res_save = $objCsh->setGapValTemp(
							$doc_tp,
							$promo_code,
							$promo_tp,
							$employee_id,
	...
	function changeNewCard($member_no, $member_no_ref, $ops_day_ref)
	{
		...
		if (!empty($row_chkopsday)) {
			...
			switch ($ops_day_cmp) {
				case '31':
					$ops_day_cmp = 'TH1';
					$ops_product_id = '25654';
					break;
				case '32':
					$ops_day_cmp = 'TH2';
					$ops_product_id = '25655';
					break;
				case '33':
					$ops_day_cmp = 'TH3';
					$ops_product_id = '25656';
					break;
				case '34':
					$ops_day_cmp = 'TH4';
					$ops_product_id = '25657';
					break;
				default:
					$ops_day_cmp = '';
			}
	...
	function chkEmpPrivilage($employee_id)
	{
		...
		if (
			$arr_opsday[0] == '31' || $arr_opsday[0] == '32' || $arr_opsday[0] == '33' || $arr_opsday[0] == '34' || $arr_opsday[0] == '35'
	...
	function setProBalance($application_id, $free_product_amount, $product_amount_type)
	{
		...
		if (count($row_a) > 0) {
			...
			foreach ($row_a as $data) {
				...
				if (
					$chk_amount > $free_product_amount && $application_id != 'OPPS300' &&
	...
	function chkAmtProBalanceCoupon($promo_code, $product_id, $quantity)
	{
		...
		$sql_maxseq = "SELECT seq,amount,net_amt,quantity FROM `trn_promotion_tmp1`
								WHERE
									`corporation_id`='$this->m_corporation_id' AND
									`company_id`='$this->m_company_id' AND
									`branch_id`='$this->m_branch_id' AND
		...
		$quantity = $row_maxseq[0]['quantity'];
		...
		if ($buy_type == 'G') {
			...
			if ($buy_status == 'L') {
				if ($sum_curr > $promo_amt) {
					...
					// $this->db->query($sql_del);
					if ($quantity == 1) {
						$total1 = $promo_amt - $sum_check;
						$new_discount = $total1 * $discount;
						$new_net_amt = $total1 - $new_discount;
						$sql_upd = "UPDATE`trn_promotion_tmp1`
											...
											WHERE
												...
												`seq`='$max_seq'";
						$this->db->query($sql_upd);
						$res_msg = "K#ยอดซื้อครบแล้ว";
						if ($new_discount <= 0) {
							$res_msg = "N#The purchase cannot exceed the amount, please check again.";
							$sql_del = "DELETE FROM `trn_promotion_tmp1`
											WHERE
												`computer_no`='$this->m_computer_no' AND
												`computer_ip`='$this->m_com_ip' AND
						...
						$res_msg = "N#The purchase amount must not exceed 1 amount, please check again.";
						$sql_del = "DELETE FROM `trn_promotion_tmp1`
										WHERE
											`computer_no`='$this->m_computer_no' AND
				 							`computer_ip`='$this->m_com_ip' AND
	...
	function incDummy()
	{
		$this->db = SSUP_Controller_Plugin_Db::checkDbOnline("com", "com_member_dummy");
		$sql = "UPDATE com_member_dummy SET member_no = member_no+1";
		$this->db->query($sql);
	}

	// function sentOtpCode($mobile, $chkstatus = 'Y')
	// {
	// 	$this->db = SSUP_Controller_Plugin_Db::checkDbOnline("com", "trn_diary1");
	// 	$chkstatus = $chkstatus == 'N' ? 'N' : 'Y';
	// 	$url = "http://crmcpskh.ssup.co.th/app_service_cpskh/api_member/api_confirmby_mobile.php?mobile_no=" . $mobile;
	// 	$result = @file_get_contents($url, 0);
	// 	$data = json_decode($result, true);
	// 	$member = $data['member_no'];
	// 	/*
	// 		$sql="SELECT count(*) FROM trn_diary1 WHERE mobile_no = '$mobile' ";
	// 		$mem_offline=$this->db->fetchOne($sql);
	// 		if($mem_offline>0){
	// 			$data['status']=='Y';
	// 		}
	// 		*/
	// 	if ($data['status'] == $chkstatus) {
	// 		//return array('status'=>'Y','msg'=>"");	
	// 		$postdata = http_build_query(array('mobile' => $mobile, 'idcard' => 'XXX', 'memberid' => "XXX", 'shop' => 'cpskh'));
	// 		$opts = array('http' => array('method' => 'POST', 'header' => "Content-type: application/x-www-form-urlencoded\r\n" . "Referer: urltopost\r\n", 'content' => $postdata));
	// 		$context = stream_context_create($opts);
	// 		$result = file_get_contents('http://mshop.ssup.co.th/shop_cps/cpskh_promotion.php', false, $context);
	// 		$data = json_decode($result, true);
	// 		return array('status' => $data['sms_status'], 'msg' => $data['msg'], 'member' => $member);
	// 	} else {
	// 		if ($chkstatus == 'Y') {
	// 			return array('status' => $data['status'], 'msg' => "Member not Found. Please check mobile again.");
	// 		} else {
	// 			return array('status' => $data['status'], 'msg' => "This mobile already regist");
	// 		}
	// 	}
	// }

	function sentOtpCode($mobile)
	{
		$postdata = http_build_query(array('mobile' => $mobile, 'idcard' => 'XXX', 'memberid' => "XXX", 'shop' => 'cpskh'));
		$opts = array('http' => array('method' => 'POST', 'header' => "Content-type: application/x-www-form-urlencoded\r\n" . "Referer: urltopost\r\n", 'content' => $postdata));
		$context = stream_context_create($opts);
		$result = file_get_contents('http://mshop.ssup.co.th/shop_cps/cpskh_promotion.php', false, $context);
		// $result = file_get_contents('http://mshop.ssup.co.th/shop_op/op_promotion.php', false, $context);
		$data = json_decode($result, true);
		return array('status' => $data['sms_status'], 'msg' => $data['msg']);
	}



	function checkOtpCode($mobile, $otpcode)
	{
		//return array('status'=>'Y','msg'=>"");	
		$postdata = http_build_query(array('mobile' => $mobile, 'idcard' => 'XXX', 'memberid' => "XXX", 'shop' => 'cpskh', 'otp' => $otpcode));
		$opts = array('http' => array('method' => 'POST', 'header' => "Content-type: application/x-www-form-urlencoded\r\n" . "Referer: urltopost\r\n", 'content' => $postdata));
		$context = stream_context_create($opts);
		$result = file_get_contents('http://mshop.ssup.co.th/shop_cps/cpskh_promotion_check.php', false, $context);
		$data = json_decode($result, true);
		return array('status' => $data['otp_status'], 'msg' => $data['msg']);
	}

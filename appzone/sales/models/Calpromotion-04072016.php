...
class Model_Calpromotion extends Model_PosGlobal
{
	   ...
	   function datatmp2($promo_seq){
		...
		function chkNewmemberFromBill($member_no){
			$sql="select * from trn_diary1 where member_id='$member_no' and doc_date='$this->doc_date' and status_no='01' and application_id in('OPPN300','OPPS300','OPPH300','OPPL300','OPPL300','OPMGMC300','OPID') and flag<>'C'";
				$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("crm","crm_card");
				$r = $this->db->fetchAll($sql, 2);
				if($r){
					$new_member="Y";
		...
		function send_bill($doc_no){
			...
			if($status_no=="01" && $member_id!="" && $refer_member_id!="" && $application_id!="" && $application_id!="OPPN300" && $application_id!="OPPS300"  && $application_id!="OPPH300"   && $application_id!="OPPL300"   && $application_id!="OPMGMC300"   && $application_id!="OPID"){
...
doc_date between 
		...
		function re_card_missing($bec,$doc_no,$doc_date,$application_id,$member_id,$refer_member_id){
			...
			if($status_no=="01" && ($application_id=="OPPN300" || $application_id=="OPPS300" || $application_id=="OPPH300" || $application_id=="OPPL300" || $application_id=="OPMGMC300" || $application_id=="OPID") && $member_id!=""){//New member
		...
		function up_point($doc_no){
			...
			/*$date_download=$this->doc_date;
            $fp = @fopen("http://192.168.3.247/download_promotion/toserver_bill_day.php?doc_date=$date_download", "r");
            $text=@fgetss($fp, 4096);	*/		    
			/*$bill="select * from trn_diary1 where doc_no='$doc_no'";
			$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
	 		$databill=$this->db->fetchAll($bill, 2);
		...
		function up_cancle_bill($doc_no){
			    ...
			    $result_up_cancle_promoplay=$this->db->query($up_cancle_promoplay, 2);
				 ...
				 if($application_id=="OPPN300" || $application_id=="OPPS300" || $application_id=="OPPH300"  || $application_id=="OPPL300"  || $application_id=="OPMGMC300"  || $application_id=="OPID"){
				    //del member
				    $del_member_no="delete from member_history where member_no='$member_id' ";
					$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("view","point_summary");
				    $result_del_member=$this->db->query($del_member_no, 2);
...
/*
		...
		function transfer_point($doc_no){
			...
			if($rows_find>0){
					if( ($status_no=="01") && ($application_id!="OPPN300") && ($application_id!="OPPH300") && ($application_id!="OPPL300") && ($application_id!="OPPS300") && ($application_id!="OPMGMC300") && ($application_id!="OPID") && ($application_id!="") &&  ($member_id!="") ){//ต่ออายุ  member
			...
			$sql="
				...
				$find="
					...
					from
						...
						on a.promo_code=c.promo_code
						where
						'$this->doc_date' between a.start_date and a.end_date and a.year_apply='2014' and
				...
				$find="
					...
					from
						...
						where
						'$this->doc_date' between a.start_date and a.end_date and '$this->doc_date'>='2015-02-01'  and a.year_apply='2015' and
				...
				if($rows==0){
					if($status_show=="Y"){
						echo "<center><span style='font-size:20px;color:#ff0000;'>ไม่ใช่ลูกค้าสมัครใหม่ในปีนี้</span></center><br><br>";
					}
				}
				$conn_online=mysql_connect($this->sv_online,$this->user_online,$this->pass_online);	
				mysql_select_db($this->db_online);			
				...
				if($chk_timeplay=="N"){
					...
					a.promo_code in('OX07061114','OX07071114','OX07081114') and
					 a.doc_date between '2015-02-01' and '2015-12-31'
					 group by a.doc_no
					 order by a.doc_date
					  ";
					//echo $chk_local;
					...
					if($count_chk_local>=3){//เล่นแล้ว
						...
						$chk="select *
							from promo_play_history
							where 
							doc_date  between '2015-02-01' and '2015-12-31' and member_id in($missing_seq) and 
							 flag<>'C'  and promo_code in('OX07061114','OX07071114','OX07081114')
							 order by doc_date
						 ";
		...
		function promo_chk_play_renew($member_no){
			...
			if($year_app==2015){ //รอบ
				...
				if($rows_chk_set>0){
					...
					if($type_member=="Re" && $year_app==2015 && ($month_now==$month_app || $month_now==$month_app_pointx2  || $month_now==$month3) ){
				...
				if($rows_find_pro>0){
					...
					if($chk_play_date=="yes_play_date"){
							...
							if($type_member=="New" && $year_app=='2014' && ($age_card==0 || $age_card==2)){
									//<=เดือนเกิดหรือไม่
									if($year_app==$year_now-1 && $month_app<$m_hbd){
										$show_play="<img src='../img/tick.png' hieght='20' width='20'></img>";
										$remark="";//เหตุผล
									}else if($year_app==$year_now-1 && $month_app==$m_hbd && $day_app<=$d_hbd){
										$show_play="<img src='../img/tick.png' hieght='20' width='20'></img>";
										$remark="";//เหตุผล									
									}else{
										$show_play="";
									...
									//เล่นได้ปีละครัง
									$play="select * from promo_play_history where member_id in($map_member) and promo_code='$promo_code' and doc_date>='2014-02-01' and flag<>'C' group by doc_no order by doc_date desc limit 1  ";
									//echo $play;
				...
				if($rows_find_pro>0){
					...
					if($chk_play_date=="yes_play_date"){
							...
							if($type_member=="New" && $year_app=='2015' && ($age_card==0 || $age_card==2)){
									...
									//เล่นได้ปีละครัง
									$play="select * from promo_play_history where member_id in($map_member) and promo_code='$promo_code' and flag<>'C'  and doc_date between '$start_date' and '$end_date' group by doc_no  ";
									//echo $play;
									$run_play=mysql_query($play,$conn_online);
									$rows_play=mysql_num_rows($run_play);
		...
		function promo_chk_other($member_no){
				...
				$find="
					...
					from
						...
						where
						'$this->doc_date' between a.start_date and a.end_date and
						a.type_member='magazine'  and a.flag='' and a.promo_code<>'OM13060115'
				";
				//echo $find;
		...
		function sale_history($member_id){
				...
				mysql_query("SET collection_connection='utf8_general_ci'");
			  	...
			  	SELECT a.branch_id,a.doc_no,a.doc_date,a.amount,a.net_amt,a.point1,a.point2,a.redeem_point,a.total_point,a.doc_time FROM (
				  	select branch_id,doc_no,doc_date,amount,net_amt,point1,point2,redeem_point,total_point,doc_time  from trn_diary1 where member_id='$member_id' 
				  	UNION ALL
				  	select branch_id,doc_no,doc_date,amount,net_amt,point1,point2,redeem_point,total_point,doc_time  from trn_diary1_all where member_id='$member_id' 
			  	) as a 

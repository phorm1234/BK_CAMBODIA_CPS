<?php	
	class Model_PosGlobal{		
		...
		function cutNum($num, $precision = 2){
			...
			$test = explode('.',$num);
			return $test[0].'.'.substr(str_pad(str_replace('|','',$test[1]),$precision,'0',STR_PAD_RIGHT ),0,$precision+1);//1

			//return floor($num).substr($num-floor($num),1,$precision+1);//2			
		}//func
...
//				$res_check=$objPosPlugin->checkInOut($this->doc_date,$this->employee_id);
		...
		public function getRosSaleman($employee_id,$password){
			...
			WHERE employee_id='$employee_id'  AND password_id='$password' AND group_id in('OpArom','OpRos','CpShopEmp','CpShopMng')";

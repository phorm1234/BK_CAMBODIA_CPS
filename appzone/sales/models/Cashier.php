<?php 
	class Model_Cashier extends Model_PosGlobal{
		...
		function capturecamera()
		{
			...
			$sql = "SELECT cam_ip_1 FROM com_branch_cam WHERE branch_id = '" . $shop . "' ";
			$data = $this->db->fetchAll($sql);
			return $data[0]['cam_ip_1'];
		}
		...
		function clearBill()
		{
			...
			// echo $data_new;
			// exit();
			$this->db = SSUP_Controller_Plugin_Db::checkDbOnline("com", "com_branch");
			$sql_branch = "SELECT * FROM com_branch WHERE active = '1'";
			$branch_arr = $this->db->fetchAll($sql_branch);
			$branch_id = $branch_arr[0]['branch_id'];
			if ($branch_id != '1190' && $branch_id != '1191') {
				if ($m == 01 && $d == 01) {
					$this->db = SSUP_Controller_Plugin_Db::checkDbOnline("conf", "conf_doc_no");
					$sql_conf_doc_no = "UPDATE conf_doc_no set def_value = '$year' WHERE id = '1'";
					@$this->db->query($sql_conf_doc_no);
					$sql_def_value = "SELECT * FROM conf_doc_no WHERE id = '1'";
					$def_value = $this->db->fetchAll($sql_def_value);
					$sub_y = $def_value['0']['def_value'];
					if ($sub_y == $year) {
						$sql_com_doc_no = "UPDATE com_doc_no set doc_no = '1'";
						@$this->db->query($sql_com_doc_no);
						echo "$m#$d";
					}
				}
			}
		...
		function addPdtAutoShipping(){
			...
			//check duplicate
			$sql_dup="SELECT COUNT(*) FROM $tbl_temp 
						WHERE
							...
							`promo_code` IN('CPKH1001','CPKH2001') ";
			$n_dup=$this->db->fetchOne($sql_dup);
			$n_dup=intval($n_dup);
			if($n_dup>0){
				return true;
			}

			$sql_sum_amt="SELECT SUM(net_amt) 
							FROM $tbl_temp 
							WHERE
							`corporation_id`='$this->m_corporation_id' AND 
							`company_id`='$this->m_company_id'  AND
			...
			$sum_amt=$this->db->fetchOne($sql_sum_amt);
			$sql_shipping="SELECT* FROM `promo_shipping` 
								WHERE 
									'$sum_amt' BETWEEN start_amount AND end_amount 
									AND '$this->doc_date' BETWEEN start_date AND end_date";
			$row_shipping=$this->db->fetchAll($sql_shipping);			
			$doc_tp='SL';
			$promo_code=$row_shipping[0]['promo_code'];
			$promo_tp='';
			$employee_id=$this->employee_id;
			$member_no='';
			$product_id=$row_shipping[0]['product_id'];
			...
			$co_promo_percent=0;
			$coupon_percent=0;
			$promo_st=$row_shipping[0]['type_discount'];


			$type_discount=$row_shipping[0]['type_discount'];
			if(strtoupper($type_discount)=='FREE'){
				...
				$discount=$sp_price-$row_shipping[0]['discount'];
			}else{
				$promo_tp='P';
			}
			...
			$check_repeat='Y';
			@$this->setGapValTemp($doc_tp,$promo_code,$promo_tp,$employee_id,$member_no,$product_id,$quantity,
		...
		public function getPdtOfBill($product_id){
			...
			echo "<table width='100%' border='0' cellpadding='2' cellspacing='1' bgcolor='#cccccc'>
						<tr height='32' bgcolor='#66CDAA'>
							<td align='center' width='5%'><span style='font-size:22px;color:#fff;'>STATUS</span></td>
							<td align='center' width='20%'><span style='font-size:22px;color:#fff;'>BILL NO.</span></td>
							<td align='center' width='13%'><span style='font-size:22px;color:#fff;'>PRODUCT ID</span></td>
							<td align='center'><span style='font-size:22px;color:#fff;'>DESCRIPTION</span></td>
							<td align='center' width='10%'><span style='font-size:22px;color:#fff;'>PRICE</span></td>
							<td align='center' width='5%'><span style='font-size:22px;color:#fff;'>QUANTITY</span></td>
							<td align='center' width='15%'><span style='font-size:22px;color:#fff;'>AMOUNT</span></td>
						</tr>";
			$i=1;
			$sum_amount=0.00;
			$sum_qty=0;
			...
			echo "
				<tr height='32' bgcolor='#B0C4DE'>
						<td align='center'>&nbsp;</td>
						<td align='center'>&nbsp;</td>
						<td align='center'><span style='font-size:22px;color:#000;'>Total</span></td>
						<td align='center'>&nbsp;</td>
						<td align='center'>&nbsp;</td>
						<td align='center'><span style='font-size:22px;color:#000;'>".intval($sum_qty)."</span></td>
						<td align='center'><span style='font-size:22px;color:#000;'>".number_format($sum_amount,'2','.',',')."</span></td>
					</tr>
				</table>";
		}//func
		...
		public function chkAmtVip($product_id,$quantity,$limited,$limited_type,$sum_amt,$vip_percent_discount){
			/**
			 * @desc cal net,amt by limited_type
			 * @modify 15062017
			 * @return Boolean Y,N
			 */
			...
			$inp_amount=$price*$quantity;//ยอด amount ที่ key เข้ามา G
			$p_amt=$inp_amount-$inp_amount*($vip_percent_discount/100);		//ยอด net ที่ key เข้ามาแล้วหักส่วนลด 35 %	N
			...
			if(count($row_trn2)>0){
				if($limited_type=='N'){
					...
					$chk_amt=$row_trn2[0]['sum_amount']+$inp_amount;
				}
				if($chk_amt>$sum_amt){
					return 'N';
				}else{
...
//					$objPos->delSrcTableData($folder_backup);
		...
		function getSumPromoDetail(){
			...
			if(count($row)>0){
				...
				$sum_amount=parent::getSatang($sum_amount,'normal');//up
				$sum_discount=parent::getSatang($sum_discount,'normal');//up		
		...
		function setGapValTemp($doc_tp,$promo_code,$promo_tp,$employee_id,$member_no='',$product_id,$quantity,
				...
				$co_promo_percent,$coupon_percent,$promo_st,$net_amt,$discount,$discount_member,$check_repeat,$unitn=''){
					 ...
					 * @name setGapValTemp
					 * @modify 13072016
					 * @return $proc_status."#".$this->amount."#".$tbl_temp."#".$bal_discount;
					 */
					$sql_chk_tbl="SELECT COUNT(*)
					FROM trn_promotion_tmp1
					WHERE
					...
					if(!empty($arr_product)){
						...
						if($balance<$this->quantity){
							...
							//กลับมาดูใหม่อาจจะปรับเป็น promo_detail.type_discount=='Free'
							if($this->promo_tp=='F'){
								$this->price=0;
							}else{
								$this->price=$arr_product[0]['price'];
							...
							//*WR17102012
							$bal_discount=0;
							if($this->amount < $this->discount && $this->discount_percent!='PERCENT'){
								$bal_discount=$this->discount-$this->amount;
								$this->discount=$this->amount;
							}
							//*WR17102012
								
							//*WR30062016 new idea for support discount promotion
		...
		function setCshValTemp($new_member,$doc_tp,$promo_code,$employee_id,$member_no,$product_id,$quantity,
							$status_no,$product_status,$application_id,$card_status,
							$get_point,$promo_id,$discount_percent,$member_percent1,$member_percent2,
		...
		function setCshTemp($doc_tp,$promo_code,$employee_id,$member_no='',$product_id,$quantity,
							...
							$get_point,$promo_id,$discount_percent,$member_percent1,$member_percent2,
							$co_promo_percent,$coupon_percent,$promo_st,$promo_tp,$promo_amt,$promo_amt_type,
							$check_repeat,$web_promo,$point1='',$point2='',$discount='0',$coupon_discount='',$seq_set='0'){
			/**
			 * @name setCshTemp
			 * @desc store data temp before insert odiary
			 * @param String $promo_code :
		...
		function setCshTemp_BD($doc_tp,$promo_code,$promo_tp,$employee_id,$member_no='',$product_id,$quantity,
				$status_no,$product_status,$application_id,$card_status,
				$get_point,$promo_id,$discount_percent,$member_percent1,$member_percent2,
				$co_promo_percent,$coupon_percent,$promo_st,$net_amt,$discount,$discount_member,$check_repeat,$unitn=''){
					/**
		...
		function getCshTemp($order_no,$page,$qtype,$query,$rp,$sortname,$sortorder){
			 ...
			 */
			$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_tdiary2_sl");//trn_tdiary2_sl
			...
			if(count($arr_list)>0){
				...
				foreach($arr_list as $row){	
					...
					*/
					...
					$discount_tmp=number_format($row['sum_discount'],'2','.',',');
					$net_amt=$row['amount']-$row['sum_discount'];
					
					$price=parent::cutNum($row['price'],2);
					$amount=parent::cutNum($row['amount'],2);
					$net_amt=parent::cutNum($net_amt);
...
// 							);
		...
		function getSumCshTemp($xpoint,$flg_chk_point,$flg_tbl,$cn_amount){
			...
			if(count($row_l2h)>0){
				$discount=parent::getSatang($row_l2h[0]['sum_discount'],'normal');
				$member_discount1= parent::getSatang($row_l2h[0]['sum_member_discount1'],'normal');			
			 	$member_discount2= parent::getSatang($row_l2h[0]['sum_member_discount2'],'normal');						 	
			 	$co_promo_discount= parent::getSatang($row_l2h[0]['sum_co_promo_discount'],'normal'); 
			 	$coupon_discount= parent::getSatang($row_l2h[0]['sum_coupon_discount'],'normal');
			 	$special_discount= parent::getSatang($row_l2h[0]['sum_special_discount'],'normal');
			 	$other_discount= parent::getSatang($row_l2h[0]['sum_other_discount'],'normal');
				//new	
				//$amount= parent::getSatang($row_l2h[0]['sum_amount'],'normal');
				
			 	$amount= $row_l2h[0]['sum_amount'];
				$sum_discount=parent::getSatang($row_l2h[0]['sum_discount'],'normal');					
				//$net_amt=parent::getSatang($row_l2h[0]['sum_net_amt'],'normal');
				$net_amt=$amount - $sum_discount;
					
				//$net_chk_point=parent::getSatang($net_amt,'normal');
				$net_chk_point=$net_amt;
				...
				if($flg_chk_point=='yes'){
					//*WR21112016 for support get_point<>'Y'
					$net_ungetpoint=parent::getNetAmtUnGetPoint($tbl_temp);
					$net_getpoint=$net_chk_point-$net_ungetpoint;
					$point1=parent::getPoint1($net_getpoint);
					$point2=parent::getPoint2($net_getpoint);
		...
		function paymentNew($user_id,$cashier_id,$saleman_id,$doc_tp,$status_no,$member_no='',$member_percent,
							$special_percent,$net_amount,$ex_vat_amt,$ex_vat_net,$vat,$paid,$pay_cash,$pay_cash2,
							$pay_credit,$redeem_point,$change,$change2,$coupon_code,$pay_cash_coupon,$credit_no,
							$credit_tp,$bank_tp,$name,$address1,$address2,$address3,$point_begin,$point_receive,
							$point_used,$point_net,$xpoint,$get_point,$refer_doc_no,$cn_amount,$remark1,
		    $remark2,$xpoint_promo_code,$special_day='',$idcard='',$mobile_no='',$bill_manual_no='',$ticket_no='',$passport_no='',$application_id='',$not_cn_st=''){
			/**
			 * @desc
			 * @param
			 * @return
			...
			$this->discount=parent::getSatang($row_l2h[0]['sum_discount'],'normal');
			...
			if($mp_sum_discount>0){
					...
					//*WR15072015 coupon discount recal net_amt
					if($coupon_code!=''){
								$arr_coupon_s=explode('#',$coupon_code);
								$co_promo_code=$arr_coupon_s[0];									
								//*WR24012017 for support pro tour
								if($co_promo_code=="TOUR01"){
									$coupon_code=$arr_coupon_s[1]."#".$arr_coupon_s[2]."#".$arr_coupon_s[3];
								}else{
									$coupon_code=$arr_coupon_s[1];
								}
								//$coupon_code=$arr_coupon_s[1];
								$sql_ecoupon="SELECT* FROM promo_other WHERE  promo_code='$co_promo_code' AND '$this->doc_date' BETWEEN start_date AND end_date";
					...
					$point2=0;
					$total_point=0;
					//*WR21112016 resout problem stamp point on noral bill
					if($member_no!=''){
						$net_ungetpoint=parent::getNetAmtUnGetPoint($tbl_tmp_detail);
						$net_getpoint=$this->net_amt-$net_ungetpoint;
						$point1=parent::getPoint1($net_getpoint);
						$point2=parent::getPoint2($net_getpoint);
						//*WR15072015 coupon discount recal net_amt
			...
			//* ======= WR27072015 GUIDE_TOUR ==========
			$chk_cooperation=strpos($special_day,'#');
			if($chk_cooperation!==FALSE){
				$arr_cooperation=explode('#',$special_day);
				if($arr_cooperation[2]=='GROUP_TOUR'){
					$member_no=$arr_cooperation[0];
					$remark1=$arr_cooperation[1];
					$co_promo_code=$arr_cooperation[2];
					$idcard=$arr_cooperation[3];
					$special_day='';
				}
			}
			//* ======= WR27072015 GUIDE_TOUR ==========
			...
			if($this->chk_trans){
				...
				if($this->status_no=='01'){
				    $sql_upd="UPDATE 	trn_diary1
										SET
											status_no='01',
											application_id='$application_id'
										WHERE doc_no='$this->m_doc_no'";
				    $this->db->query($sql_upd);
				    $sql_upd2="UPDATE trn_diary2 SET	status_no='01' WHERE doc_no='$this->m_doc_no'";
				    $this->db->query($sql_upd2);
				}
				...
				parent::TrancateTable("trn_tdiary2_sl");
				parent::TrancateTable("trn_tdiary1_sl");
				$strResult= "1#".$this->m_doc_no."#".$this->doc_tp."#".$this->m_thermal_printer."#".$this->net_amt."#".$status_no."#".$this->m_branch_tp."#".$this->m_cashdrawer."#N#".$keyin_st;	
				//*WR30062016 for support product expire
				$objPdtEpire=new Model_ProductExpire();
				$objPdtEpire->updStockProductExpire();
				////////COMPLETE TRANSACTION///////////
			}else{
				$strResult= "0#".$this->msg_error."#".$this->doc_tp."#".$this->m_thermal_printer."#".$this->net_amt."#".$status_no."#".$this->m_branch_tp."#".$this->m_cashdrawer."#N#".$keyin_st;
			}//if	
			return $strResult;
		}//func

	...
	class MemberController extends Zend_Controller_Action{
		...
		function checkismemberAction(){
		    ...
		    if ($this->_request->isPost()){
		        ...
		        if($is_member=='N'){
		            $url_gcard="http://crmcpskh.ssup.co.th/app_service_cpskh/api_member/api_confirmby_mobile.php?mobile_no=".$mobile_no;
		...
		function formregistermobileAction(){
		    $this->_helper->layout()->disableLayout();
		    $objM=new Model_Member();
		    $chkbillregister=$objM->chkPlaybillRegister();	
			$this->view->dummy_code = $chkbillregister;
		...
		function formregisterAction(){
		    ...
		    $objM=new Model_Member();
		    $chkbillregister=$objM->chkPlaybillRegister();		    
		    echo json_encode($chkbillregister);
		}//func
	   ...
	   public function getproredeempointAction(){
		...
		function searchmemberAction(){
		    ...
		    if ($this->_request->isPost()){
		        ...
		        $name=$filter->filter($this->getRequest()->getPost('name'));
		        $surname=$filter->filter($this->getRequest()->getPost('surname'));
		        $ws = "http://crmcpskh.ssup.co.th/app_service_cpskh/api_member/search_member.php?";
		        $type = "json"; 
		        $shop=$this->m_branch_id;
		        $act = "search_lost";
		        $src = $ws."callback=jsonpCallback&mobile_no=".$mobile_no.
		 ...
		 * @modify:11112015
		 */
		function searchmember11062018Action(){
			$this->_helper->layout()->disableLayout();
			Zend_Loader::loadClass('Zend_Filter_StripTags');
			$filter = new Zend_Filter_StripTags();
			if ($this->_request->isPost()){
...
// 		}//func
		...
		function formkeymobileAction(){
				$this->_helper->layout()->disableLayout();
							
			}//action

		function reqotpcodeAction(){
			$this->_helper->layout->disableLayout();
			$this->_helper->viewRenderer->setNoRender(TRUE);
			$filter = new Zend_Filter_StripTags();
			$mobile_no=$filter->filter($this->getRequest()->getPost('mobile_no'));
			$status_chk = $filter->filter($this->getRequest()->getPost('status_chk'));
			$objMember=new Model_Member();
			$out  = $objMember->sentOtpCode($mobile_no,$status_chk);
			echo json_encode($out);
		}

		function checkotpcodeAction(){
			$this->_helper->layout->disableLayout();
			$this->_helper->viewRenderer->setNoRender(TRUE);
			$filter = new Zend_Filter_StripTags();
			$mobile_no=$filter->filter($this->getRequest()->getPost('mobile_no'));
			$otpcode=$filter->filter($this->getRequest()->getPost('otp_code'));
			$objMember=new Model_Member();
			$out = $objMember->checkOtpCode($mobile_no,$otpcode);
			echo json_encode($out);
		}
		
		function incdummyAction(){
			$this->_helper->layout->disableLayout();
			$this->_helper->viewRenderer->setNoRender(TRUE);
			$filter = new Zend_Filter_StripTags();
			$objMember=new Model_Member();
			$out = $objMember->incDummy();
		}

...
//				      	$objPos->markCancelBill($doc_no);
		...
		function listdocnowhAction(){
			...
			if ($this->_request->isPost()){
				...
				$actions=$filter->filter($this->getRequest()->getPost('actions'));
				$objAcc=new Model_Accessory();
				$this->view->arr_docno=$objAcc->getDocNofromwh($doc_tp_cancel,$actions,$start_date,$end_date);
			}
		}//func
		...
		function downloadinvfromwhAction(){
			$this->_helper->layout->disableLayout();
			$this->_helper->viewRenderer->setNoRender(TRUE);
			Zend_Loader::loadClass('Zend_Filter_StripTags');
			$filter = new Zend_Filter_StripTags();
			if ($this->_request->isPost()){
				...
				$dest_ip=$filter->filter($this->getRequest()->getPost('dest_ip'));
				$objPos=new Model_Accessory();
				$result=$objPos->download_inv_fromwh($doc_no,$dest_branch,$dest_ip);
				echo $result;
			}
		}//func

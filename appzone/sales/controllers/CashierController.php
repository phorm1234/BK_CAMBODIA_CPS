<?php 
	class CashierController extends Zend_Controller_Action{
		 ...
		 * @return imagepath 
		 */
		public function capturecameraAction()
		{
			...
			$cam_ip = $objCsh->capturecamera();
			$doc_no = $filter->filter($this->getRequest()->getPost('doc_no'));
			$shop = $this->_BRANCH_ID;
			$year = substr($this->_DOC_DATE, 0, 4);
			if (!empty($cam_ip)) {
				$user = 'admin'; // TODO change to select from database
				$cam_url = 'http://' . $user . '@' . $cam_ip;
				$cam_path = '/image/jpeg.cgi';

				// case new camera version
				// $path = '/Streaming/channels/1/picture';
				// $cam_url = 'http://admin:kfckfc8989@'.$cam_ip.$path;
				// $cam_path = '';
				
	
				$exe = shell_exec("ping -n 3 $cam_ip");
				if (strrpos($exe, "100% loss") > 0) {	// Ping check status camera  y : offline
					echo "y";
					exit();
				} else {
					$img = file_get_contents($cam_url . $cam_path);	// Get picture from camera
					if (!is_null($img)) {
						$folder = 'data_capture_' . $year;
						$new_file = $doc_no . '.jpg';
						$path = '/var/www/pos/htdocs/' . $folder;	//$path = $path.$folder;
						...
						shell_exec("cd $path");
						shell_exec("chown -R ss-$shop.www-data"); // TODO change cps to config  || $company_id = strtolower($this->_COMPANY_ID)
						shell_exec("chmod 777 -R $path");

						file_put_contents("$path/$new_file", $img);
						echo "/$folder/$new_file";	// return path to tag <img>
					} else {
						echo 'y';	// false file_get_contents
					}
				}
			} else {	// not found cam_ip_1 in table com_branch_cam
				echo $cam_url = '404';
			}
		}
		 ...
		 * @create 20082018
		 */
		public function addpdtautoshippingAction(){
			$this->_helper->layout->disableLayout();
			$this->_helper->viewRenderer->setNoRender(TRUE);
			$objCsh=new Model_Cashier();
			$objCsh->addPdtAutoShipping();
		}//func
		...
		function clearbillAction()
		{
			...
			if ($this->_request->isPost()) {
				// $action = $filter->filter($this->getRequest()->getParam('action'));
				// if($action == "updatePrice"){
				$objCashier = new Model_Cashier();
				$data = $objCashier->clearBill();
				// }
				echo $data;
			}
		}
		...
		public function memberontransAction(){
			...
			if ($this->_request->isPost()){
				...
				if(strlen($mem[0])==9 || strlen($mem[0])==10 || strlen($mem[0])==6){
					$on_trans=$objPos->memberOnTrans($mem[0]);
					echo $on_trans."#".$mem[0].'#M';
				}elseif($timecheck > $mem[3]){
					//echo $timecheck .">". $mem[3];
					echo "T"."#".$mem[1]."#".$timecheck."#".$mem[3]."#".$member_no.'#V';
				}else{
					$on_trans=$objPos->memberOnTrans($mem[1]);
					echo $on_trans."#".$mem[1]."#".$timecheck."#".$mem[3]."#".$member_no.'#V';
				}
...
//				$netvt=substr($netvt,0,$stangpos).$stang;
		...
		public function ajaxAction(){
			...
			if ($this->_request->isPost()) {						
				...
				if($action=='setCshTemp_BD'){	
					...
					$objCashier = new Model_Cashier();		
					$result=$objCashier->setCshTemp_BD($doc_tp,$promo_code,$promo_tp,$employee_id,$member_no,$product_id,$quantity,
													$status_no,$product_status,$application_id,$card_status,$get_point,
													$promo_id,$discount_percent,$member_percent1,$member_percent2,
													$co_promo_percent,$coupon_percent,$promo_st,$net_amt,$discount,$discount_member,$check_repeat);
					$arr_data=array(
						'action'=>'setCshTemp_BD',
						'data'=>$result
					);
					$this->view->arr_data=$arr_data;
				}//
		...
		function setcshvaltempAction(){
			...
			if ($this->_request->isPost()) {	
					...
					$coupon_percent= $filter->filter($this->getRequest()->getPost('coupon_percent'));
					$get_point= $filter->filter($this->getRequest()->getPost('get_point'));			
					$new_member = $filter->filter($this->getRequest()->getPost('new_member'));	
					//*WR17082016 for support lucky draw promotion
					$mem_card_info= $filter->filter($this->getRequest()->getPost('mem_card_info'));
					$objCashier = new Model_Cashier();		
					$result=$objCashier->setCshValTemp($new_member,$doc_tp,$promo_code,$employee_id,$member_no,$product_id,$quantity,
											$status_no,$product_status,$application_id,$card_status,$get_point,
											$promo_id,$discount_percent,$member_percent1,$member_percent2,
		...
		function paymentAction(){
			...
			if($actions=='savetransaction'){
				...
				//*WR27032018
				$application_id=$filter->filter($this->getRequest()->getPost('application_id'));
				$not_cn_st=$filter->filter($this->getRequest()->getPost('not_cn_st'));
				...
				$result=$objPay->paymentNew(
											...
											$refer_doc_no,
											$cn_amount,
				    $remark1,$remark2,$xpoint_promo_code,$special_day,$idcard,$mobile_no,$bill_manual_no,$ticket_no,$passport_no,$application_id,$not_cn_st);		
				if($result==FALSE){
					$result=0;
				}			
				$this->view->result=$result;

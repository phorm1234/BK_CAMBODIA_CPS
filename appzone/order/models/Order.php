...
class Model_Order{	
	...
	public function initOrderQuota(){
		...
		$avg = $this->calAVG($avg);
		//clear table
		$clear_com_order_quota="DELETE FROM `pos_ssup`.`com_order_quota` ";
		$rs_clear=$this->db->query($clear_com_order_quota);
		//get month_amount
		$sql="SELECT month_amount FROM com_order_quota_shop WHERE corporation_id = '".$this->corporation_id."'  AND company_id = '".$this->company_id."' AND branch_id = '".$this->branch_id."' ";
		$month_amount=$this->db->fetchOne($sql);
		...
		$insert_arr=array(
			...
			'balance_amount'=>$month_amount);
			// 'month_amount'=>$avg,
			// 'balance_amount'=>$avg);
		$rs_tdiary2_or=$this->db->insert('com_order_quota', $insert_arr);
		//echo $rs_tdiary2_or;
		//exit();
		return array('0'=>array('month_amount'=>$month_amount,'month_order'=>0,'balance_amount'=>$month_amount));
	}
}	
?>
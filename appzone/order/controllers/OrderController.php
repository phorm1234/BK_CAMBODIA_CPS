<?php 
	class OrderController extends Zend_Controller_Action{
		...
		////// test
		public function processtoordersAction(){
			$date=date("d/m/Y");
			$this->view->date=$date;
			$obj=new Model_Order();
			$view=$obj->getfixproductdetail();
			// $this->view->data=json_encode($view);
			$this->view->data=$view;
		...
		public function updateorderAction(){
			...
			$update_value = $filter->filter($this->getRequest()->getParam("update_value"));
			$obj=new Model_Order();
			$view=$obj->updateorder($update_value,$seq,"trn_tdiary2_or");
			// $this->view->data=json_encode($view);
			$this->view->data=$view;
		...
		public function getdataproductorderfixAction(){
			...
			$obj=new Model_Order();
			$view=$obj->getdataproductfix($sort);
		...
		//fix stock
		public function inserttdiaryorstockAction(){
			$this->_helper->layout()->disableLayout();
			$filter = new Zend_Filter_StripTags(); 
			$obj=new Model_Order();
			$product_id = $obj->convproduct_id($filter->filter($this->getRequest()->getParam("product_id"))); 
			...
			$view=$obj->insert_tdiary_or_stock($array_data);
			//print_r($view);

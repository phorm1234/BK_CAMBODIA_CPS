<?php 
	class Model_Summary{
		...
		function getSumฺAbaSales($date_start,$date_stop){
			/**
			 * @desc
			 * @param
			 */
		...
		function zreportPreviews($doc_date){
			...
			$sale_quantity=$this->getSumQtyByDay($doc_date);//จำนวนชิ้นที่ขาย
			$sale_net_amt=$this->getSumNetByDay($doc_date);//จำนวนเงิน net
			$sale_amount=$this->getSumAmtByDay($doc_date);//จำนวนเงิน amount			
			$bill_total=$this->getCountBillByDay($doc_date);//จำนวนบิลทั้งหมด
			
			//$bill_net_amt=$bill_net_amt;//จำนวนเงินสุทธิ
			
			$pay_cash=$this->getSumPayCash($doc_date);//จำนวนเงินสดสุทธิ
			$pay_credit=$this->getSumPayCredit($doc_date);//จำนวนเครดิตสุทธิ

			$pay_credit_aba=$this->getSumPayCreditABA($doc_date);//จำนวนเครดิตสุทธิABA
			
			$pay_cn=$this->getSumPayCn($doc_date);//จำนวนเงินเปลี่ยนสินค้าสุทธิ
			$bill_total_check=0;
...
// 				</table>";
		...
		function dailysalesPreviews($date_start,$date_stop){		
			...
			$total_aba_sales=$this->getSumฺAbaSales($date_start,$date_stop);

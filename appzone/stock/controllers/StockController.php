...
class StockController extends Zend_Controller_Action
{
	...
	public function getkeyproducttranfertoAction()
	{
		...
		$doc_no = $filter->filter($this->getRequest()->getParam("doc_no"));
		$obj = new Model_Stock();
		if($doc_no==''){	// for keyin dont have docno
			$view = $obj->sumivn_to_bykeyin();
		}else{
			$view = $obj->sumivn_to($doc_no);
		}
		$this->view->sum = $view;
		$this->view->chk = $chk;
	...
	public function tranin2shopheadtodiarytobykeyinAction()
	{
		$this->_helper->layout()->disableLayout();
		$filter = new Zend_Filter_StripTags();
		$doc_remark = $filter->filter($this->getRequest()->getParam("doc_remark"));
		...
		$inv = $filter->filter($this->getRequest()->getParam("inv"));
		$obj = new Model_Stock();
		$view = $obj->tranin2shopheadtodiarytobykeyin($doc_remark, $status_no, $doc_tp, $inv);
		$this->view->status = $view;
	}
	...
	public function checkpwdtokeyinAction()
	{
		$this->_helper->layout()->disableLayout();
		$filter = new Zend_Filter_StripTags();
		$pwd = $filter->filter($this->getRequest()->getParam("pwd"));
		$date = date("Y-m-d");
		$obj = new Model_Stock();
		$view = $obj->checkpwd_tokeyin($date, $pwd);
		$count = $obj->checkcount_tokeyin($date, $pwd);
	...
	public function tranfertoAction()
	{
		...
		$type_id = $obj->gettype_id($branch_id); //check branch_type
		$this->view->data = $view;
		$this->view->menu = $to_menu;
		$this->view->type_id = $type_id[0]['type_id'];
		$date = date("Y-m-d");
		$count = $obj->countinv_tokeyin($date);
		$this->view->count = $count;
	}
	...
	public function searchbranchAction()
	{
		...
		$dest_branch_id = $obj->gettype_id($branch_id); //check branch_type
		if (!$dest_branch_id) {
			$dest_branch_id[0]['branch_id'] = '0000';
		}
		$this->view->dest_branch_id = $dest_branch_id[0]['branch_id'];
		$this->view->key_branch_id = $branch_id;
	}
	//transfertobyorder
	...
	public function getdataproducttranfertotoAction()
	{
		$this->_helper->layout()->disableLayout();
		$filter = new Zend_Filter_StripTags();
		$page = $filter->filter($this->getRequest()->getParam("page"));
		...
		$sort = $sortname . " " . $sortorder;
		$obj = new Model_Stock();
		$view = $obj->getdataproduct_toto($sort, $doc_no);
		$this->view->arr_data = $view;
		$this->view->page = $page;
		$this->view->sum = $page;
	}
	...
	public function getproducttotoAction()
	{
		$this->_helper->layout()->disableLayout();
		$filter = new Zend_Filter_StripTags();
		$doc_no = $filter->filter($this->getRequest()->getParam("doc_no"));
		...
		$status_no = $filter->filter($this->getRequest()->getParam("status_no"));
		$obj = new Model_Stock();
		$view = $obj->sumivn_to_to($doc_no);
		$this->view->status_no = $status_no;
		$this->view->sum = $view;
		$this->view->doc_no = $doc_no;
		$this->view->chk = $chk;
		$chkdocno = $obj->checkinv_to_to($doc_no);
		$count_chkdocno = count($chkdocno);
		$this->view->chkdocno = $count_chkdocno;
	}
	...
	public function getdoctobytoAction()
	{
		$this->_helper->layout()->disableLayout();
		$obj = new Model_Stock();
		$view = $obj->getdoctobyto();
		$this->view->data = $view;
	}
	...
	public function printstockAction()
	{
		...
		if ($doc_tp[0]['stock_st'] > 0) {
			$head_txt = "ใบโอนสินค้าเข้า";
		} else {
			$head_txt = "ใบโอนสินค้าออก";
		}
	...
	public function checkproductinvoiceqtyAction()
	{
		...
		$this->view->data_onhand = $view_onhand;	// product onhand from com_stock_master where month=now
		$this->view->data_product = $view;	// product trans in trn_tdiary2_to
		$this->view->data_product_max = $view_max_inv;	// from trn_diary2 where doc_no = invoce
		$this->view->qty = $qty;	// qty from input
	}
	//end to
	//rq
	...
	public function checkproductinvoiceqtytoAction()
	{
		$this->_helper->layout()->disableLayout();
		$filter = new Zend_Filter_StripTags();
		$inv = $filter->filter($this->getRequest()->getParam("inv"));
	...
	public function addtmpdiaryAction()
	{
		...
		$qty = $filter->filter($this->getRequest()->getParam("qty"));
		$invoice = $filter->filter($this->getRequest()->getParam("invoice"));
		$obj = new Model_Stock(); //new 26-06-2019
		$pro_id = $obj->convproduct_id($product_id); // new 26-06-2019
		$data_array = array(
			'doc_tp' => $doc_tp,
			'status_no' => $status_no,
			// 'product_id'=>$product_id,  //old
			'product_id' => $pro_id[0]['product_id'],  //new 26-06-2019
			'qty' => $qty,
			'invoice' => $invoice,
		);
		// $obj=new Model_Stock(); //old

		...
		function chkLimit_qtybyone($promo_code){
			...
			if($rows_laststart>0){	
				...
				if($send_member_no!=""){
						...
						$ftp_missing = @fopen("http://$this->sv_online/ims/joke/app_service_op/process/member_bypeple.php?member_no=$send_member_no", "r");
...
doc_date between 
		...
		function up_point($doc_no){
			...
			if($status_no=="05"){	
				echo "Missing";			
				$fp = @fopen("http://10.100.53.2/ims/joke/app_service_op/process/member_new3.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);
				$fp = @fopen("http://10.100.53.2/ims/joke/app_service_op/process/member_his_seq.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);
			}
			
			if($status_no=="01" && $refer_member_id!=""){
				echo "Renew";	
				$fp = @fopen("http://10.100.53.2/ims/joke/app_service_op/process/member_new2.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);
				$fp = @fopen("http://10.100.53.2/ims/joke/app_service_op/process/member_renew_seq.php?doc_no=$doc_no", "r");
				$text=@fgetss($fp, 4096);			
			}
...
/*
		...
		function promo_chk_play($member_no,$status_show='Y'){
			...
			$sql="
				...
				$find="
					...
					SELECT 
						...
						last_day( '$apply_date' ) + INTERVAL 1 DAY as play_start,
						last_day( '$apply_date' ) + INTERVAL 1 DAY + INTERVAL 3 MONTH - INTERVAL 1 DAY as play_end,
						month(last_day( '$apply_date' ) + INTERVAL 1 DAY) as time_play,
					...
					from
						...
						where
						'$this->doc_date' between a.start_date and a.end_date  and a.year_apply='2016' and
						a.type_member  ='$type_member' and a.flag=''
				...
				if($rows==0){
					if($status_show=="Y"){
						echo "<center><span style='font-size:20px;color:#ff0000;'>ไม่ใช่ลูกค้าสมัครใหม่ในปีนี้ หรือโปรโมชั่นสมัครใหม่ยังไม่ถึงเวลาเล่น</span></center><br><br>";
					}
				}
				$conn_online=mysql_connect($this->sv_online,$this->user_online,$this->pass_online);	
				mysql_select_db($this->db_online);			
				$chk_timeplay="
				select if('$this->doc_date' between last_day( '$apply_date' ) + INTERVAL 1 DAY and 
						last_day( '$apply_date' ) + INTERVAL 1 DAY + INTERVAL 3 MONTH - INTERVAL 1 DAY,'Y','N') as chk_timeplay
				";
				$run_chk_timeplay=mysql_query($chk_timeplay,$conn_online);
				$data_chk_timeplay=mysql_fetch_array($run_chk_timeplay);
				$chk_timeplay=$data_chk_timeplay['chk_timeplay'];
				if($chk_timeplay=="N"){
					$play_ok="stop";
					$play_des="ไม่ใช่ช่วงเวลาเล่น";
				}else{
				
					//chk local
					$chk_local="select a.doc_date,a.doc_no,a.promo_code from trn_diary2 as a inner join trn_diary1 as b on a.doc_no=b.doc_no
					where 
					b.member_id in($missing_seq) and a.flag<>'C' and 
					a.promo_code in('OX07121115','OX07131115','OX07141115') and
					 a.doc_date between '2016-02-01' and '2017-03-31'
					 group by a.doc_no
					 order by a.doc_date
					  ";
					//echo $chk_local;
					...
					if($count_chk_local>=3){//เล่นแล้ว
						$play_ok="stop";
						$play_des="เล่นครบ 3 ครั้งแล้ว";
					}else{
						//chk online
						$conn_online=mysql_connect($this->sv_online,$this->user_online,$this->pass_online);	
						mysql_select_db($this->db_online);					
						$chk="select *
							from promo_play_history
							where 
							doc_date  between '2016-02-01' and '2017-03-31' and member_id in($missing_seq) and 
							 flag<>'C'  and promo_code in('OX07121115','OX07131115','OX07141115')
							 order by doc_date
						 ";
						...
						if($rows_chk>=3){	
							...
							$chk_month="select *
								from promo_play_history
								where 
								month(doc_date)=month('$this->doc_date') and member_id in($missing_seq) and 
								 flag<>'C'  and promo_code in('OX07121115','OX07131115','OX07141115')
							 ";
							 //echo $chk_month;
							$run_chk_month=mysql_query($chk_month,$conn_online);
							$rows_chk_month=mysql_num_rows($run_chk_month);
							if($rows_chk_month>0){
									$play_ok="stop";
									$play_des="เดือนนี้เล่นไปแล้ว";
							}else{
								$chk_bymonth_local="select a.doc_date,a.doc_no,a.promo_code from trn_diary2 as a inner join trn_diary1 as b on a.doc_no=b.doc_no
								where 
								b.member_id in($missing_seq) and a.flag<>'C' and 
								a.promo_code in('OX07121115','OX07131115','OX07141115') and
								 month(a.doc_date)=month('$this->doc_date')
								 group by a.doc_no
								 order by a.doc_date
								  ";
								//echo $chk_local;
								$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
								$data_chk_bymonth_local=$this->db->fetchAll($chk_bymonth_local, 2); 
								if(count($data_chk_bymonth_local)>0){
									$play_ok="stop";
									$play_des="เดือนนี้เล่นไปแล้ว";
								}else{
									$play_ok="$rows_chk";
				...
				if($count_chk_local>$rows_chk){
					$rows_play_all=$count_chk_local;
				}else{
					$rows_play_all=$rows_chk;
				}
				if($status_show=="Y"){
					echo "<center><span style='color:#21ba0a;font-size:26px;'>เล่นไปทั้งหมด <span style='color:#ff0000;'>$rows_play_all</span> ครั้งแล้ว</span><br>";
					for($xxx=1; $xxx<=$rows_chk; $xxx++){
						$data_chk=mysql_fetch_array($run_chk);
						$date_play_all=$this->set_formatdate($data_chk['doc_date']);
						echo "$data_chk[promo_code]  |  บิล $data_chk[doc_no]  ||  เมื่อ $date_play_all<br>";
					}
					echo "</center>";
				}
			    ...
			    while($data=mysql_fetch_assoc($result)){
					...
					$play_start=$this->set_formatdate($data['play_start']);
					$play_end=$this->set_formatdate($data['play_end']);
			    	if($des_cr=="This"){
			    		$promo_detail="(เล่นได้ในช่วง $play_start-$play_end  ทั้งหมด 3 ครั้ง แต่ได้เดือนละ 1 ครั้ง)";
					...
					$chk_bypro="select *
						from promo_play_history
						where 
						doc_date between '$data[start_date]' and '$data[end_date]' and month(doc_date)=month('$this->doc_date') and member_id in($missing_seq) and 
						 flag<>'C'  and promo_code ='$data[promo_code]'
					 ";
					$run_chk_bypro=mysql_query($chk_bypro,$conn_online);
					$rows_chk_bypro=mysql_num_rows($run_chk_bypro);
					if($rows_chk_bypro>0){
						$data_chk_bypro=mysql_fetch_array($run_chk_bypro);
						$date_play=$this->set_formatdate($data_chk_bypro['doc_date']);
						$promo_detail="ใช้แล้วในบิล $data_chk_bypro[doc_no] เมื่อ $date_play";
						$promo_color="#FF3333";
						$chk_play="N";
					}else{
						//chk local
						$chk_local_bypro="select a.doc_date,a.doc_no,a.promo_code from trn_diary2 as a inner join trn_diary1 as b on a.doc_no=b.doc_no
						where 
						a.doc_date between '$data[start_date]' and '$data[end_date]' and month(a.doc_date)=month('$this->doc_date') and
						b.member_id in($missing_seq) and a.flag<>'C' and 
						a.promo_code='$data[promo_code]' 
						 
						 group by a.doc_no
						 order by a.doc_date
						  ";
						//echo $chk_local_bypro;
						$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
						$data_local_bypro=$this->db->fetchAll($chk_local_bypro, 2); 
						if(count($data_local_bypro)>0){
							$date_play_local=$this->set_formatdate($data_local_bypro[0]['doc_date']);
							$date_no_local=$data_local_bypro[0]['doc_no'];
							$promo_detail="ใช้แล้วในบิล $date_no_local เมื่อ $date_play_local";
							$promo_color="#FF3333";
							$chk_play="N";
						}else{
			...
			//promotion new2017==================================================================
				...
				$find="
					...
					SELECT 
						*,
						if(promo_code is null,'N','Y')as chk_play,
						last_day( '$apply_date' ) + INTERVAL 1 DAY as play_start,
						last_day( '$apply_date' ) + INTERVAL 1 DAY + INTERVAL 3 MONTH - INTERVAL 1 DAY as play_end,
						month(last_day( '$apply_date' ) + INTERVAL 1 DAY) as time_play,
						year('$apply_date'+interval next_month month) as year_play,
						year(end_date) as promo_year
					from
						promotion_cr

						where
						'$this->doc_date' between start_date and end_date  and year_apply='2017' and
						type_member  ='$type_member' and flag=''
			...
			//	echo $find;
			    ...
			    while($data=mysql_fetch_assoc($result)){
					...
					mysql_select_db($this->db_online);			
					$chk_timeplay="
					select if('$this->doc_date' between  '$apply_date' + INTERVAL 1 DAY and 
							last_day( '$apply_date' ) + INTERVAL 1 DAY + INTERVAL 3 MONTH - INTERVAL 1 DAY,'Y','N') as chk_timeplay
						, date( '$apply_date' ) + INTERVAL 1 DAY  as play_start,
						 last_day( '$apply_date' ) + INTERVAL 1 DAY + INTERVAL 3 MONTH - INTERVAL 1  day as play_end
					";
					//echo $chk_timeplay;
					$run_chk_timeplay=mysql_query($chk_timeplay,$conn_online);
					...
					$chk_timeplay=$data_chk_timeplay['chk_timeplay'];
					if($chk_timeplay=="N"){
						$play_start=$this->set_formatdate($data_chk_timeplay['play_start']);
						$play_end=$this->set_formatdate($data_chk_timeplay['play_end']);
						if($des_cr=="This"){
							$promo_detail="(เล่นได้ในช่วง $play_start-$play_end)";
						}else if($des_cr=="Month_next_number"){
							$promo_detail="(เล่นได้ในช่วง $play_start-$play_end )";
						}
						...
						//chk local
						$chk_local="select doc_date,doc_no,member_id from trn_diary1 
						where 
						member_id in($missing_seq) and flag<>'C' and 
						co_promo_code ='$data[promo_code]' and
						 doc_date='$this->doc_date' limit 1
						  ";
						
						$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
						$data_chk_local=$this->db->fetchAll($chk_local, 2); 
						$count_chk_local=count($data_chk_local);
						if($count_chk_local>=1){//เล่นแล้ว
							$play_ok="stop";
							$play_des="เล่นแล้วด้วยบิลเลขที่" . $data_chk_local[0]['doc_no'] . " เมื่อวันที่ " . $data_chk_local[0]['doc_date'] . " ด้วยรหัสสมาชิก " .$data_chk_local[0]['member_id'];
							...
							mysql_select_db($this->db_online);					
							$chk="
							select x.doc_date,x.doc_no,x.member_id from (
								select doc_date,doc_no,member_id from trn_diary1 
								where 
								member_id in($missing_seq) and flag<>'C' and 
								co_promo_code ='$data[promo_code]' and
								 doc_date='$this->doc_date'
								union all
								select  doc_date,doc_no,member_id
									from promo_play_history
									where 
									doc_date  between '$data[start_date]' and '$data[end_date]' and member_id in($missing_seq) and 
									 flag<>'C'  and promo_code='$data[promo_code]'
									  limit 1
								 ) as x
								 limit 1
							...
							mysql_close($conn_online);
							if($rows_chk>=1){	
								$dataPlay=mysql_fetch_array($run_chk);
								$play_ok="stop";
								$play_des="เล่นแล้วด้วยบิลเลขที่ $dataPlay[doc_no] เมื่อวันที่ $dataPlay[doc_date] ด้วยรหัสสมาชิก $dataPlay[member_id]";
		...
		function promo_chk_play_renew($member_no,$status_show="Y"){
			...
			if($year_app==2015){ //รอบ
				if($type_member!="Re"){
					if($status_show=="Y"){
						echo  "สำหรับสมาชิกต่ออายุในปี 2015 เท่่านั้นค่ะ";
					}
					return false;
				}
				//renew 2015 ============================================================================================
				...
				if($rows_chk_set>0){
					...
					$month3=$month_app_pointx2+1;
					if($type_member=="Re" && $year_app==2015 && ($month_now==$month_app || $month_now==$month_app_pointx2  || $month_now==$month3) && strtotime(date("Y-m-d"))>=strtotime("2015-01-01") && strtotime(date("Y-m-d"))<=strtotime("2016-03-31") ){
			...
			//renew 2016 ============================================================================================
			if($year_app==2016){ //รอบ
				
				if($type_member!="Re"){
					if($status_show=="Y"){
						echo  "สำหรับสมาชิกต่ออายุในปี 2016 เท่่านั้นค่ะ";
						return false;
					}
				}
				//if($date_now>='2014-01-01'){
				$chk_set="select * from promo_play_history where member_id='$member_no' and flag<>'C' and promo_code in('OX07231115','OX07241115','OX07251115')";
				mysql_select_db($this->db_online);
				$run_chk_set=mysql_query($chk_set,$conn_online);
				$rows_chk_set=mysql_num_rows($run_chk_set);
				$data_chk_set=mysql_fetch_array($run_chk_set);
				...
				if($rows_chk_set==0){ //ที่ online ไม่มีให้ chk ที่เครื่องต่อ
					...
					where 
					b.member_id='$member_no' and a.flag<>'C' and 
					a.promo_code in('OX07231115','OX07241115','OX07251115') and
					 a.doc_date >='2016-01-01' limit 1 ";
					$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
					$data_chk_local=$this->db->fetchAll($chk_local, 2); 
					$rows_chk_set=count($data_chk_local);
					$promo_code_set=$data_chk_local[0]['promo_code'];
				...
				if($rows_chk_set>0){
					...
					$promo_color="#FF3333";
					$chk_play="N";
					if($promo_code_set=="OX07231115"){
						$show_play1=$x;
					}else if($promo_code_set=="OX07241115"){
						$show_play2=$x;
					}else if($promo_code_set=="OX07251115"){	
						$show_play2=$x;
					}
					...
					//echo "chk:$month_now==$month_app || $month_now==$month_app_pointx2";
					$month3=$month_app_pointx2+1;
					if($type_member=="Re" && $year_app==2016 && ($month_now==$month_app || $month_now==$month_app_pointx2  || $month_now==$month3)  && strtotime(date("Y-m-d"))>=strtotime("2016-01-01") && strtotime(date("Y-m-d"))<=strtotime("2017-03-31")  ){
				...
				array_push($xarray,array("start_date"=>"2016-01-01","end_date"=>"2017-02-28","promo_year"=>$promo_year,"promo_code"=>"OX07231115","promo_name"=>"ซื้อผลิตภัณฑ์ใดๆภายในร้านครบ 800 บาท(สุทธิ) รับฟรีสินค้าใดก็ได้ในร้าน 1 ชิ้น ราคาไม่เกิน 155 บาท","next_month"=>3,"dupble_point"=>$point_dupble,"promo_status"=>$chk_play,"promo_color"=>$promo_color,"time_play"=>$time_play,"promo_detail"=>$promo_detail,"xpoint"=>$point_dupble));  
				array_push($xarray,array("start_date"=>"2016-01-01","end_date"=>"2017-02-28","promo_year"=>$promo_year,"promo_code"=>"OX07241115","promo_name"=>"ซื้อผลิตภัณฑ์ใดๆภายในร้านครบ 1,200 บาท(สุทธิ) รับฟรีสินค้าใดก็ได้ในร้าน 1 ชิ้น ราคาไม่เกิน 255 บาท","next_month"=>3,"dupble_point"=>$point_dupble,"promo_status"=>$chk_play,"promo_color"=>$promo_color,"time_play"=>$time_play,"promo_detail"=>$promo_detail,"xpoint"=>$point_dupble));  
				array_push($xarray,array("start_date"=>"2016-01-01","end_date"=>"2017-02-28","promo_year"=>$promo_year,"promo_code"=>"OX07251115","promo_name"=>"ซื้อผลิตภัณฑ์ใดๆภายในร้านครบ 1,500 บาท(สุทธิ) รับฟรีสินค้าใดก็ได้ในร้าน 1 ชิ้น ราคาไม่เกิน 315 บาท","next_month"=>3,"dupble_point"=>$point_dupble,"promo_status"=>$chk_play,"promo_color"=>$promo_color,"time_play"=>$time_play,"promo_detail"=>$promo_detail,"xpoint"=>$point_dupble));  
		...
		function promo_sp($member_no){
	            ...
	            $fp = @fopen("http://10.100.53.2/ims/joke/app_service_op/api_member/api_pro_change_idcard.php?member_no=$member_no&doc_date=$this->doc_date", "r");
	            $ans=@fgetss($fp, 4096);
				//echo $ans;
				$arr=explode("###",$ans);
				$status=$arr[0];
		...
		function promo_chk_hbd($member_no){
			...
			mysql_query("SET collection_connection='utf8_general_ci'");
				...
				*/
				...
				$profile="
					SELECT 
						customer_id,apply_date,expire_date,
						day(apply_date) as day_app,
						month(apply_date) as month_app,
						year(apply_date) as year_app,
						month('$this->doc_date') as month_now,
						year('$this->doc_date') as year_now,
						age_card
					FROM `member_history` 
				...
				//if( ($year_now==2016 && (int)$month_now>=1  &&  (int)$month_now<=12) || ($year_now==2017 && (int)$month_now==1 && $m_hbd==12) ){ //รอบ
					//$first_hbd="$year_now-$m_hbd-01";
						

						$path_run="http://10.100.53.2/ims/joke/app_service_op/api_member/api_gen_promotion_hbd_test2.php?member_no=$member_no&year_now=$year_now&month_now=$month_now&month_hbd=$m_hbd";
					//echo $path_run;
					$fp = @fopen($path_run, "r");
					$text=@fgetss($fp, 4096);
					//echo $text;
					$arrpro=explode("@",$text);
					$gen_promo_code=$arrpro[0];
					$gen_promo_des=$arrpro[1];		
					$status_chk_play=$arrpro[2];	
					$status_chk_des=$arrpro[3];	
					$card_level=$arrpro[4];	
					$way_pro=$arrpro[5];	
					$start_chk=$arrpro[6];	
					$end_chk=$arrpro[7];	
					...
					if($status_chk_play=="Y"){
						//chk local
						$chk_local="
							select 
								a.* 
							from 
								trn_diary1 as a left join (select refer_doc_no from trn_diary1 where doc_date between doc_date-interval 30 day  and '$this->doc_date' and doc_tp='CN' and flag<>'C') as b
								on a.doc_no=b.refer_doc_no
							where 
								a.doc_date between '$this->doc_date'-interval 3 day and '$this->doc_date' and
								a.member_id='$member_no' and a.flag<>'C' and 
								a.co_promo_code in('OM02071113','OM02081113','OM02091113','OX02651014','OX02671014','OX02691014','OX02151115','OX02171115','OX02120416')  and
						...
						$this->db=SSUP_Controller_Plugin_Db::checkDbOnline("trn","trn_diary1");
						$data_chk_local=$this->db->fetchAll($chk_local, 2); 
						//print_r($data_chk_local);
						$count_chk_local=count($data_chk_local);
						if($count_chk_local>0){//เล่นแล้ว
							$chk_play="N";
							$status_chk_play="N";
							$promo_color="#FF3333";
							$doc_no_play=$data_chk_local[0]['doc_no'];
							$doc_date_play=$data_chk_local[0]['doc_date'];
							$member_id_play=$data_chk_local[0]['member_id'];
							$status_chk_des="ใช้ไปแล้วก่อนหน้านี้ บิล $doc_no_play วันที่ $doc_date_play ด้วยรหัสสมาชิก $member_id_play";							
						}
					}
						...
						$find_pro="
							...
							*,
							...
							from
							promotion_cr where type_member='HBD' and '$this->doc_date' between start_date and end_date  and flag<>'C' and year_apply='$year_now' and promo_code<>'OX02120416'	
...
select 
							*,
							...
							,last_day('$first_hbd' + interval 1 month) as end_hbd
							from
							promotion_cr where type_member='HBD' and  year_apply='$year_now' and promo_code='OX02120416'							
							 ";
							 */
						...
						$find_pro="
							...
							*,
							...
							,last_day('$first_hbd' + interval 1 month) as end_hbd
							from
							promotion_cr where type_member='HBD' and '$this->doc_date' between start_date and end_date  and flag<>'C'  and promo_code<>'OX02120416'	
...
select 
							*,
							...
							,last_day('$first_hbd' + interval 1 month) as end_hbd
							from
							promotion_cr where type_member='HBD' and promo_code='OX02120416'							
							 ";				 
					...
					if($rows_find_pro>0){
						...
						for($g=1; $g<=$rows_find_pro; $g++){
							...
							$chk_play_hbd=$data_find_pro['chk_play_hbd'];
							$end_hbd=$data_find_pro['end_hbd'];
							//echo "$promo_code!=$gen_promo_code<br>";
							if($promo_code!=$gen_promo_code){
								
								/*if(substr($member_no,0,2)=="15"){
									$promo_des_no="สำหรับบัตร White Card เท่านั้น";
								}else if(substr($member_no,0,2)=="ID" && $age_card==4){
									$promo_des_no="สำหรับบัตร White Card เท่านั้น";
								}else{
									$promo_des_no="สำหรับบัตร Gold Card เท่านั้น";
								}*/
								if($promo_code=="OX02151115"){
									$promo_des_no="สำหรับบัตร White Card เท่านั้น";
								}else if($promo_code=="OX02171115"){
									$promo_des_no="สำหรับบัตร Gold Card เท่านั้น";
								}else if($promo_code=="OX02120416"){
									$promo_des_no="สำหรับบัตร Platinum Card เท่านั้น";
								}
								
								
								array_push($xarray,array("start_date"=>$start_date_format,"end_date"=>$end_date_format,"promo_year"=>$promo_year,"promo_code"=>$promo_code,"promo_name"=>$promo_des,"next_month"=>"","dupble_point"=>"","promo_status"=>"N","promo_color"=>"#FF3333","time_play"=>"","promo_detail"=>$promo_des_no,"xpoint"=>"")); 
							}else{
								
								//ขยายสิทธิ์hbd
								if($chk_play_hbd=="no_play_hdb" && strtotime($this->doc_date)>=strtotime("2017-05-01") && strtotime($this->doc_date)<=strtotime("2017-05-31") && (int)$m_hbd>=1 and (int)$m_hbd<=3){
									$chk_play_hbd="yes_play_hbd";
								}
								
								
								
								if($chk_play_hbd=="yes_play_hbd"){ //อยู่ในช่วงการเล่น
									
								
									if($status_chk_play=="Y"){//เล่นได้
										$chk_play="Y";
										$promo_color="#66FF99";
										
									}else{//เล่นไม่ได้
										if($status_chk_play=="N"){
											$chk_play="N";
											$promo_color="#FF3333";
											$status_chk_des=$status_chk_des;
										}else{
											$chk_play="N";
											$promo_color="#FF3333";
											$status_chk_des="ไม่สามารถใช้สิทธิ์ได้";
										}
									...
									$promo_color="#FF3333";
									if($status_chk_des==""){
										$status_chk_des="เล่นได้ช่วงเดือน $first_hbd ถึง $end_hbd";
									}
								}		
								
								
								array_push($xarray,array("start_date"=>$start_date_format,"end_date"=>$end_date_format,"promo_year"=>$promo_year,"promo_code"=>$promo_code,"promo_name"=>$promo_des,"next_month"=>"","dupble_point"=>"","promo_status"=>$chk_play,"promo_color"=>$promo_color,"time_play"=>"","promo_detail"=>$status_chk_des,"xpoint"=>"")); 
					...
					//=================birthday gif
					$first_hbd="2017-$m_hbd-01";
					$find_pro="
							select 
							*,
							if('$this->doc_date' between start_date and end_date,'yes_play_date','no_play_date') as chk_play_date
							,if('$this->doc_date' between '$first_hbd' and last_day('$first_hbd'),'yes_play_hbd','no_play_hdb') as chk_play_hbd
							,last_day('$first_hbd') as end_hbd
							from
							promotion_cr where type_member='HBDSP' and '$this->doc_date' between start_date and end_date  and flag<>'C' 		
					";				 
					...
					if($rows_find_pro>0){
						...
						for($g=1; $g<=$rows_find_pro; $g++){
							...
							$promo_des=$data_find_pro['promo_des'];
							$chk_play_hbd=$data_find_pro['chk_play_hbd'];
							//ขายสิทธิ์ให้คนมาเล่น
							
							if($chk_play_hbd=="no_play_hdb" && (int) $m_hbd==1 && strtotime($this->doc_date)>=strtotime('2017-02-01')  && strtotime($this->doc_date)<=strtotime('2017-02-28') ){
								$chk_play_hbd="yes_play_hbd";
							}
							...
							if($count_chk_local>0){//เล่นแล้ว
								...
								$member_id_play=$data_chk_local[0]['member_id'];
								$status_chk_des="ใช้ไปแล้วก่อนหน้านี้ บิล $doc_no_play วันที่ $doc_date_play ด้วยรหัสสมาชิก $member_id_play";							
							}else{ //ถ้าที่เครื่องยังไม่เล่น ไป chk online ต่อ
							
								$chk_online="
									select x.doc_no,x.member_id,x.doc_date from (
									SELECT doc_no,member_id,doc_date FROM trn_diary1 where 
									doc_date='$this->doc_date' and flag<>'C' and member_id in($map_member) and
									co_promo_code ='$promo_code'
									union all
									SELECT doc_no,member_id,doc_date FROM promo_play_history where 
									doc_date between '$start_date' and '$end_date' and flag<>'C' and member_id in($map_member) and
									promo_code='$promo_code'
									) as x
									group by x.doc_no limit 1
								";
								//echo $chk_online;
								mysql_select_db($this->db_online);
								$run_chk_online=mysql_query($chk_online,$conn_online);
								$rows_chk_online=mysql_num_rows($run_chk_online);
								
								if($rows_chk_online>0){//เล่นแล้ว
									$billonline=mysql_fetch_array($run_chk_online);
									
									$chk_play="N";
									$status_chk_play="N";
									$promo_color="#FF3333";
									$doc_no_play=$billonline['doc_no'];
									$doc_date_play=$billonline['doc_date'];
									$member_id_play=$billonline['member_id'];
									$status_chk_des="ใช้ไปแล้วก่อนหน้านี้ บิล $doc_no_play วันที่ $doc_date_play ด้วยรหัสสมาชิก $member_id_play";							
								}
						}
							...
							if($chk_play=="Y"){
								if($chk_play_hbd=="yes_play_hbd"){ //อยู่ในช่วงการเล่น
										$status_chk_play="Y";
										if(strtotime($apply_date)<=strtotime($chk_hbd)){
											$chk_apply_befor="Y";
										}else{
											$chk_apply_befor="N";
										}
										
										if($chk_apply_befor=="N"){
											$status_chk_play="N";
											$status_chk_des="ต้องสมัครก่อนวันเกิดหรือตรงกับวันเกิดเท่านั้นค่ะ";
										}
		...
		function promo_chk_other($member_no){
				...
				$profile2="
					SELECT 	a.* FROM `member_register`  as a inner join member_history as b
					on a.customer_id=b.customer_id
					where
						b.member_no='$member_no' limit 1
				";
				//echo $profile2;
				mysql_select_db($this->db_online);
				$run_profile2=mysql_query($profile2,$conn_online);
				$data_profile2=mysql_fetch_array($run_profile2);
				$id_card=$data_profile2['id_card'];
				...
				$find="
					...
					from
						...
						where
						'$this->doc_date' between a.start_date and a.end_date and
						a.type_member='magazine'  and a.flag='' and a.promo_code not in('OM13060115','OM13050114','OM13160416')
				";
				//echo $find;
				...
				*,
				...
				if($rows_find_pro>0){
					...
					if($chk_play_date=="yes_play_date"){
							...
							if( ($type_member=="New" && strtotime($apply_date)<=strtotime($register_befor)) ||   ($type_member=="Re" && strtotime($apply_date)<=strtotime('2014-03-31'))  ){
								...
								$show_play="";
								$remark="ต้องสมัครภายในวันที่ 31/12/2556 หรือ ต่ออายุภายในเดือนมีนาคม 2557";//เหตุผล	
				...
				//รับ Beauty book 2015
				$find_pro="select 
				
				*,
				if('$this->doc_date' between start_date and end_date,'yes_play_date','no_play_date') as chk_play_date
				from
				promotion_cr where type_member='magazine' and promo_code='OM13060115' and flag<>'C' ";
				...
				if($rows_find_pro>0){
					...
					if($chk_play_date=="yes_play_date"){
							...
							if( ($type_member=="New" && strtotime($apply_date)<=strtotime($register_befor)) ||   ($type_member=="Re" && strtotime($apply_date)<=strtotime('2015-03-31'))  ){
								$show_play="<img src='../img/tick.png' hieght='20' width='20'></img>";
								$remark="";//เหตุผล
							}else{
								$show_play="";
								$remark="ต้องสมัครภายในวันที่ 31/12/2557 หรือ ต่ออายุภายในเดือนมีนาคม 2558";//เหตุผล	
				...
				//รับ Beauty book 2016
				$find_pro="select 
				
				*,
				if('$this->doc_date' between start_date and end_date,'yes_play_date','no_play_date') as chk_play_date
				from
				promotion_cr where type_member='magazine' and promo_code='OM13160416' and flag<>'C' ";
					...
					if($chk_play_date=="yes_play_date"){
							...
							*/
							...
							if($id_card!=""){
								...
								$play="select doc_no,doc_date,member_id from trn_diary1 where doc_date between '$start_date' and '$end_date'  and member_id in($map_member) and co_promo_code='$promo_code' and flag<>'C'   and month(doc_date)=month('$this->doc_date') 
								union all
	select doc_no,doc_date,member_id from trn_diary1 where doc_date between '$start_date' and '$end_date'  and idcard='$id_card' and co_promo_code='$promo_code' and flag<>'C'   and month(doc_date)=month('$this->doc_date') 
							...
							if($rows_play==1){//
								...
								if($year_play==$year_now){
									$show_play="";
									$remark="เดือนนี้ลูกค้าเล่นไปแล้วในบิล $doc_no เมื่อ $doc_date_format ด้วยบัตร $mem_play";//เหตุผล	
									...
									//chk local
									if($id_card!=""){
										$chk_local="select doc_no,doc_date,member_id from trn_diary1 where doc_date between '$start_date' and '$end_date' and member_id in($map_member) and co_promo_code='$promo_code' and flag<>'C'   and month(doc_date)=month('$this->doc_date') 
								union all
	select doc_no,doc_date,member_id from trn_diary1 where doc_date between '$start_date' and '$end_date' and idcard='$id_card' and co_promo_code='$promo_code' and flag<>'C'   and month(doc_date)=month('$this->doc_date')  ";
									 }else{
										$chk_local="select doc_no,doc_date,member_id from trn_diary1 where doc_date between '$start_date' and '$end_date' and member_id in($map_member) and co_promo_code='$promo_code' and flag<>'C'   and month(doc_date)=month('$this->doc_date') ";
